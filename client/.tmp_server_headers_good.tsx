        ...
        {result ? (
          <div className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              <StatCard icon={<Shield className="w-5 h-5 text-white/80" />} label="Security headers" value={`${securityCount}/9`} />
              <StatCard icon={<Clock3 className="w-5 h-5 text-white/80" />} label="Response time" value={`${result.responseTimeMs} ms`} />
              <StatCard icon={<Database className="w-5 h-5 text-white/80" />} label="Method used" value={result.methodUsed} />
              <StatCard icon={<Globe className="w-5 h-5 text-white/80" />} label="Status" value={`${result.statusCode}${result.ok ? " OK" : ""}`} />
            </div>
