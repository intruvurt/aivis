...
// Defensive guard: ensure generated file never starts with UTF-8 BOM.
fs.writeFileSync(outputPath, output.replace(/^\uFEFF/, ''), 'utf8');

console.log(
  `[blog:sync] wrote ${entries.length} entries to ${path.relative(clientRoot, outputPath)}`
);
if (skipped.length > 0) {
