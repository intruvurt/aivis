...
function LimitBadge({ used, max }: { used: number; max: number }) {
  ...
  return (
    <span
      className={`text-[11px] px-2 py-0.5 rounded-full font-medium ${atCap ? 'bg-red-500/20 text-red-300' : 'bg-white/10 text-white/60'}`}
    >
      {used}/{max}
    </span>
  );
...
type A11yButtonProps = React.ButtonHTMLAttributes<HTMLButtonElement> & {
  label: string;
};

function A11yButton({ label, title, ...props }: A11yButtonProps) {
  return <button {...props} aria-label={label} title={title || label} />;
}

type A11yInputProps = React.InputHTMLAttributes<HTMLInputElement> & {
  label: string;
};

function A11yInput({ label, id, ...props }: A11yInputProps) {
...
type A11ySelectProps = React.SelectHTMLAttributes<HTMLSelectElement> & {
  label: string;
};

function A11ySelect({ label, id, children, ...props }: A11ySelectProps) {
  const generatedId = useId();
  ...
  if (brand === 'slack') {
    return (
      <svg className="w-4 h-4" viewBox="0 0 122.8 122.8" aria-hidden="true">
        ...
        <path
          fill="#36C5F0"
          d="M38.5 77.1a15.4 15.4 0 0 1 30.7 0v38.4a15.4 15.4 0 1 1-30.7 0V77.1z"
        />
        <path fill="#2EB67D" d="M45.6 30.7a15.4 15.4 0 1 1 15.4-15.4v15.4H45.6z" />
        <path
          fill="#2EB67D"
          d="M45.6 38.5a15.4 15.4 0 0 1 0 30.7H7.2a15.4 15.4 0 1 1 0-30.7h38.4z"
        />
        <path fill="#ECB22E" d="M92.1 45.6a15.4 15.4 0 1 1 15.4 15.4H92.1V45.6z" />
        <path
          fill="#ECB22E"
          d="M84.3 45.6a15.4 15.4 0 0 1-30.7 0V7.2a15.4 15.4 0 1 1 30.7 0v38.4z"
        />
        <path fill="#E01E5A" d="M77.1 92.1a15.4 15.4 0 1 1-15.4 15.4V92.1h15.4z" />
        <path
          fill="#E01E5A"
          d="M77.1 84.3a15.4 15.4 0 0 1 0-30.7h38.4a15.4 15.4 0 1 1 0 30.7H77.1z"
        />
      </svg>
    );
  }
  ...
  if (brand === 'zapier') {
    return (
      <svg className="w-4 h-4" viewBox="0 0 24 24" aria-hidden="true">
        <path
          fill="#FF4F00"
          d="M10.2 1h3.6v6.2H10.2V1zm0 15.8h3.6V23h-3.6v-6.2zM1 10.2h6.2v3.6H1v-3.6zm15.8 0H23v3.6h-6.2v-3.6zM4.5 4.5l2.5-2.5 4.4 4.4-2.5 2.5-4.4-4.4zm10.1 10.1 2.5-2.5 4.4 4.4-2.5 2.5-4.4-4.4zM15.9 8.9l-2.5-2.5L17.8 2l2.5 2.5-4.4 4.4zM8.9 15.9l-2.5-2.5L2 17.8l2.5 2.5 4.4-4.4z"
        />
      </svg>
    );
  }
  ...
  if (brand === 'teams') {
    return (
      <svg className="w-4 h-4" viewBox="0 0 24 24" aria-hidden="true">
        ...
        <path
          fill="#5059C9"
          d="M22.5 10.5h-4.875a.375.375 0 0 0-.375.375v5.625a3.75 3.75 0 0 1-1.125 2.625A2.625 2.625 0 0 0 22.5 16.5v-6z"
        />
        <path fill="#7B83EB" d="M15.375 6a3.375 3.375 0 1 1-6.75 0 3.375 3.375 0 0 1 6.75 0z" />
        <path
          fill="#7B83EB"
          d="M18 10.5H6a.75.75 0 0 0-.75.75V18a5.25 5.25 0 1 0 10.5 0h2.25a.75.75 0 0 0 .75-.75v-6a.75.75 0 0 0-.75-.75z"
        />
        <path
          fill="#4B53BC"
          d="M12.375 10.5H5.25V18a5.25 5.25 0 0 0 7.125-4.875V10.5z"
          opacity=".1"
        />
      </svg>
    );
  ...
  if (brand === 'google_chat') {
    return (
      <svg className="w-4 h-4" viewBox="0 0 24 24" aria-hidden="true">
        ...
        <path
          fill="#fff"
          d="M17 8h-4V6h4a2 2 0 0 1 2 2v4h-2V8zM7 16h4v2H7a2 2 0 0 1-2-2v-4h2v4zM7 6a1 1 0 0 0-1 1v3h2V8h3V6H7zm10 12a1 1 0 0 0 1-1v-3h-2v2h-3v2h4z"
        />
      </svg>
    );
  }
...
const INTEGRATION_PRESETS: IntegrationPreset[] = [
  ...
  },
  {
    ...
    validator: (url) =>
      /^https:\/\/([\w-]+\.webhook\.office\.com|[\w-]+\.logic\.azure\.com\/workflows\/)/i.test(url),
...
function detectIntegration(url: string): IntegrationPreset['key'] | 'custom' {
  ...
  if (/^https:\/\/([\w-]+\.webhook\.office\.com|[\w-]+\.logic\.azure\.com\/workflows\/)/i.test(url))
    return 'teams';
...
const SectionCard: React.FC<{
  ...
  return (
    <div className="rounded-xl border border-white/10 bg-charcoal-light dark:bg-charcoal overflow-hidden">
      <A11yButton
        label={`${open ? 'Collapse' : 'Expand'} ${title} section`}
        type="button"
    ...
    <div className="space-y-3">
      <div className="p-3 rounded-lg bg-charcoal-deep border border-white/8">
        <p className="text-xs text-white/60 leading-relaxed">
          Operating model for cross-functional teams shipping AI-visibility improvements with clear
          ownership, measurable outcomes, and fast iteration loops.
        </p>
      </div>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
        {pillars.map((pillar) => (
          <div
            key={pillar.title}
            className="rounded-lg border border-white/8 bg-charcoal-deep px-3 py-2.5"
          >
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex gap-2">
        ...
        />
        <A11ySelect
          label="Scheduled rescan frequency"
          value={freq}
        ...
        <A11yButton
          label="Add scheduled rescan"
          onClick={handleAdd}
          disabled={atCap}
          title={
            atCap ? `Limit: ${limits.maxScheduledRescans} on your plan` : 'Add scheduled rescan'
          }
      ...
      {atCap && (
        <p className="text-xs text-amber-400/80">
          Plan limit reached ({limits.maxScheduledRescans} scheduled rescans). Upgrade for more.
        </p>
      )}
      ...
      {rescans.map((r) => (
        <div
          key={r.id}
          className="flex items-center gap-3 p-3 rounded-lg bg-charcoal-deep border border-white/8"
        >
          <div className="flex-1 min-w-0">
            <p className="text-sm text-white truncate">{r.url}</p>
            <p className="text-xs text-white/50">
              {r.frequency} &bull; Next:{' '}
              {r.next_run_at ? new Date(r.next_run_at).toLocaleDateString() : 'N/A'}
              {r.last_run_at && <> &bull; Last: {new Date(r.last_run_at).toLocaleDateString()}</>}
            </p>
          </div>
          <A11yButton
            label={r.enabled ? 'Disable scheduled rescan' : 'Enable scheduled rescan'}
...
function ThirdPartyIntegrationsPanel() {
  ...
  const handleAdd = async () => {
    ...
    if (preset && !preset.validator(value)) {
      toast.error(
        `That doesn\u2019t look like a valid ${preset.label} webhook URL. Check the URL and try again.`
      );
      return;
    }
    ...
    <div className="space-y-4">
      ...
      {workspaces.length > 1 && (
        <div>
          ...
          <A11ySelect
            label="Select workspace for integrations"
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm text-white flex items-center gap-2">
                    {label}
                    <span
                      className={`inline-block w-1.5 h-1.5 rounded-full ${hook.enabled ? 'bg-green-400' : 'bg-white/30'}`}
                    />
                  </p>
                ...
                <A11yButton
                  label={hook.enabled ? 'Pause integration' : 'Resume integration'}
        ...
        <div className="space-y-3">
          ...
          <div>
            <p className="text-xs font-medium text-white/70 mb-2">
              <span className="inline-flex items-center justify-center w-4 h-4 rounded-full bg-white/10 text-[10px] text-white/80 mr-1.5">
                1
              </span>
          ...
          {selected && (
            <div>
              ...
              {selected !== 'custom' && (
                <p className="text-[11px] text-white/40 mt-1.5">{preset?.hint}</p>
              )}
              {selected === 'custom' && (
                <p className="text-[11px] text-white/40 mt-1.5">
                  Receives <code className="text-white/60">POST</code> with{' '}
                  <code className="text-white/60">X-AiVIS.biz-Signature</code> HMAC header on audit
                  events.
                </p>
              )}
            </div>
          ...
          {!selected && hooks.length === 0 && (
            <div className="text-center py-4 px-3 rounded-lg border border-dashed border-white/10">
              ...
              <p className="text-xs text-white/50">
                Get notified when audits complete. Select a service above to connect it in under a
                minute.
              </p>
            </div>
          )}
      ...
      <p className="text-[11px] text-white/35 border-t border-white/5 pt-3">
        Triggers on: audit completed, audit failed, score changed, competitor updated, citation
        completed, rescan events. All payloads are HMAC-signed for security.
      </p>
    </div>
  );
}
...
function ApiKeysPanel() {
  ...
  return (
    <div className="space-y-3">
      ...
      {atCap && (
        <p className="text-xs text-amber-400/80">
          Plan limit reached ({limits.maxApiKeys} API keys). Upgrade for more.
        </p>
      )}

      <p className="text-xs text-white/50">
        Use: <code className="text-white/70">Authorization: Bearer avis_xxx</code> with{' '}
        <code className="text-white/70">GET /api/v1/audits</code>
      </p>
        ...
        >
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-xs text-white/50 font-mono">
              {k.key_prefix}•••••
              {k.last_used_at && (
                <> &bull; Last used: {new Date(k.last_used_at).toLocaleDateString()}</>
              )}
            </p>
          </div>
          <span
            className={`text-xs px-2 py-0.5 rounded-full ${k.enabled ? 'bg-green-500/20 text-green-400' : 'bg-white/10 text-white/50'}`}
          >
...
function WebhooksPanel() {
  ...
  return (
    <div className="space-y-3">
      ...
      <p className="text-xs text-white/50">
        Receives <code className="text-white/70">POST</code> with{' '}
        <code className="text-white/70">X-AiVIS.biz-Signature</code> header on{' '}
        <code className="text-white/70">audit.completed</code> events.
      </p>

      {atCap && (
        <p className="text-xs text-amber-400/80">
          Plan limit reached ({limits.maxWebhooks} webhooks). Upgrade for more.
        </p>
      )}
        ...
        >
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-xs text-white/50">
              ...
              {h.failure_count > 0 && (
                <span className="text-red-400"> &bull; {h.failure_count} failures</span>
              )}
              {h.last_triggered_at && (
                <> &bull; Last: {new Date(h.last_triggered_at).toLocaleDateString()}</>
              )}
            </p>
          </div>
          <A11yButton
            label={h.enabled ? 'Disable webhook' : 'Enable webhook'}
  ...
  return (
    <div className="space-y-3">
      <div className="rounded-lg bg-charcoal-deep border border-white/8 p-3 text-xs text-white/60 leading-relaxed">
        When an audit completes, AiVIS.biz can render a PDF and deliver it automatically. Email
        receives the PDF attachment. Slack, Discord, Zapier, and generic webhooks receive a summary
        plus public report link when available.
      </div>
      ...
      <div className="grid gap-2 md:grid-cols-2">
        ...
        <A11yInput
          label="Report delivery display name"
          type="text"
      ...
      <div className="flex gap-2">
        <A11yInput
          label={
            provider === 'email' ? 'Report delivery email target' : 'Report delivery webhook target'
          }
        ...
        />
        <A11yButton
          label="Add report delivery target"
          onClick={handleAdd}
          disabled={atCap}
          title={
            atCap
              ? `Limit: ${limits.maxReportDeliveries} targets on your plan`
              : 'Add delivery target'
          }
          className="px-3 py-2 rounded-lg bg-white/10 hover:bg-white/20 text-white text-sm font-medium transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
          type="button"
      ...
      {atCap && (
        <p className="text-xs text-amber-400/80">
          Plan limit reached ({limits.maxReportDeliveries} delivery targets). Upgrade for more.
        </p>
      )}
      ...
      {targets.length === 0 && (
        <p className="text-xs text-white/50 text-center py-2">
          No automatic report deliveries configured
        </p>
      )}
      ...
      {targets.map((delivery) => (
        ...
        >
          ...
          <div className="flex-1 min-w-0">
            <p className="text-sm text-white">
              {delivery.display_name ||
                delivery.provider.charAt(0).toUpperCase() + delivery.provider.slice(1)}
            </p>
            <p className="text-xs text-white/50 truncate">{delivery.target}</p>
            <div className="mt-1 flex flex-wrap gap-1.5">
              <span className="text-[10px] px-2 py-0.5 rounded-full bg-white/10 text-white/60">
                {delivery.include_pdf ? 'PDF' : 'No PDF'}
              </span>
              <span className="text-[10px] px-2 py-0.5 rounded-full bg-white/10 text-white/60">
                {delivery.include_share_link ? 'Share link' : 'No share link'}
              </span>
              <span className="text-[10px] px-2 py-0.5 rounded-full bg-white/10 text-white/60">
                {delivery.branded ? 'Branded' : 'Standard'}
              </span>
          ...
          <A11yButton
            label={delivery.enabled ? 'Disable report delivery' : 'Enable report delivery'}
    ...
    <div className="space-y-4">
        ...
        <div className="flex items-center gap-4">
          {logoPreview ? (
            <img
              ...
              className="h-12 w-auto rounded-lg border border-white/10 object-contain bg-white/5 px-2"
            />
          ) : (
      ...
      <div className="flex items-center justify-between p-3 rounded-lg border border-white/10 bg-white/[0.03]">
        <div>
          ...
          <p className="text-xs text-white/50">
            Add a branded cover page with logo, company details, and report metadata to exported
            PDFs
          </p>
        </div>
        <A11yButton
          label={showCoverPage ? 'Disable cover page' : 'Enable cover page'}
        ...
        >
          <span
            className={`absolute top-0.5 left-0.5 w-4 h-4 rounded-full bg-white transition-transform ${showCoverPage ? 'translate-x-5' : ''}`}
          />
        </A11yButton>
      </div>
      ...
      <div
        ...
        style={{ background: `linear-gradient(135deg, ${primaryColor}12, ${accentColor}12)` }}
      >
      ...
      try {
        ...
        if (f) {
          setFeatureCounts({
            ...
            reportDeliveries: {
              used: f.reportDeliveries?.count ?? 0,
              max: f.reportDeliveries?.max ?? 0,
            },
          });
        }
      } catch {
      ...
      <div className="space-y-4">
        ...
        <div className="p-4 rounded-xl bg-gradient-to-r from-white/25 to-white/14 border border-white/12 mb-2">
          <p className="text-sm text-white/80 flex items-center gap-2">
            ...
            <span>
              Advanced features require <strong>Alignment</strong> or higher. Upgrade to access API
              keys, webhooks, scheduled rescans, integrations, and more.
            </span>
          </p>
        </div>
        {[
          {
            ...
            desc: 'Automated monthly audits (daily with Signal+)',
            tier: 'Alignment+',
          },
          {
            icon: Key,
            title: 'API Keys',
            desc: 'Read-only API access to audit data',
            tier: 'Alignment+',
          },
          {
            icon: LinkIcon,
            title: 'Webhooks',
            desc: 'POST notifications on audit completion',
            tier: 'Alignment+',
          },
          {
            icon: MessageSquare,
            title: 'Integrations',
            desc: 'Notion, Slack, Teams, Google Chat, Zapier & more',
            tier: 'Alignment+',
          },
          {
            icon: Palette,
            title: 'White-Label Branding',
            desc: 'Branded PDF exports with your logo',
            tier: 'Signal+',
          },
        ].map((f) => (
          <div
            key={f.title}
            className="flex items-center gap-4 p-4 rounded-xl bg-charcoal-light border border-white/10 opacity-60"
          >
  ...
  return (
    <div className="space-y-3">
      ...
      <SectionCard
        ...
        counter={
          featureCounts ? (
            <LimitBadge used={featureCounts.webhooks.used} max={featureCounts.webhooks.max} />
          ) : undefined
        }
        defaultOpen
      >
      ...
      <SectionCard
        ...
        counter={
          featureCounts ? (
            <LimitBadge used={featureCounts.rescans.used} max={featureCounts.rescans.max} />
          ) : undefined
        }
        defaultOpen
      >
      ...
      <SectionCard
        ...
        counter={
          featureCounts ? (
            <LimitBadge used={featureCounts.apiKeys.used} max={featureCounts.apiKeys.max} />
          ) : undefined
        }
      >
        <ApiKeysPanel />
      ...
      <SectionCard
        ...
        counter={
          featureCounts ? (
            <LimitBadge
              used={featureCounts.reportDeliveries.used}
              max={featureCounts.reportDeliveries.max}
            />
          ) : undefined
        }
        defaultOpen
        ...
        {hasSignalAccess ? (
          ...
          <div className="p-4 rounded-lg bg-charcoal-deep border border-white/10">
            <p className="text-sm text-white/70 flex items-center gap-2">
              ...
              White-label branding requires <strong className="text-purple-300 ml-1">
                Signal
              </strong>{' '}
              or higher. Upgrade to add your logo, colors, and custom footer to exported PDFs.
            </p>
          </div>
        )}
      </SectionCard>
