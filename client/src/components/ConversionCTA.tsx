 ...
 */
export default function ConversionCTA({ variant, scansUsed = 0, scansLimit = 3, className = "" }: ConversionCTAProps) {
  if (variant === "usage-nudge") {
    ...
    return (
      <section className={`rounded-3xl border p-5 sm:p-6 ${urgent ? "border-amber-400/25 bg-gradient-to-br from-amber-500/10 to-amber-500/[0.03]" : "border-cyan-300/15 bg-gradient-to-br from-cyan-500/8 to-cyan-500/[0.02]"} ${className}`}>
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="min-w-0">
            ...
            <p className="mt-2 text-sm text-white/80">
              <span className={`font-semibold ${urgent ? "text-amber-300" : "text-white"}`}>{remaining}</span> of {scansLimit} free scans remaining this month
            </p>
            <div className="mt-2 h-1.5 w-48 max-w-full overflow-hidden rounded-full bg-white/10">
              <div className={`h-full rounded-full transition-all ${urgent ? "bg-amber-400" : "bg-cyan-400"}`} style={{ width: `${pct}%` }} />
            </div>
            {urgent && (
              <p className="mt-2 text-xs text-amber-200/70">Upgrade to Alignment for 60 monthly scans, report history, and evidence trails.</p>
            )}
          </div>
  ...
  if (variant === "scan-limit") {
    return (
      <section className={`rounded-3xl border border-amber-400/25 bg-gradient-to-br from-amber-500/10 to-orange-500/[0.04] p-5 sm:p-6 ${className}`}>
        <div className="flex items-start gap-3">
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="mt-1 text-sm text-white/70">
              You&apos;ve used all {scansLimit} Observer scans this cycle. Cached results are still accessible.
            </p>
            <div className="mt-4 flex flex-wrap gap-3">
              ...
              >
                ...
                Buy ScoreFix credits
              </Link>
            </div>
            <p className="mt-3 text-xs text-white/45">Alignment: 60 scans/mo for $49 &middot; ScoreFix: 250 one-time credits for $299</p>
          </div>
        </div>
      </section>
    );
  ...
  if (variant === "free-tool") {
    return (
      <section className={`rounded-3xl border border-cyan-300/15 bg-gradient-to-r from-cyan-500/8 to-blue-500/[0.04] p-5 sm:p-6 ${className}`}>
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="min-w-0">
            <p className="text-sm font-semibold text-white">Want the full picture?</p>
            <p className="mt-1 text-sm text-white/65">
              This free check covers one signal. A full AI visibility audit scores your site across schema, extractability, trust signals, and citation readiness — in one scan.
            </p>
          </div>
          <Link
            to="/analyze"
  ...
  if (variant === "blog") {
    return (
      <section className={`rounded-3xl border border-orange-400/20 bg-gradient-to-r from-orange-500/8 to-amber-500/[0.04] p-5 sm:p-6 ${className}`}>
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="min-w-0">
            <p className="text-sm font-semibold text-white">See how your site scores</p>
            <p className="mt-1 text-sm text-white/65">
              Run a free AI visibility audit and find out whether your site is citation-ready — in under 60 seconds.
            </p>
          </div>
          <Link
            to="/analyze"
