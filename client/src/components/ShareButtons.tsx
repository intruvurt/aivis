...
export default function ShareButtons({
  ...
  const shareText = (() => {
    if (score >= 90)
      return `Visibility benchmark for ${displayDomain}: ${score}/100 - citation-ready across ChatGPT, Perplexity, Claude, and Google AI.`;
    if (score >= 75)
      return `Visibility benchmark for ${displayDomain}: ${score}/100 - strong AI visibility with room to tighten structured data and citation signals.`;
    if (score >= 60)
      return `Visibility benchmark for ${displayDomain}: ${score}/100 - decent foundation but missing key signals that AI models look for.`;
    if (score >= 40)
      return `Visibility benchmark for ${displayDomain}: ${score}/100 - most answer engines are overlooking this site. Fixable with structured markup.`;
  ...
  function formatShareExpiryLabel(expiresAt?: string): string {
    ...
    if (diffMs > 60 * 60 * 24 * 365 * 5 * 1000)
      return 'Link set to never expire (long-lived public snapshot).';
  ...
  async function resolveShareUrl(): Promise<{ url: string; expiresAt?: string }> {
    ...
    try {
      ...
      if (!payload?.token && !payload?.share_path) {
        console.error(
          '[ShareButtons] Missing token/share_path:',
      ...
      setResolvedScanLabel(
        payload.scan_label || formatScanLabel(payload.scan_ordinal || scanCount)
      );
  ...
  async function handleCopy() {
    ...
    if (copied) {
      ...
      try {
        window.prompt('Copy this link (Ctrl+C / Cmd+C):', shareUrl);
      } catch {}
    }
  }
  ...
  async function handleNativeShare() {
    ...
    if (navigator.share) {
      navigator
        .share({
          title: title || `Citation Probability: ${score}/100`,
          text: shareText,
          url: shareUrl,
        })
        .catch(() => {});
    ...
    <div className="flex items-center gap-2 flex-wrap">
      ...
      <button
        ...
        className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full border text-xs font-semibold transition-all ${
          copied ? 'bg-charcoal border-white/10 text-white/80' : `${shareBtnBase} brand-border-cyan`
        }`}
