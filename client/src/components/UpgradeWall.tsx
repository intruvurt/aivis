...
export default function UpgradeWall({
  ...
  const trialUsed =
    user?.trial_used === true || (user?.trial_active === false && user?.trial_ends_at != null);
  ...
  const tierLabel =
    requiredTier === 'scorefix' ? 'Score Fix' : requiredTier === 'signal' ? 'Signal' : 'Alignment';
  ...
  return (
    <div className="rounded-2xl border border-white/10 bg-charcoal p-8 text-center">
      ...
      <div
        className={`inline-flex items-center gap-1.5 rounded-full border px-2.5 py-1 text-[11px] font-medium mb-3 ${tierBadgeColor}`}
      >
        <Lock className="w-3 h-3" />
        {tierLabel}
        {requiredTier !== 'scorefix' ? ' plan required' : ' pack required'}
      </div>

      <h3 className="text-xl font-semibold text-white mb-2">Upgrade to Access {feature}</h3>
      <p className="text-white/55 mb-4 max-w-md mx-auto">{description}</p>
      ...
      {featurePreview && featurePreview.length > 0 && (
        <div className="rounded-xl border border-white/[0.08] bg-white/[0.03] p-4 mb-6 text-left max-w-sm mx-auto">
          <p className="text-[11px] uppercase tracking-wider text-white/35 mb-3 font-medium">
            What you unlock
          </p>
      ...
      <div className="flex flex-col sm:flex-row items-center justify-center gap-3">
        ...
        <button
          onClick={() =>
            requiredTier && requiredTier !== 'scorefix'
              ? navigate(`/pricing?tier=${requiredTier}&confirm=1`)
              : navigate('/pricing')
          }
      ...
      {trialError && <p className="text-red-400/80 text-sm mt-3">{trialError}</p>}
    </div>
  );
}
