...
const categories: FAQCategory[] = [
    ...
    items: [
      ...
      },
      {
        ...
        answer: [
          ...
          "AiVIS generates findings from a live headless browser crawl of your actual URL. Every BRAG evidence ID corresponds to a specific, timestamped, crawl-observable field — not an inference. That is the difference between evidence and opinion.",
        ],
      },
      {
      ...
      },
      {
        ...
        answer: [
          ...
          "The only countermeasure is strong entity disambiguation at the source: consistent Organization JSON-LD across your site, sameAs links to all verified profiles, and specific, verifiable descriptions that resist paraphrase distortion. AiVIS measures entity clarity as a scored dimension and flags every inconsistency with an evidence ID.",
        ],
      },
      {
        question: "How does AI act as a 'summary builder' for my site and what are the failure modes?",
        answer: [
    ...
    items: [
      {
        ...
        answer: [
          "BRAG is the evidence framework that powers every AiVIS audit finding. It stands for Background, Reasoning, Action, Guidance — a structure that ensures every finding is grounded in a crawl-observable fact rather than a heuristic inference.",
      ...
      },
      {
        ...
        answer: [
          "AiVIS uses a Python-based validation microservice that runs parallel to the primary Node.js audit pipeline. When the headless browser crawl completes and the raw HTML is available, the Python service performs structured extraction analysis: it parses JSON-LD blobs, validates schema.org type hierarchies, verifies canonical URL consistency, and checks heading structure against the extracted body text.",
      ...
      },
      {
        ...
        answer: [
          "Every finding in an AiVIS audit report includes a BRAG evidence ID that traces back to a specific observation in the crawl record. If the finding is 'Organization schema is missing,' the evidence ID links to the raw HTML response for your URL, the JSON-LD extraction log showing no application/ld+json blocks with @type: Organization, and the timestamp of the crawl.",
      ...
      },
      {
        ...
        answer: [
          "AiVIS scores every page across six extraction readiness dimensions, each reflecting a different layer of how AI systems evaluate and use web content.",
  ...
  // ── 3. How AI Reads Your Site ─────────────────────────────────────────
  {
    ...
    items: [
      ...
      {
        question: "What is the difference between how Google crawls my site and how ChatGPT crawls it?",
        answer: [
      ...
      },
      {
        ...
        answer: [
          ...
          "The fix is to create explicit allowlist rules in your CDN configuration for the specific user-agent strings of AI crawlers you want to allow. AiVIS detects CDN blocking during audits by testing with multiple AI crawler user-agents and flagging discrepancies between browser access and crawler access.",
        ],
      },
      {
  ...
  // ── 4. Security — SSRF, XSS, and Scraping ────────────────────────────
  {
    ...
    items: [
      ...
      {
        question: "How does AiVIS protect against XSS when rendering audit results?",
        answer: [
      ...
      {
        question: "Why is it dangerous to optimize your site based purely on what AI tools say about it?",
        answer: [
      ...
      {
        question: "How does AiVIS prevent AiVIS itself from being scraped or abused?",
        answer: [
      ...
      {
        question: "What security headers does AiVIS enforce?",
        answer: [
          "AiVIS enforces a strict security header set on all responses. Strict-Transport-Security: max-age=31536000; includeSubDomains; preload forces HTTPS for all connections including future first-load requests. X-Frame-Options: DENY prevents clickjacking via iframe.",
    ...
    items: [
      {
        ...
        answer: [
          "AiVIS is an evidence-backed AI extraction audit platform. It answers one question: can ChatGPT, Perplexity, Claude, Gemini, and Google AI Overviews actually access, extract, and cite your website — and if not, exactly what is blocking them?",
          "The word 'evidence-backed' is load-bearing. Every finding AiVIS produces is traced to a specific crawl observation via a BRAG evidence ID. You are not told 'add schema' — you are told what schema type is missing, what property is absent, and what it costs you in extraction probability. The Python validation layer ensures these findings are deterministic and reproducible, not AI-inferred.",
          "AiVIS is built for 2026's multi-platform AI reality: a world where your site needs to be extractable by a dozen different AI systems, not just rankable in one search engine.",
        ],
      },
      {
        question: "Who specifically is AiVIS built for?",
        answer: [
          ...
          "Digital agencies managing client portfolios where AI visibility is becoming a deliverable. AiVIS supports multi-domain workflows, evidence trails that survive team changes, and client-shareable reports.",
      ...
      },
      {
        ...
        answer: [
          "If your site is blocked at the crawl stage — by robots.txt, CDN WAF, or server firewall — AiVIS detects this and reports it as a critical gating finding. This is the highest-priority class of finding because all other extraction improvements are irrelevant until crawler access is restored.",
      ...
      },
      {
        ...
        answer: [
          "No. Every AiVIS finding includes a plain-language description of the problem, a specific action item, and in Starter and above tiers, implementation code ready to copy and paste. The code is generated for your specific site context — not generic examples.",
    ...
    items: [
      ...
      },
      {
        ...
        answer: [
          ...
          "An AiVIS audit optimizes for AI extraction pipelines: crawler access, schema completeness, rendering, entity attribution. Success is measured in extraction probability — the likelihood that an AI answer engine selects your page as a citable source for relevant queries.",
          "Many SEO best practices carry over (HTTPS, clean URLs, meta descriptions), many do not (backlinks have no effect on AI extraction), and several are actively reversed (some CDN configurations recommended for SEO performance inadvertently block AI crawlers). AiVIS scores the dimensions that matter specifically for AI extraction, not the signals Google uses for ranking.",
        ],
      },
      {
  ...
  // ── 7. Plans & Pricing ────────────────────────────────────────────────
  {
    ...
    items: [
      ...
      },
      {
        ...
        answer: [
          "Score Fix (AutoFix PR) is a 250-credit one-time purchase at $299. Rather than a recurring audit subscription, it provides a structured remediation execution service: AiVIS generates a pull request (or equivalent deployment-ready code) for all critical and high-priority findings across your site.",
      ...
      {
        question: "Can I use AiVIS for client reporting in an agency context?",
        answer: [
          "Yes. Alignment and Signal tiers are both designed for agency workflows. Shareable report links can be sent to clients without requiring them to create an AiVIS account. PDF exports are formatted for client presentation with the BRAG evidence findings summarized in non-technical language.",
          ...
          "There is no branded white-label layer currently — reports are AiVIS-branded. If agency white-labeling is a requirement for your workflow, reach out through the contact form.",
        ],
      },
    ],
  },
  ...
  // ── 8. Privacy & Security ─────────────────────────────────────────────
  {
    ...
    items: [
      {
        question: "What data does AiVIS store from my audits?",
        answer: [
          "AiVIS stores the audit output — the score, dimension scores, BRAG findings, and recommendations — associated with your user ID, the target URL, and a timestamp. This is stored in the analysis_cache and audits tables in our PostgreSQL database.",
      ...
      },
      {
        ...
        answer: [
          "No raw payment card data is stored or processed by AiVIS servers. All payment processing is handled entirely by Stripe, a PCI DSS Level 1 certified payment processor. AiVIS receives and stores only the Stripe customer ID and subscription status — no card numbers, CVV codes, or billing addresses.",
          "Subscription state changes (upgrades, downgrades, cancellations) are processed via Stripe webhooks — cryptographically signed requests that AiVIS validates with the Stripe webhook secret before acting on. This prevents payment state manipulation by unauthorized parties.",
        ],
      },
      {
      ...
      {
        question: "Does AiVIS use my site data to train AI models?",
        answer: [
          "No. Crawled content from your site is used only to generate your specific audit findings and is not persisted after audit completion. AiVIS does not use audit content, user-submitted URLs, or finding data to train any AI model.",
  ...
  // ── 9. Technical & Platform ───────────────────────────────────────────
  {
    ...
    items: [
      ...
      },
      {
        ...
        answer: [
          "AiVIS exposes a Model Context Protocol (MCP) JSON-RPC 2.0 server at /api/mcp/* for Alignment and above tiers. MCP is an emerging protocol that allows AI assistants (Claude, Cursor, custom agents) to invoke AiVIS audit tools directly from their context window.",
          "This means you can run AiVIS audits from within an AI coding assistant: type 'audit this URL for AI extraction readiness' and the assistant invokes the AiVIS MCP tool, retrieves the findings, and incorporates them into its response. The 15+ MCP tools include audit execution, competitor comparison, mentor retrieval, and citation testing.",
      ...
      {
        question: "Does AiVIS test whether my site is actually being cited in live AI answers?",
        answer: [
      ...
      {
        question: "Does AiVIS monitor brand mentions across the web?",
        answer: [
  ...
  // ── 10. Interpreting Results ──────────────────────────────────────────
  {
    ...
    items: [
      {
        question: "My site ranked #1 on Google last month — why is my AiVIS score low?",
        answer: [
      ...
      },
      {
        ...
        answer: [
          "Re-run the AiVIS audit on the same URL after deploying fixes. Every BRAG evidence ID from the previous audit either resolves (finding disappears, evidence confirmed absent) or persists (finding remains, evidence still observed). The score delta between the two audits reflects the aggregate improvement.",
          "For schema fixes: use schema.org's Structured Data Testing Tool or Rich Results Test to verify the JSON-LD is syntactically valid before running the AiVIS audit. An invalid JSON-LD block that your site technically has still scores as missing because malformed schema is not extractable.",
          "For rendering fixes: use View Source (Ctrl+U) on your page. If the fix is correct, you should see the content in the raw HTML response now — not just in the browser-rendered DOM. AiVIS will confirm this on re-audit.",
        ],
      },
      {
    ...
    items: [
      {
        ...
        answer: [
          ...
          "After editing robots.txt, verify the result manually: visit yoursite.com/robots.txt in a browser and confirm the allow rules are present. Then run an AiVIS audit — the robots.txt dimension check will confirm whether the rules are correctly parsed.",
        ],
      },
      {
        ...
        answer: [
          ...
          "The most common errors: missing sameAs (eliminates cross-platform entity linking), using an inconsistent company name (different from what appears in page text or meta tags), and missing the @id property (prevents schema.org graph resolution). AiVIS flags all three as specific evidence findings.",
        ],
      },
      {
      ...
      },
      {
        ...
        answer: [
          "Content depth (20% of the AiVIS score) measures whether a page provides enough specific, verifiable information for an AI model to extract something genuinely useful. A page that says 'We help businesses grow with AI-powered solutions' gives an extraction pipeline almost nothing: no specific claims, no verifiable facts, no attributable statements.",
          "AiVIS measures content depth by analyzing word count, claim specificity, factual density (ratio of verifiable statements to promotional language), and structural segmentation (whether headings break content into extractable sections). A page scores high on depth if it contains multiple specific, independently verifiable claims organized under clear headings.",
      ...
      },
      {
        ...
        answer: [
          ...
          "AiVIS checks canonical declaration and consistency as part of the technical SEO dimension. The finding triggers if the canonical tag is missing, if it points to a different domain (a common CMS misconfiguration), or if the crawl returns a URL that does not match the declared canonical.",
        ],
      },
    ],
  },
      ...
      {isOpen ? (
        <div id={panelId} className="mt-4 space-y-3">
          {item.answer.map((para, i) => (
            <p key={i} className="text-sm leading-7 text-white/64">{para}</p>
          ))}
        </div>
      ) : null}
    </div>
...
export default function FAQ() {
  ...
  usePageMeta({
    ...
    structuredData: [
      ...
      buildFaqSchema(allItems.map((item) => ({ question: item.question, answer: item.answer.join(' ') }))),    
  ...
  const filteredCategories = useMemo(() => {
    ...
    return categories
      .map((category) => ({
        ...category,
        items: category.items.filter((item) => `${item.question} ${item.answer.join(' ')}`.toLowerCase().includes(needle)),
      }))
