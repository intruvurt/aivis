...
export default function InsightsPage() {
  usePageMeta({
    ...
    structuredData: [
      buildWebPageSchema({
        path: '/insights',
        name: 'AiVIS.biz Insights Hub | AI Visibility Playbooks',
        description:
          'Evidence-backed strategy hub for AI visibility, answer engine optimization, and citation-readiness implementation playbooks.',
      }),
      buildCollectionSchema(
      ...
      buildItemListSchema(
        INSIGHT_ARTICLES.map((article) => ({ name: article.title, path: article.path }))
      ),
      buildFaqSchema(
        [
          {
            question: 'What is the purpose of the AiVIS.biz Insights Hub?',
            answer:
              'The Insights Hub organizes strategic playbooks for improving AI visibility, extractability, and citation readiness using evidence-backed implementation guidance.',
          },
          {
            question: 'Are insights connected to real audit workflows?',
            answer:
              'Yes. The content is aligned to recurring audit dimensions such as schema quality, heading structure, metadata clarity, and machine-readability signals used in AiVIS.biz analysis.',
          },
          {
            question: 'How should teams use these playbooks?',
            answer:
              'Teams should run baseline audits, apply targeted playbook fixes, and re-audit to verify measurable movement in visibility and citation-related outcomes.',
          },
        ],
        { path: '/insights' }
      ),
  ...
  return (
    ...
    >
          ...
          <div className="grid md:grid-cols-2 gap-8 items-center">
            ...
            <div>
              ...
              <p className="text-white/65 text-sm leading-relaxed mb-6 max-w-lg">
                Evidence-based strategies for answer engine optimization, citation readiness, and
                geo-adaptive ranking. Every playbook is grounded in real audit data from the
                AiVIS.biz platform.
              </p>
              <div className="flex flex-wrap gap-2">
                {(['Strategy', 'AEO', 'Geo AI', 'Conversational'] as const).map((cat) => {
                  ...
                  return (
                    <span
                      key={cat}
                      className="inline-flex items-center gap-1.5 rounded-full border border-white/10 bg-white/[0.04] px-3 py-1 text-xs text-white/70"
                    >
            ...
            <div className="space-y-3">
              {[
                {
                  icon: TrendingUp,
                  label: 'Structured Data Focus',
                  value: '100%',
                  sub: 'every audit checks JSON-LD & schema markup',
                ...
                },
                {
                  ...
                  value: '4+',
                  sub: 'ChatGPT · Perplexity · Claude · Gemini',
                  color: 'text-blue-400',
                },
                {
                  icon: Shield,
                  ...
                  value: 'BRAG',
                  sub: 'findings grounded in scraped evidence IDs',
                ...
                },
                {
                  ...
                  value: '6',
                  sub: 'audit categories per scan',
                  color: 'text-purple-400',
                },
              ].map((stat) => (
                <div
                  key={stat.label}
                  className="flex items-center gap-4 rounded-xl border border-white/[0.06] bg-white/[0.02] p-3.5"
                >
      ...
      <header className="mb-10 rounded-2xl border border-white/10/70 bg-charcoal-deep p-8">
        <p className="text-xs font-semibold tracking-wide uppercase text-white/85 mb-3">
          Insights Hub
        </p>
        <h1 className="text-3xl md:text-4xl brand-title mb-4">AI Search Playbooks for 2026</h1>
        <p className="text-white/75 leading-relaxed">
          High-intent guides built for extractability, citation quality, and semantic reinforcement
          across the AiVIS.biz domain.
        </p>
      </header>
        ...
        {INSIGHT_ARTICLES.map((article) => (
          ...
          >
            <div className="flex items-center justify-between mb-3 text-xs">
              <span className="px-2 py-1 rounded-full bg-charcoal/15 text-white/85 font-medium">
                {article.category}
              </span>
              <span className="text-white/60">{article.readMinutes} min read</span>
            </div>
            <h2 className="text-xl font-semibold text-transparent bg-clip-text bg-gradient-to-r from-blue-300 to-indigo-400 mb-2">
              {article.title}
            </h2>
            ...
            <div className="flex flex-wrap gap-2 mb-4">
              {article.keywords.slice(0, 3).map((keyword) => (
                <span
                  key={keyword}
                  className="text-xs text-white/55 border border-white/10 rounded-full px-2 py-1"
                >
                  {keyword}
                </span>
              ))}
      ...
      <section className="rounded-2xl border border-white/10/70 bg-charcoal p-6">
        ...
        <p className="text-white/75 mb-4">
          Run audits after each content update, compare recommendation deltas, and track whether
          your high-value pages are becoming more citation-ready.
        </p>
