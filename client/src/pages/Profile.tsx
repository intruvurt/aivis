...
export default function ProfilePage() {
  ...
  if (!isAuthenticated || !user) {
    ...
    return (
      <Navigate to={`/auth?mode=signin&redirect=${encodeURIComponent(redirectTarget)}`} replace />
    );
  }
  ...
  const handleCopyInviteLink = async () => {
    ...
    try {
      ...
      window.alert(
        'We couldn’t copy automatically on this browser. Please copy the invite link manually from your profile.'
      );
    }
  };
  ...
  > = {
    observer: {
      bg: 'from-stone-400/18 to-amber-600/8',
      border: 'border-amber-300/20',
      text: 'text-amber-200',
      badge: 'bg-amber-500/12 text-amber-100 border-amber-300/20',
      ring: 'text-amber-300',
    },
    starter: {
      bg: 'from-emerald-400/16 to-teal-600/8',
      border: 'border-emerald-300/20',
      text: 'text-emerald-200',
      badge: 'bg-emerald-500/12 text-emerald-100 border-emerald-300/20',
      ring: 'text-emerald-300',
    },
    alignment: {
      bg: 'from-orange-400/16 to-rose-500/8',
      border: 'border-orange-300/20',
      text: 'text-orange-200',
      badge: 'bg-orange-500/12 text-orange-100 border-orange-300/20',
      ring: 'text-orange-300',
    },
    signal: {
      bg: 'from-rose-400/16 to-red-500/8',
      border: 'border-rose-300/20',
      text: 'text-rose-200',
      badge: 'bg-rose-500/12 text-rose-100 border-rose-300/20',
      ring: 'text-rose-300',
    },
    scorefix: {
      bg: 'from-amber-400/18 to-orange-500/10',
      border: 'border-amber-300/20',
      text: 'text-amber-200',
      badge: 'bg-amber-500/12 text-amber-100 border-amber-300/20',
      ring: 'text-amber-300',
  ...
  return (
    <div className="space-y-6 text-white">
      {/* ──── Profile Hero Card ──── */}
      <div
        className={`relative overflow-hidden rounded-[30px] border ${tc.border} bg-[linear-gradient(145deg,rgba(27,21,18,0.96),rgba(12,10,9,0.98))] shadow-[0_24px_70px_rgba(0,0,0,0.28)]`}
      >
        <div
          className={`absolute inset-0 bg-gradient-to-r ${tc.bg} opacity-60 pointer-events-none`}
        />
          ...
          <div className="flex items-start gap-5">
            ...
            <div className="relative flex-shrink-0">
              {avatarUrl ? (
                ...
                <div
                  className={`flex h-20 w-20 items-center justify-center rounded-2xl bg-gradient-to-br ${tc.bg} ring-2 ${tc.ring} ring-offset-2 ring-offset-charcoal-deep text-2xl font-bold ${tc.text}`}
                >
                  {initials}
                </div>
              )}
              <span
                className={`absolute -bottom-1.5 -right-1.5 flex h-7 w-7 items-center justify-center rounded-lg border ${tc.badge}`}
              >
            ...
            <div className="min-w-0 flex-1">
              ...
              <div className="mt-2 flex flex-wrap items-center gap-2">
                <span
                  className={`inline-flex items-center gap-1 rounded-full border px-2.5 py-0.5 text-xs font-semibold ${tc.badge}`}
                >
                ...
                {memberDays && (
                  <span className="inline-flex items-center gap-1 rounded-full border border-white/10 bg-white/5 px-2.5 py-0.5 text-xs text-white/60">
                    <Calendar className="h-3 w-3" /> Member for{' '}
                    {memberDays > 365
                      ? `${Math.floor(memberDays / 365)}y ${Math.floor((memberDays % 365) / 30)}mo`
                      : memberDays > 30
                        ? `${Math.floor(memberDays / 30)} months`
                        : `${memberDays} days`}
                  </span>
                )}
              </div>
              <p className="mt-2 text-xs text-white/45 leading-relaxed max-w-md">
                {tierPositioning}
              </p>
            </div>
          </div>
      ...
      <div className="space-y-6">
        ...
        <div className="grid gap-4 sm:grid-cols-3">
              ...
              <div
                className={`absolute inset-y-0 left-0 rounded-full transition-all duration-500 ${
                  usagePct >= 90
                    ...
                    : usagePct >= 70
                      ? 'bg-amber-500'
                      : `bg-gradient-to-r ${tc.bg.replace('/15', '/60').replace('/5', '/40')}`
                }`}
          ...
          {/* Tier summary card */}
          <div
            className={`rounded-2xl border ${tc.border} bg-gradient-to-br ${tc.bg} p-5 flex flex-col justify-between`}
          >
            <div>
              <p className={`text-[10px] font-semibold uppercase tracking-[0.18em] ${tc.text}`}>
                Running Plan
              </p>
            ...
            >
              {tierKey === 'scorefix' ? 'Buy more scans' : 'Upgrade plan'}{' '}
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {[
            { to: '/app/settings', icon: Settings, label: 'Settings', color: 'text-white/70' },
            { to: '/app/billing', icon: CreditCard, label: 'Billing', color: 'text-white/70' },
            { to: '/app/reports', icon: BookOpen, label: 'Reports', color: 'text-white/70' },
            { to: '/app/analytics', icon: BarChart3, label: 'Analytics', color: 'text-white/70' },
          ].map((action) => (
            <Link
              key={action.to}
              to={action.to}
              className="flex flex-col items-center gap-2 rounded-2xl border border-[#f4b860]/12 bg-[linear-gradient(180deg,rgba(27,21,18,0.88),rgba(16,13,11,0.96))] px-3 py-4 text-center hover:bg-[#201712] transition-colors group"
            >
              <action.icon
                className={`h-5 w-5 ${action.color} group-hover:text-white transition-colors`}
              />
        ...
        <div className="rounded-2xl border border-white/12 bg-charcoal-deep/50 p-5">
          ...
          <div className="rounded-xl border border-violet-300/20 bg-gradient-to-br from-violet-500/10 via-transparent to-cyan-500/10 p-4 mb-4">
            <p className="text-[11px] uppercase tracking-[0.16em] text-violet-200/90">
              Executive automation layer
            </p>
            <p className="mt-2 text-sm text-white/70 leading-relaxed">
              API keys, scheduled rescans, delivery workflows and developer ready docs for
              production integrations.
            </p>
            <div className="mt-3 grid gap-2 sm:grid-cols-3">
              <Link
                to="/api-docs"
                className="rounded-xl border border-cyan-300/20 bg-cyan-500/10 px-3 py-2 text-xs font-semibold text-cyan-100 transition hover:border-cyan-200/40 text-center"
              >
                API Docs
              </Link>
              <Link
                to="/integrations"
                className="rounded-xl border border-violet-300/20 bg-violet-500/10 px-3 py-2 text-xs font-semibold text-violet-100 transition hover:border-violet-200/40 text-center"
              >
                Integrations
              </Link>
              <Link
                to="/app/settings?section=advanced"
                className="rounded-xl border border-emerald-300/20 bg-emerald-500/10 px-3 py-2 text-xs font-semibold text-emerald-100 transition hover:border-emerald-200/40 text-center"
              >
                API Access
              </Link>
            </div>
          ...
          {referralSummary ? (
            <>
              <div className="rounded-xl border border-cyan-300/20 bg-gradient-to-br from-cyan-500/10 via-transparent to-violet-500/10 p-4 mb-4">
                ...
                <p className="mt-2 text-sm text-white/70 leading-relaxed">
                  Your code is{' '}
                  <span className="font-semibold text-white">{referralSummary.code}</span> - you get
                  +{referralSummary.constants.creditsToReferrer} credits and they get +
                  {referralSummary.constants.creditsToReferred} once referral eligibility is met (
                  {referralSummary.constants.requiredAuditsForReward || 5}+ audits), with{' '}
                  {referralSummary.constants.paidRewardMultiplier || 3}x rewards if they upgrade.
                </p>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                {[
                  ...
                  <div
                    key={stat.label}
                    className="rounded-lg border border-white/10 bg-charcoal-light/40 px-3 py-2.5 text-center"
                  >
