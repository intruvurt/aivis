...
import { ShieldCheck, Cpu, GitCompare, CheckCircle2, AlertTriangle, ArrowRight, FileSearch } from "lucide-react";
...
const stages = [
  {
    ...
    protocol:
      "The target URL is fetched through a three-step fallback pipeline: HTTP fetch, headless Puppeteer, and Cloudflare Browser Rendering. The full HTML is parsed into structured fields — title, meta tags, headings, links, images, JSON-LD blocks, robots.txt directives, llms.txt content, and sitemap entries. Each extracted field becomes a typed evidence item in the evidence ledger.",
    pipelineFunctions: [
      { fn: "scrapeWebsite()", file: "scraper.ts", desc: "3-step fetch pipeline (HTTP → Puppeteer → CF Browser Rendering)" },
      { fn: "parseHtml()", file: "scraper.ts", desc: "Regex extraction of title, meta, headings, links, images, JSON-LD" },
      { fn: "parseJsonLdStrings()", file: "scraper.ts", desc: "Parses all JSON-LD blocks from raw HTML" },
      { fn: "fetchRobotsForOrigin()", file: "scraper.ts", desc: "Fetches and parses robots.txt against 30+ AI crawler user-agents" },
      { fn: "fetchLlmsTxt()", file: "scraper.ts", desc: "Fetches and parses /llms.txt for LLM-specific directives" },
      { fn: "fetchSitemap()", file: "scraper.ts", desc: "Parses sitemap XML for discovery signals" },
      { fn: "extractEvidenceFromScrapedData()", file: "audit/evidenceLedger.ts", desc: "Converts raw scrape result into 30 typed evidence items with status and confidence" },
      { fn: "detectContradictions()", file: "audit/evidenceLedger.ts", desc: "Cross-evidence contradiction detection (e.g. title vs OG title mismatch)" },
      { fn: "extractSchemaSignalsFromHtml()", file: "server.ts", desc: "Extracts JSON-LD and Microdata schema types, validates graph structure" },
      { fn: "assessCitationStrength()", file: "citationStrength.ts", desc: "Heuristic citation readiness scoring based on crawl signals" },
    ],
    output: "Evidence ledger with 30+ typed fields, contradiction flags, and a structured evidence manifest.",
    constraint:
  ...
  },
  {
    ...
    pipelineFunctions: [
      { fn: "evaluateRules()", file: "audit/ruleEngine.ts", desc: "30 deterministic scoring rules across 11 families, each citing evidenceKeys[]" },
      { fn: "computeScore()", file: "audit/ruleEngine.ts", desc: "Hard blocker detection + family score breakdown + score cap enforcement" },
      { fn: "Evidence manifest construction", file: "server.ts", desc: "Builds 25+ ev_* evidence keys from scrape data for AI prompt injection" },
      { fn: "computeEvidenceScores()", file: "server.ts", desc: "Evidence-derived scoring bounds that constrain AI model output" },
      { fn: "buildAuditPrimaryPrompt()", file: "promptRegistry.ts", desc: "Injects evidence block, requires model to cite evidence_ids" },
      { fn: "callAIProvider()", file: "aiProviders.ts", desc: "Routes to OpenRouter with tier-based model allocation and fallback chain" },
      { fn: "verifyRecommendationEvidence()", file: "server.ts", desc: "Post-AI validation: computes proof_score per recommendation" },
    ],
    output: "AI analysis with per-recommendation proof_scores, rule-derived family scores, hard blocker caps, and evidence-bound findings.",
    constraint:
  ...
  },
  {
    ...
    pipelineFunctions: [
      { fn: "buildAuditPeerReviewPrompt()", file: "promptRegistry.ts", desc: "AI2 critique prompt — score_adjustment range −15 to +10" },
      { fn: "callAIProvider() [AI2]", file: "aiProviders.ts", desc: "Claude Sonnet 4.6 peer critique execution" },
      { fn: "detectPeerCritiqueEvidenceConflicts()", file: "server.ts", desc: "AI2 vs crawl evidence conflict detection" },
      { fn: "evaluateAi2PenaltyGate()", file: "server.ts", desc: "8-check quality gate for AI2 critique reliability" },
      { fn: "buildAuditValidationPrompt()", file: "promptRegistry.ts", desc: "AI3 validation gate prompt" },
      { fn: "callAIProvider() [AI3]", file: "aiProviders.ts", desc: "Grok 4.1 Fast validation gate execution" },
      { fn: "Score anchoring", file: "server.ts", desc: "Evidence-derived score + bounded AI2 adjustment (capped −6 to +8)" },
    ],
    output: "Consensus-validated score, peer critique summary, validation verdict, and conflict log.",
    constraint:
...
const enforcementItems = [
  {
    ...
    detail: "If evaluateRules() detects one or more hard blockers (e.g., AI crawler blocked in robots.txt, missing title tag, missing JSON-LD), computeScore() caps the score at 79 (or 59 for 3+ blockers) regardless of AI model output.",
    fn: "evaluateRules() + computeScore() → audit/ruleEngine.ts",
  },
  {
    label: "Integrity metadata",
    detail: "Every response includes buildAnalysisIntegrity() metadata: model identifiers, token counts, pipeline stage completion flags, and evidence manifest hash.",
    fn: "buildAnalysisIntegrity() → server.ts",
  },
  {
    label: "Dual-write persistence",
    detail: "The complete audit result — score, evidence manifest, recommendations, and pipeline metadata — is written to both Redis (7-day TTL) and PostgreSQL for durable storage.",
    fn: "AnalysisCacheService.set() → cacheService.ts",
  },
  {
    label: "Evidence manifest shipped in every response",
    detail: "The evidence_manifest is included in every cached and returned response. Any consumer of the audit result can independently verify which evidence was available during analysis.",
    fn: "evidence_manifest → JSON response payload",
  },
  {
    label: "Graceful degradation",
    detail: "If a multi-stage AI pipeline partially fails, valid earlier work is preserved. The response shape remains stable. Only if no valid output remains from any stage does the audit return an error.",
...
export default function TripleCheckMethodologyPage() {
  ...
  return (
    ...
    >
      ...
      {stages.map((stage) => (
        <section key={stage.number} id={`stage-${stage.number}`} className="mt-14" data-speakable={`stage-${stage.number}`}>
          <div className="flex items-center gap-3 mb-5">
            <div className={`flex h-8 w-8 items-center justify-center rounded-full border ${stage.tone}`}>
              <stage.icon className={`h-4 w-4 ${stage.accentText}`} />
            </div>
            <div>
              <span className={`text-xs font-bold uppercase tracking-[0.15em] ${stage.accentText}`}>Stage {stage.number}</span>
          ...
          <div className="rounded-2xl border border-white/10 bg-white/[0.02] overflow-hidden">
            <div className="grid grid-cols-[1fr_auto_2fr] gap-x-3 px-4 py-2 border-b border-white/10 bg-white/[0.03]">
              <span className="text-[11px] font-semibold uppercase tracking-widest text-white/40">Function</span>
              <span className="text-[11px] font-semibold uppercase tracking-widest text-white/40">File</span>
              <span className="text-[11px] font-semibold uppercase tracking-widest text-white/40">Purpose</span>
            </div>
      ...
      <section id="enforcement" className="mt-14">
        ...
        <p className="text-sm leading-6 text-white/56 mb-6">
          These constraints apply across all three stages. They are not configurable per-audit and cannot be overridden by AI model output.
        </p>
          ...
          {enforcementItems.map((item, i) => (
            <div key={i} className="rounded-2xl border border-white/10 bg-white/[0.03] p-4">
              <div className="flex items-start justify-between gap-4 flex-wrap">
                ...
                <span className="text-[11px] font-mono text-white/30 whitespace-nowrap self-start mt-0.5">{item.fn}</span>
              </div>
            </div>
          ))}
        </div>
      ...
      <section id="pipeline-flow" className="mt-14">
        <h2 className="text-xl font-semibold tracking-tight text-white mb-5">Pipeline execution order</h2>
      ...
      <section id="tier-availability" className="mt-14">
        ...
        <div className="rounded-2xl border border-white/10 bg-white/[0.02] overflow-hidden">
          <div className="grid grid-cols-[1fr_1fr_1fr_1fr] gap-x-2 px-4 py-2 border-b border-white/10 bg-white/[0.03]">
            <span className="text-[11px] font-semibold uppercase tracking-widest text-white/40">Tier</span>
            <span className="text-[11px] font-semibold uppercase tracking-widest text-white/40">Stage 1</span>
            <span className="text-[11px] font-semibold uppercase tracking-widest text-white/40">Stage 2</span>
            <span className="text-[11px] font-semibold uppercase tracking-widest text-white/40">Stage 3</span>
          </div>
          {[
            ...
            { tier: "Alignment ($49/mo)", s1: true, s2: true, s3: false, note: "GPT-5 Nano primary" },
            { tier: "Signal ($149/mo)", s1: true, s2: true, s3: true, note: "Triple-check: GPT-5 Mini → Claude Sonnet 4.6 → Grok 4.1 Fast" },
            { tier: "Score Fix ($299)", s1: true, s2: true, s3: true, note: "Triple-check + AutoFix PR" },
          ].map((row, i) => (
            <div key={i} className={`grid grid-cols-[1fr_1fr_1fr_1fr] gap-x-2 px-4 py-2.5 ${i % 2 === 0 ? "bg-transparent" : "bg-white/[0.015]"} ${i < 4 ? "border-b border-white/5" : ""}`}>
              <div>
                <span className="text-sm text-white/80">{row.tier}</span>
                <p className="text-[11px] text-white/30">{row.note}</p>
              </div>
              <span className={`text-sm self-center ${row.s1 ? "text-emerald-400" : "text-white/20"}`}>{row.s1 ? "✓" : "—"}</span>
              <span className={`text-sm self-center ${row.s2 ? "text-emerald-400" : "text-white/20"}`}>{row.s2 ? "✓" : "—"}</span>
              <span className={`text-sm self-center ${row.s3 ? "text-emerald-400" : "text-white/20"}`}>{row.s3 ? "✓" : "—"}</span>
            </div>
          ))}
        </div>
      </section>
      ...
      <section id="faq" className="mt-14">
        <h2 className="text-xl font-semibold tracking-tight text-white mb-5">Frequently asked questions</h2>
        <div className="space-y-4">
          {faqItems.map((item, i) => (
            <details key={i} className="group rounded-2xl border border-white/10 bg-white/[0.03] overflow-hidden">
      ...
      <section className="mt-14 rounded-3xl border border-white/10 bg-white/[0.03] p-6">
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          <Link to="/methodology" className="flex items-center gap-2 text-sm text-orange-300 hover:text-orange-200 transition-colors">
            <ArrowRight className="h-3.5 w-3.5" /> Full scoring methodology — CITE LEDGER &amp; BRAG protocol
          </Link>
          <Link to="/pricing" className="flex items-center gap-2 text-sm text-orange-300 hover:text-orange-200 transition-colors">
            <ArrowRight className="h-3.5 w-3.5" /> Tier comparison and pricing
          </Link>
          <Link to="/api-docs" className="flex items-center gap-2 text-sm text-orange-300 hover:text-orange-200 transition-colors">
            <ArrowRight className="h-3.5 w-3.5" /> API documentation (v1)
          </Link>
          <Link to="/glossary" className="flex items-center gap-2 text-sm text-orange-300 hover:text-orange-200 transition-colors">
