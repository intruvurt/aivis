...
export function buildReferralInviteMessage(args: ShareReferralArgs): { link: string; message: string } {
  ...
  const message = `I use AiVIS to audit AI visibility. Sign up with my invite link and unlock +${args.creditsToReferred} credits for you and +${args.creditsToReferrer} for me once eligibility is met (${requiredAudits}+ audits), with ${paidMultiplier}x rewards on paid upgrades.`;
  return { link, message };
}
