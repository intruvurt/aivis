...
function checkFocusHint(check: string): string {
  const hints: Record<string, string> = {
    ...
    meta_description:
      'Tighten title/meta for intent match and answer ready wording. (155 chars max.)',
    title:
      'Tighten title/meta for immediate intent match and answer ready wording. (60 chars max.)',
...
function platformFill(platform: string): string {
  const palette = PLATFORM_PALETTE[platform as keyof typeof PLATFORM_PALETTE] as
    | { fill?: string }
    | undefined;
...
function ChartExplainer({
  ...
  text: string;
  tone?: 'cyan' | 'amber' | 'rose' | 'indigo' | 'emerald' | 'orange';
}) {
...
function StatCard({
  ...
  const trendColor =
    trend === 'up' ? '#34d399' : trend === 'down' ? '#fb7185' : 'rgba(255,255,255,0.4)';
  return (
    <div
        ...
        {data.map(({ date, count }) => {
          ...
          const bg =
            count === 0
              ...
              : `rgba(249,115,22,${(intensity * 0.75).toFixed(2)})`;
          return (
            <div
              key={date}
              ...
              style={{
                background: bg,
                border: `1px solid rgba(249,115,22,${(intensity * 0.4).toFixed(2)})`,
      ...
      <div className="flex items-center gap-2 mt-2 text-xs text-white/40">
        ...
        {[0, 0.15, 0.35, 0.6, 0.9].map((v) => (
          <div
            ...
            style={{ background: v === 0 ? 'rgba(255,255,255,0.05)' : `rgba(249,115,22,${v})` }}
          />
        ))}
        <span>More</span>
      </div>
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-white/40">
                  ...
                  {item.audits} scan{item.audits !== 1 ? 's' : ''}
                </p>
              </div>
              <span
                className={`text-sm font-bold flex items-center gap-1 ${isImproved ? 'text-emerald-400' : isRegressed ? 'text-rose-400' : 'text-white/40'}`}
              >
                {isImproved ? (
...
function VisibilityRubricRadar({ familyScores }: { familyScores: Record<string, number> }) {
  ...
  const avgScore =
    FAMILY_ORDER.reduce((sum, k) => sum + (familyScores[k] ?? 0), 0) / FAMILY_ORDER.length;
  const weakest = FAMILY_ORDER.filter((k) => (familyScores[k] ?? 0) > 0).sort(
    (a, b) => (familyScores[a] ?? 0) - (familyScores[b] ?? 0)
  )[0];
  const strongest = FAMILY_ORDER.filter((k) => (familyScores[k] ?? 0) > 0).sort(
    (a, b) => (familyScores[b] ?? 0) - (familyScores[a] ?? 0)
  )[0];
  ...
  return (
    <div>
      ...
      <div className="flex flex-col lg:flex-row items-center gap-6">
        ...
        <div className="flex-1 w-full space-y-2">
              ...
              <div key={key} className="flex items-center gap-3">
                ...
                >
                  <div
                    ...
                    style={{ width: `${score}%`, background: familyColor(score) }}
                  />
                </div>
                <span
                  className="text-xs font-bold w-10 text-right"
                  style={{ color: familyColor(score) }}
                >
                  {score}
                </span>
              </div>
          ...
          <div className="flex items-center gap-4 text-xs text-white/50 mt-3 pt-3 border-t border-white/10">
            <span>
              Avg: <strong className="text-white/80">{avgScore.toFixed(1)}</strong>
            </span>
            {weakest && (
              <span>
                Weakest:{' '}
                <strong style={{ color: familyColor(familyScores[weakest] ?? 0) }}>
                  {FAMILY_LABELS[weakest]}
                </strong>
              </span>
            )}
            {strongest && (
              <span>
                Strongest:{' '}
                <strong style={{ color: familyColor(familyScores[strongest] ?? 0) }}>
                  {FAMILY_LABELS[strongest]}
                </strong>
              </span>
            )}
          </div>
      ...
      <ChartExplainer
        tone="indigo"
        text="The radar maps 9 deterministic rubric dimensions that drive your overall visibility score - address the flattest axis first."
      />
    </div>
  );
}
    ...
    >
      ...
      <div>
        ...
        <p className="text-xs text-white/55 mt-1">
          Averaging {stats.avgBlockersPerAudit} blocker{stats.avgBlockersPerAudit !== 1 ? 's' : ''}{' '}
          per audit. Hard blockers prevent AI models from extracting or citing your content - they
          should be resolved before optimizing other dimensions.
        </p>
      </div>
    </div>
  );
      ...
      {hardBlockers.length > 0 && (
        <div>
          <div className="flex items-center gap-1.5 mb-3">
            ...
            <span
              className="ml-auto text-xs px-2 py-0.5 rounded-full text-rose-300"
              style={{
              ...
              >
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-white/45">
                    {FAMILY_LABELS[r.family] || r.family} · {r.severity}
                  </p>
                </div>
      ...
      {topFailing.length > 0 && (
        <div>
          ...
          <div className="space-y-2">
            {topFailing.map((r) => {
              const barColor =
                r.passRate >= 80 ? '#34d399' : r.passRate >= 50 ? '#fbbf24' : '#fb7185';
              return (
                <div
                  key={r.ruleId}
                ...
                >
                  ...
                  >
                    <div
                      ...
                      style={{ width: `${r.passRate}%`, background: barColor }}
                    />
                  </div>
                  <p className="text-xs text-white/40 mt-1">
                    {r.passRate}% pass rate across {r.total} check{r.total !== 1 ? 's' : ''}
                  </p>
                </div>
              );
            })}
                ...
                return (
                  ...
                  >
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <span className="text-xs font-bold" style={{ color: familyColor(pRate) }}>
                        {pRate}%
                      </span>
                    </div>
                    <div
                    ...
                    >
                      <div
                        ...
                        style={{ width: `${pRate}%`, background: familyColor(pRate) }}
                      />
                    </div>
                    <p className="text-xs text-white/40 mt-1.5">
                      {familyRules.length} rule{familyRules.length !== 1 ? 's' : ''} · {totalFails}{' '}
                      fail{totalFails !== 1 ? 's' : ''} / {totalChecks} checks
                    </p>
                  </div>
                );
              })}
...
function ActionableInsights({
  ...
  data: AnalyticsData;
  pipeline: DeterministicPipeline | null;
}) {
  const insights: Array<{ text: string; icon: React.ReactNode; tone: string }> = [];

  if (data.averageScore > 0 && data.averageScore < 50) {
    insights.push({
      text: `Average score is ${data.averageScore.toFixed(0)}% - most audits are in the weak band. Focus on hard blockers first.`,
  ...
  if (data.averageScore >= 70) {
    insights.push({
      text: `Average score of ${data.averageScore.toFixed(0)}% puts you in the strong range. Optimize individual low-performing pages.`,
  ...
  if (pipeline) {
    const weak = FAMILY_ORDER.filter(
      (k) => (pipeline.avgFamilyScores[k] ?? 0) > 0 && (pipeline.avgFamilyScores[k] ?? 0) < 50
    ).map((k) => FAMILY_LABELS[k]);
    if (weak.length > 0) {
      insights.push({
        text: `Weak rubric dimensions: ${weak.join(', ')}. These drag your overall visibility score.`,
    ...
    const blockerPct =
      pipeline.hardBlockerStats.totalAudits > 0
        ? (pipeline.hardBlockerStats.auditsWithBlockers / pipeline.hardBlockerStats.totalAudits) *
          100
        : 0;
    if (blockerPct > 30) {
      insights.push({
        text: `${blockerPct.toFixed(0)}% of audits have hard blockers - these prevent AI citation entirely.`,
  ...
  if (worstDelta) {
    insights.push({
      text: `${worstDelta.domain} regressed by ${Math.abs(worstDelta.delta ?? 0).toFixed(0)} points - investigate recent changes.`,
  ...
  if (data.streakDays >= 7) {
    insights.push({
      text: `${data.streakDays}-day audit streak! Consistent scanning helps catch regressions early.`,
  ...
  return (
    <div
      ...
      style={{ background: 'rgba(10,14,28,0.75)', border: '1px solid rgba(249,115,22,0.15)' }}
    >
      <div className="space-y-2">
        {insights.map((insight, i) => {
          ...
          return (
            <div
              key={i}
              className={`rounded-xl px-4 py-3 flex items-start gap-3 text-sm ${s.text}`}
...
export default function AnalyticsPage() {
  ...
  usePageMeta({
    title: 'Analytics',
    description:
      'Track your AI visibility score trends, real score distribution, real/actual improvement turns, real platform performance, and real daily activity.',
    path: '/analytics',
    structuredData: [
      buildWebPageSchema({
        path: '/analytics',
        ...
        description:
          'Track AI visibility score trends across ChatGPT, Perplexity, Claude, and Google AIO. Monitor citation rates, entity recognition, evidence scores, and cross-platform performance over time.',
      }),
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        /* silent - community stats are non-critical */
      }
    })();
    return () => {
      cancelled = true;
  ...
  const diagnosticRanking = useMemo(() => {
    return Object.entries(data.seoDiagnosticsSummary || {})
      .map(([check, counts]) => {
        const total = counts.pass + counts.warn + counts.fail;
        return {
          check,
          label: CHECK_LABELS[check] || check.replace(/_/g, ' '),
          ...counts,
          total,
  ...
  const exportDomainsCsv = async () => {
    try {
      ...
      const response = await apiFetch(`${API_URL}/api/analytics/domains.csv?range=${range}`, {
        headers: {},
      });
      ...
      a.href = url;
      a.download = `aivis-analytics-${range}.csv`;
  ...
  const exportTrendsCsv = () => {
    ...
    const a = document.createElement('a');
    a.href = url;
    a.download = `aivis-trends-${range}.csv`;
    document.body.appendChild(a);
    a.click();
    a.remove();
  ...
  const exportSeoChecksCsv = () => {
    ...
    entries.forEach(([key, counts]) => {
      ...
      rows.push(
        [name, counts.pass || 0, counts.warn || 0, counts.fail || 0, total, rate].join(',')
      );
    });
    const blob = new Blob([rows.join('\n')], { type: 'text/csv' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = `aivis-seo-health-${range}.csv`;
    document.body.appendChild(a);
    a.click();
    a.remove();
    URL.revokeObjectURL(url);
  ...
  const miniMultiSeries = useMemo(() => {
    if (!data.recentTrend.length)
      return [] as Array<{ date: string; score: number; volume: number }>;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-4 rounded-[30px] border border-[#f4b860]/12 bg-[linear-gradient(145deg,rgba(27,21,18,0.94),rgba(13,11,10,0.98))] p-5 shadow-[0_24px_60px_rgba(0,0,0,0.28)] sm:flex-row sm:items-center sm:justify-between">
        <div>
          <p className="text-[10px] font-semibold uppercase tracking-[0.28em] text-[#f4b860]/70">
            Signal registry
          </p>
          <h1 className="mt-2 flex items-center gap-2 text-2xl font-semibold text-[#fff4e6]">
            <BarChart3 className="h-5 w-5 text-[#ff8a3d]" />
            Analytics
            {hasData && (
              <span className="ml-1 rounded-full border border-[#f4b860]/16 bg-[#1f1712] px-2.5 py-1 text-xs font-medium text-white/70">
                {data.totalAnalyses} scans
              </span>
            )}
          </h1>
          <p className="mt-2 max-w-2xl text-sm text-white/58">
            Cross-scan visibility movement, platform drift, and evidence-backed trend lines across
            your audit history.
          </p>
        </div>

        <div className="flex items-center gap-2 flex-wrap">
          <div className="flex gap-0.5 rounded-2xl border border-[#f4b860]/12 bg-[#161210] p-1">
            {(['7d', '30d', '90d', 'all'] as RangeKey[]).map((r) => (
              <button
                ...
                className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-all ${
                  range === r
                    ? 'border border-[#ff8a3d]/35 bg-[#2b1b12] text-[#ffd2a3]'
          ...
          <button
            ...
            disabled={loading}
            className="flex items-center gap-2 rounded-full border border-[#f4b860]/18 bg-[linear-gradient(135deg,#ffbe6f,#ff8a3d)] px-4 py-2 text-sm font-semibold text-[#180f0a] transition-all hover:brightness-110 disabled:opacity-50"
          >
      ...
      <FeatureInstruction
        ...
        accentClass="text-[#f4b860] border-[#f4b860]/25 bg-[#ff8a3d]/[0.06]"
        defaultCollapsed
      />
        ...
        {token && hasAccess && !loading && !error && (
          <>
            ...
            <SectionTitle
              title="Key Metrics"
              tone="orange"
              sub="Your personal audit performance at a glance"
            />
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3 mb-6">
              ...
              <StatCard
                icon={Globe}
                label="Sites Tracked"
                value={data.urlsAudited}
                accent={P.indigo}
              />
              <StatCard
                icon={Award}
                label="Best Score"
                value={data.bestScore ? `${data.bestScore}%` : '–'}
                accent={P.amber}
              />
              <StatCard
                icon={Flame}
                ...
                sub={data.streakDays > 0 ? 'consecutive days' : 'No streak yet'}
                accent={P.orange}
              />
              <StatCard
                icon={TrendingUp}
                label="Latest Score"
                value={data.latestScore ? `${data.latestScore}%` : '–'}
                accent={
                  data.latestScore >= 70 ? '#34d399' : data.latestScore >= 50 ? P.cyan : P.orange
                }
              />
            </div>
            ...
            {String(user?.role || '').toLowerCase() === 'admin' &&
              platformMetrics.timeframeMetrics.length > 0 && (
                <>
                    ...
                    <div className="flex items-center justify-between gap-3 flex-wrap mb-3">
                      <div>
                        <SectionTitle
                          ...
                          tone="cyan"
                          sub={`${platformMetrics.membershipTotals.realMembers} real · ${platformMetrics.membershipTotals.testMembers} test · ${platformMetrics.membershipTotals.totalMembers} total · ${platformMetrics.membershipTotals.stripePaidMembers} stripe paid`}
                        />
                      </div>
                      <div
                      ...
                      >
                        {(
                          ['1h', '24h', '7d', '30d', '90d', '180d', '365d'] as PlatformRangeKey[]
                        ).map((key) => (
                          <button
                            key={key}
                            onClick={() => setPlatformRange(key)}
                      ...
                      <>
                        <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-7 gap-3">
                          <StatCard
                            icon={Activity}
                            label="Analyses"
                            value={selectedPlatformMetrics.analysesRan}
                            sub="in window"
                            accent={P.orange}
                          />
                          <StatCard
                            ...
                            value={selectedPlatformMetrics.activeMembers}
                            sub="ran ≥1 audit"
                            accent={P.cyan}
                          />
                          <StatCard
                          ...
                          <StatCard
                            icon={Globe}
                            label="Traffic Proxy"
                            value={selectedPlatformMetrics.sessionTraffic}
                            sub="new login sessions"
                            accent={P.indigo}
                          />
                          <StatCard
                            icon={Layers}
                            label="Free Analyses"
                            value={selectedPlatformMetrics.freeMemberAnalyses}
                            sub="observer tier"
                            accent={P.amber}
                          />
                          <StatCard
                            icon={Zap}
                            label="Non-Free Analyses"
                            value={selectedPlatformMetrics.paidMemberAnalyses}
                            sub="elevated tier"
                            accent="#a78bfa"
                          />
                          <StatCard
                            ...
                            value={`${selectedPlatformMetrics.avgVisibilityScore.toFixed(1)}%`}
                            accent={P.cyan}
                          />
                        </div>
                        <div className="mt-3 grid grid-cols-2 sm:grid-cols-5 gap-2">
                          ...
                          >
                            ...
                            <span className="text-sm font-bold text-emerald-300">
                              {platformMetrics.membershipTotals.realMembers.toLocaleString()}
                            </span>
                          </div>
                          <div
                            className="rounded-xl px-3 py-2 flex items-center justify-between gap-2"
                          ...
                          >
                            ...
                            <span className="text-sm font-bold text-rose-300">
                              {platformMetrics.membershipTotals.testMembers.toLocaleString()}
                            </span>
                          </div>
                          <div
                            className="rounded-xl px-3 py-2 flex items-center justify-between gap-2"
                          ...
                          >
                            ...
                            <span className="text-sm font-bold text-amber-300">
                              {platformMetrics.membershipTotals.freeMembers.toLocaleString()}
                            </span>
                          </div>
                          <div
                            className="rounded-xl px-3 py-2 flex items-center justify-between gap-2"
                          ...
                          >
                            ...
                            <span className="text-sm font-bold text-violet-300">
                              {platformMetrics.membershipTotals.elevatedMembers.toLocaleString()}
                            </span>
                          </div>
                          <div
                            className="rounded-xl px-3 py-2 flex items-center justify-between gap-2"
                          ...
                          >
                            ...
                            <span className="text-sm font-bold text-cyan-300">
                              {platformMetrics.membershipTotals.stripePaidMembers.toLocaleString()}
                            </span>
                          </div>
                        </div>
                      </>
            ...
            {communityBenchmarks && communityBenchmarks.total_audits > 0 && (
              ...
              >
                <SectionTitle
                  ...
                  tone="indigo"
                  sub={`Aggregate stats across all ${communityBenchmarks.total_audits.toLocaleString()} platform audits - no individual data exposed`}
                />
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3 mb-4">
                  <StatCard
                    icon={Activity}
                    label="Total Audits"
                    value={communityBenchmarks.total_audits.toLocaleString()}
                    accent={P.indigo}
                  />
                  <StatCard
                    icon={Target}
                    label="Avg Score"
                    value={`${communityBenchmarks.avg_score}%`}
                    accent={P.cyan}
                  />
                  <StatCard
                    icon={Gauge}
                    label="Median Score"
                    value={`${communityBenchmarks.median_score}%`}
                    accent={P.amber}
                  />
                  <StatCard
                    icon={TrendingDown}
                    label="Min Score"
                    value={`${communityBenchmarks.min_score}%`}
                    accent="#f87171"
                  />
                  <StatCard
                    icon={TrendingUp}
                    label="Max Score"
                    value={`${communityBenchmarks.max_score}%`}
                    accent="#34d399"
                  />
                </div>
                ...
                {communityBenchmarks.distribution &&
                  (() => {
                    ...
                    const distData = buckets.map((b) => ({
                      label: b.label,
                      count:
                        (communityBenchmarks.distribution as Record<string, number>)[b.key] || 0,
                      color: b.color,
                    }));
                        ...
                        >
                          {distData.map(({ label, count, color }) => {
                            ...
                            return (
                              ...
                              >
                                <span className="text-[11px] leading-none mb-1" style={{ color }}>
                          ...
                          >
                            ...
                            <div className="min-w-0">
                              ...
                              <p className="text-xs text-white/50">
                                {best.avg_score}% avg across {best.sample_count.toLocaleString()}{' '}
                                audits
                              </p>
                            </div>
                          </div>
                        ...
                        {worst && (
                          ...
                          >
                            ...
                            <div className="min-w-0">
                              ...
                              <p className="text-xs text-white/50">
                                {worst.avg_score}% avg across {worst.sample_count.toLocaleString()}{' '}
                                audits
                              </p>
                            </div>
                          </div>
            ...
            {(data.totalPasses > 0 || data.totalFails > 0) && (
              <>
                <SectionTitle
                  ...
                  tone="emerald"
                  sub="Pass / Warning / Fail totals across all audited checks"
                />
                <div
                  className="rounded-2xl mb-6 px-5 py-4 flex flex-wrap items-center gap-6"
                  style={{
            ...
            {(activeTab === 'overview' || !hasData) && (
              <div className="space-y-6">
                {hasData && (
                  <>
                    <SectionTitle
                      ...
                      tone="cyan"
                      sub="Score distribution, activity cadence & SEO health at a glance"
                    />
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      ...
                      >
                        ...
                        <ChartExplainer
                          tone="amber"
                          text="Distribution shows where your audits cluster, so you can move volume from weak bands into stable and strong ranges."
                        />
                      </div>
                      <div
                        className="rounded-2xl p-5"
                      ...
                      >
                        ...
                        <ChartExplainer
                          tone="indigo"
                          text="Activity tracks operational cadence; consistent audit frequency usually precedes stable score gains."
                        />
                      </div>
                      <div
                      ...
                      >
                        <SeoHealthRadial
                          passes={data.totalPasses}
                          warns={data.totalWarns}
                          fails={data.totalFails}
                        />
                        <ChartExplainer
                          tone="emerald"
                          text="SEO health summarizes pass/warn/fail pressure so teams can prioritize fixes with the highest structural impact."
                        />
                      </div>
                    </div>
                  </>
                ...
                {/* ── Visibility Rubric Radar ──────────────────────────── */}
                {hasData &&
                  Object.keys(data.deterministicPipeline?.avgFamilyScores ?? {}).length > 0 && (
                    <div
                      className="rounded-2xl p-5"
                      style={{
                        background: 'rgba(10,14,28,0.75)',
                    ...
                    >
                      <VisibilityRubricRadar
                        familyScores={data.deterministicPipeline!.avgFamilyScores}
                      />
                      <ChartExplainer
                        tone="indigo"
                        text="Rubric radar maps your average score across the 9 visibility dimensions so you can spot structural weaknesses at a glance."
                      />
                    </div>
                  )}
                ...
                <SectionTitle
                  title="Score Summary"
                  tone="orange"
                  sub="Best, average, latest & worst visibility scores"
                />
                ...
                {hasData && (
                  <>
                    <SectionTitle
                      ...
                      tone="amber"
                      sub="AI-generated recommendations based on your audit data"
                    />
                ...
                {!hasData && (
                  <div className="text-center py-16">
                    ...
                    <p className="text-white/50 mb-4">
                      No analyses yet. Run your first analysis to populate data here.
                    </p>
                    <button
            ...
            {activeTab === 'trends' && (
              <div className="space-y-6">
                <SectionTitle
                  title="Score & Volume Trends"
                  tone="amber"
                  sub="Track how your visibility scores and audit cadence evolve over time"
                />
                      ...
                      <ComposedChart key={trendMetric} data={data.recentTrend}>
                        <defs>
                          ...
                          <linearGradient id="volBar" x1="0" y1="0" x2="0" y2="1">
                            <stop offset="0%" stopColor={P.indigo} stopOpacity={0.8} />
                            <stop
                              offset="100%"
                              stopColor={P.indigoDeep ?? P.indigo}
                              stopOpacity={0.25}
                            />
                          </linearGradient>
                        </defs>
                        ...
                        {isScoreFocus ? (
                          <>
                            ...
                            <ReferenceLine
                              ...
                              strokeOpacity={0.5}
                              label={{
                                value: `avg ${data.averageScore}%`,
                                fill: P.amberDeep ?? P.amber,
                                fontSize: 10,
                              }}
                            />
                            <Area
                        ...
                        <Brush
                          ...
                          travellerWidth={8}
                          fill="rgba(10,14,28,0.80)"
                        />
                      </ComposedChart>
                    </ResponsiveContainer>
                    <ChartExplainer
                      tone="amber"
                      text="Trend view isolates whether score quality or audit volume is moving, so you can distinguish quality gains from activity spikes."
                    />
                  </div>
                )}
                ...
                {data.platformTrends?.length > 0 && (
                  ...
                  >
                    ...
                    <ResponsiveContainer width="100%" height={300}>
                      <ComposedChart data={data.platformTrends}>
                        ...
                        <Line
                          type="monotone"
                          dataKey="chatgpt"
                          stroke={PLATFORM_PALETTE.chatgpt?.fill ?? P.cyan}
                          strokeWidth={2.5}
                          dot={false}
                          name="ChatGPT"
                          connectNulls
                        ...
                        <Line
                          type="monotone"
                          dataKey="perplexity"
                          stroke={PLATFORM_PALETTE.perplexity?.fill ?? P.indigo}
                          strokeWidth={2.5}
                          dot={false}
                          name="Perplexity"
                          connectNulls
                        ...
                        <Line
                          type="monotone"
                          dataKey="google_ai"
                          stroke={PLATFORM_PALETTE.google_ai?.fill ?? P.amber}
                          strokeWidth={2.5}
                          dot={false}
                          name="Google AI"
                          connectNulls
                        ...
                        <Line
                          type="monotone"
                          dataKey="claude"
                          stroke={PLATFORM_PALETTE.claude?.fill ?? P.rose}
                          strokeWidth={2.5}
                          dot={false}
                          name="Claude"
                          connectNulls
                    ...
                    <ChartExplainer
                      tone="cyan"
                      text="Platform lines reveal where model-specific visibility lags, so optimization can be targeted per AI surface."
                    />
                  </div>
                )}
                ...
                {data.categoryAverages.length > 0 && (
                  ...
                  >
                    ...
                    <ResponsiveContainer
                      width="100%"
                      height={Math.max(260, data.categoryAverages.length * 38)}
                    >
                      <ComposedChart data={data.categoryAverages} layout="vertical">
                        <defs>
                          <linearGradient id="catBar" x1="0" y1="0" x2="1" y2="0">
                            ...
                            <stop offset="50%" stopColor={P.amber} stopOpacity={0.8} />
                    ...
                    <ChartExplainer
                      tone="amber"
                      text="Category bars show which scoring dimensions are dragging overall visibility and need focused remediation."
                    />
                  </div>
                )}
                ...
                {Object.keys(data.platformAverages).length > 0 && (
                  ...
                  >
                    ...
                    <ResponsiveContainer width="100%" height={260}>
                      <ComposedChart
                        data={Object.entries(data.platformAverages).map(([platform, score]) => ({
                          platform,
                          score,
                        }))}
                      >
                    ...
                    <ChartExplainer
                      tone="indigo"
                      text="Platform averages benchmark your baseline authority per engine to guide channel-specific improvements."
                    />
                  </div>
                )}
              </div>
            ...
            {activeTab === 'domains' && (
              <div className="space-y-4">
                <SectionTitle
                  ...
                  tone="orange"
                  sub="Per-domain audit history, scores & improvement deltas"
                />
                {data.improvementDeltas?.length > 0 && (
                  <div
                    className="rounded-2xl p-5"
                ...
                {visibleTopDomains.length > 0 && (
                  ...
                  >
                    <div className="flex items-center justify-between gap-3 mb-4 flex-wrap">
                      <h2 className="text-base brand-title-muted flex items-center gap-2">
                        ...
                        <span className="text-xs text-white/40 font-normal">
                          ({visibleTopDomains.length} of {data.topDomainsTotal})
                        </span>
                      </h2>
                      <button
                    ...
                    <ResponsiveContainer
                      width="100%"
                      height={Math.max(280, visibleTopDomains.length * 34)}
                    >
                    ...
                    <ChartExplainer
                      tone="rose"
                      text="Bars are raw audit counts per domain (not percent); line is latest score percentage for each URL."
                    />
                  </div>
                )}
                {data.topDomains.length > 0 && visibleTopDomains.length === 0 && (
                  <div
                    className="rounded-2xl p-5 text-sm text-white/50"
                    style={{
                      background: 'rgba(10,14,28,0.70)',
                      border: '1px solid rgba(255,255,255,0.07)',
                    }}
            ...
            {activeTab === 'seo' && (
              <div className="space-y-6">
                <SectionTitle
                  title="SEO Diagnostic Checks"
                  tone="emerald"
                  sub="Pass/warn/fail breakdown, compliance posture & rule-level analytics"
                />
                {Object.keys(data.seoDiagnosticsSummary || {}).length > 0 ? (
                  <>
                          ...
                          {topFailedFindings.map((f) => (
                            ...
                            >
                              <div className="flex items-center justify-between gap-2 mb-1">
                                ...
                                <span
                                  className="text-xs px-2 py-0.5 rounded-full text-rose-300"
                                  style={{
                                    background: 'rgba(251,113,133,0.18)',
                                    border: '1px solid rgba(251,113,133,0.25)',
                                  }}
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3 text-sm">
                          ...
                          >
                            ...
                            <p className="text-white/85 font-medium mt-1">
                              {complianceStatus.organization?.name || '-'}
                            </p>
                            <p className="text-xs text-white/55 mt-1">
                              {complianceStatus.organization?.registration_status ||
                                'Status unavailable'}
                            </p>
                          </div>
                          <div
                          ...
                          >
                            ...
                            <p className="text-white/85 font-medium mt-1 capitalize">
                              {complianceStatus.compliance?.soc2_type1?.status || 'unknown'}
                            </p>
                            <p className="text-xs text-white/55 mt-1">
                              Last audit:{' '}
                              {complianceStatus.compliance?.soc2_type1?.last_audit || '-'}
                            </p>
                          </div>
                          <div
                            className="rounded-xl p-3"
                          ...
                          >
                            ...
                            <p className="text-white/85 font-medium mt-1">
                              {complianceStatus.compliance?.vanta?.enabled ? 'Enabled' : 'Disabled'}
                            </p>
                            <p className="text-xs text-white/55 mt-1 capitalize">
                              {complianceStatus.compliance?.vanta?.monitoring_status || 'unknown'}
                            </p>
                          </div>
                          <div
                            className="rounded-xl p-3"
                          ...
                          >
                            ...
                            <p className="text-white/85 font-medium mt-1">
                              {complianceStatus.compliance?.drata?.enabled ? 'Enabled' : 'Disabled'}
                            </p>
                            <p className="text-xs text-white/55 mt-1">
                              Audit ready:{' '}
                              {complianceStatus.compliance?.drata?.audit_ready ? 'Yes' : 'No'}
                            </p>
                          </div>
                        </div>
                      ) : (
                        <p className="text-sm text-white/60">
                          No live compliance payload available from the backend endpoint.
                        </p>
                      )}
                      <ChartExplainer
                        tone="cyan"
                        text="Compliance panel reflects backend-reported security posture and attestation state for executive review."
                      />
                    </div>
                    ...
                    >
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                        {diagnosticRanking.map(
                          ({ check, label, pass, warn, fail, total, failRate }) => (
                            <div
                              key={check}
                              className="rounded-xl p-3"
                              style={{
                            ...
                            >
                              <div className="flex items-center justify-between mb-2">
                                ...
                                {failRate > 50 && (
                                  <span className="text-xs text-rose-400"> high fail</span>
                                )}
                              </div>
                              <div
                              ...
                              <div className="flex items-center gap-2 text-xs">
                                ...
                                <span
                                  className="px-1.5 py-0.5 rounded text-amber-300"
                                ...
                                <span
                                  className="px-1.5 py-0.5 rounded text-rose-300"
                    ...
                    {/* ── Rule Pass Rates Panel ────────────────────────── */}
                    {data.deterministicPipeline?.rulePassRates &&
                      data.deterministicPipeline.rulePassRates.length > 0 && (
            ...
            {activeTab === 'insights' && (
              <div className="space-y-6">
                <SectionTitle
                  title="Deep Insights"
                  tone="rose"
                  sub="Recommendations, schema adoption, content signals & score volatility"
                />
                  ...
                  <div className="grid grid-cols-2 gap-4 mb-4">
                    <div className="rounded-xl border border-slate-700 bg-slate-900 p-3 text-center">
                      <p className="text-2xl font-bold text-white">
                        {data.recommendationInsights.total}
                      </p>
                      <p className="text-[11px] text-white/50">Total Recommendations</p>
                    </div>
                    <div className="rounded-xl border border-slate-700 bg-slate-900 p-3 text-center">
                      <p className="text-2xl font-bold text-cyan-400">
                        {data.recommendationInsights.avgPerAudit}
                      </p>
                ...
                <div className="rounded-2xl border border-slate-700 bg-slate-900 p-5">
                  <h3 className="flex items-center gap-2 text-sm font-semibold text-white mb-4">
                    <BarChart3 className="w-4 h-4 text-emerald-400" /> Content &amp; Technical
                    Signals
                  </h3>
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                    <div className="rounded-xl border border-slate-700 bg-slate-900 p-3 text-center">
                      <p className="text-lg font-bold text-white">
                        {data.contentAndTech.avgWordCount.toLocaleString()}
                      </p>
                      <p className="text-[10px] text-white/50">Avg Words</p>
                    </div>
                    <div className="rounded-xl border border-slate-700 bg-slate-900 p-3 text-center">
                      <p className="text-lg font-bold text-emerald-400">
                        {data.contentAndTech.httpsPct}%
                      </p>
                      <p className="text-[10px] text-white/50">HTTPS</p>
                    </div>
                    <div className="rounded-xl border border-slate-700 bg-slate-900 p-3 text-center">
                      <p className="text-lg font-bold text-cyan-400">
                        {data.contentAndTech.avgResponseTimeMs}ms
                      </p>
                      <p className="text-[10px] text-white/50">Avg Response</p>
                    </div>
                    <div className="rounded-xl border border-slate-700 bg-slate-900 p-3 text-center">
                      <p className="text-lg font-bold text-amber-400">
                        {data.contentAndTech.slowestResponseMs}ms
                      </p>
                  ...
                  <div className="mt-3 grid grid-cols-2 gap-3">
                    <div className="rounded-xl border border-slate-700 bg-slate-900 p-3 text-center">
                      ...
                      <p className="text-xs text-white/40">
                        {data.contentAndTech.minWordCount.toLocaleString()} -{' '}
                        {data.contentAndTech.maxWordCount.toLocaleString()}
                      </p>
                    </div>
                    <div className="rounded-xl border border-slate-700 bg-slate-900 p-3 text-center">
                      <p className="text-sm text-white/60">Fastest Response</p>
                      <p className="text-xs text-white/40">
                        {data.contentAndTech.fastestResponseMs}ms
                      </p>
                    </div>
                  </div>
                </div>
                ...
                {data.scoreVolatility.length > 0 && (
                  <div className="rounded-2xl border border-slate-700 bg-slate-900 p-5">
                    ...
                    <p className="text-xs text-white/40 mb-3">
                      Domains with 2+ audits, ranked by score standard deviation.
                    </p>
                    <div className="space-y-2">
