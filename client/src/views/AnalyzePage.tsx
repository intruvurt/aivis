...
function getStageMicrocopy(step: string): string[] {
  ...
  if (normalized.includes('extract')) {
    return [
      'Extracting visible signals',
      'Reading headings and metadata',
  ...
  if (normalized.includes('technical')) {
    return [
      'Checking canonicals and HTTPS',
      'Reviewing crawl and answer-engine access',
      'Scoring technical trust',
    ];
  }
  if (normalized.includes('security')) {
    return [
      'Scanning threat indicators',
      'Reviewing exposure risk',
      'Adding security findings to the audit',
    ];
  }
...
const AnalyzePage: React.FC = () => {
  ...
  usePageMeta({
    title: 'Analyze',
    description:
      'Run an AI visibility audit on any website. Get evidence-backed scoring and actionable recommendations.',
    path: '/analyze',
    structuredData: [
      buildWebPageSchema({
        path: '/analyze',
        ...
        description:
          'Run an evidence-backed AI visibility audit on any website. Score citation readiness, extraction clarity, entity signals, and trust markers across ChatGPT, Perplexity, Claude, and Google AI Overviews.',
      }),
  ...
  function openProgressStream(requestId: string) {
    ...
    es.onerror = () => {
      ...
      setProgress((prev) => (prev.step === 'complete' ? prev : { ...prev, step: 'processing' }));
    };
  }
  ...
  async function handleAnalyze(overrideUrl?: string) {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (errorData?.code === 'INVALID_URL')
          throw new Error(errorData.error || 'Invalid URL format');
        if (errorData?.code === 'SCRAPE_FAILED') {
          throw new Error(
            errorData.error || 'Unable to access website. Check the URL and try again.'
          );
        }
        ...
        if (response.status === 429) {
          throw new Error(
            errorData?.error || 'Rate limit exceeded. Please wait a moment and try again.'
          );
        }
        ...
        throw new Error(
          errorData?.error || `Analysis failed with status ${response.status}. Please try again.`
        );
      }
      ...
      // Result stays in-page — AnalyzeCognitionView transitions to complete phase
    } catch (err: any) {
      closeProgressStream();
  ...
  const isSameTargetAsBaseline =
    !!activeResultUrl && !!activeBaselineUrl && activeResultUrl === activeBaselineUrl;
  ...
  const baselineDelta = useMemo(() => {
    ...
    const baselineCategories = new Map(
      (demoBaseline.category_grades ?? []).map((c) => [c.label, c.score])
    );
...
/*
 * ── Legacy render — kept for reference, no longer active ────────────────────
 * The following is the original dashboard-style render that was replaced by
 * AnalyzeCognitionView. It is preserved in this comment block so the logic
 * (FAQ, quick examples, pipeline status) remains accessible if needed.
 */
function _LegacyRender_UNUSED() {
  return (
    <div className="space-y-6 text-white">
      ...
      <div>
        ...
        <p className="text-sm text-slate-400 mt-1">
          Run an evidence-backed audit to uncover visibility blockers, trust gaps, and extraction
          weaknesses.
        </p>
      </div>
      ...
      <div className="grid gap-6 xl:grid-cols-[1.15fr_0.85fr]">
        <section className="rounded-xl border border-white/10 bg-charcoal/80 p-6 shadow-2xl sm:p-8 lg:p-10">
          ...
          <p className="mt-4 max-w-3xl text-base leading-8 text-white/65">
            Run an evidence-backed audit on any website to uncover visibility blockers, trust gaps,
            extraction weaknesses, and the fixes most likely to improve AI comprehension and
            citation readiness.
          </p>
        ...
        <aside className="rounded-xl border border-white/10 bg-charcoal/80 p-6 shadow-2xl sm:p-8">
          ...
          <div className="mt-5 space-y-3">
            {[
              [
                ...
                'We normalize the URL and attempt to reach the target safely.',
              ],
              [
                '2',
                'Extract visible signals',
                'Headings, structure, trust elements, and content patterns are inspected.',
              ],
              [
                '3',
                'Score AI visibility',
                'The page is evaluated for clarity, extraction readiness, and evidence depth.',
              ],
              [
                '4',
                'Generate actions',
                'You get concrete recommendations instead of generic SEO filler.',
              ],
          ...
          <div className="mt-5 rounded-2xl border border-white/10 bg-charcoal-deep p-4">
            ...
            <div className="mt-2 text-sm leading-7 text-white/70">
              Public pages with clear content and reachable markup tend to produce the strongest
              audits. Very slow sites, blocked pages, or heavy client-side rendering may reduce
              completeness.
            </div>
          </div>
        </aside>
        ...
        <div className="grid gap-6 lg:grid-cols-[1.15fr_0.85fr] lg:items-start">
          <div>
            ...
            <p className="mt-2 text-sm leading-7 text-white/60">
              Enter a domain or full URL. We’ll normalize it, validate it, and run the audit flow
              from there.
            </p>
            ...
            <div className="mt-5 space-y-4">
              <div>
                ...
                {validationError && (
                  <div
                    role="alert"
                    className="mt-3 flex items-start gap-2 rounded-xl border border-amber-400/20 bg-amber-400/10 px-3 py-2 text-sm text-amber-200"
                  >
                ...
                {error && !scanLimitReached && (
                  <div
                    role="alert"
                    className="mt-3 flex items-start gap-2 rounded-xl border border-red-400/20 bg-red-400/10 px-3 py-2 text-sm text-red-200"
                  >
                    <AlertCircle className="mt-0.5 h-4 w-4 shrink-0" />
                    <span>{error}</span>
                  </div>
            ...
            <div className="rounded-2xl border border-white/10 bg-charcoal-deep p-5">
              ...
              <div className="mt-4 rounded-xl border border-white/10 bg-charcoal p-4 text-xs leading-6 text-white/60">
                {loading ? (
                  ...
                  <>
                    Last audited:{' '}
                    <span className="text-white/80 font-medium">{lastAnalyzedUrl}</span>. Enter a
                    URL above to start a new audit.
                  </>
                ) : (
          ...
          <div className="rounded-2xl border border-white/10 bg-charcoal/80 p-5 shadow-2xl sm:p-6 space-y-5">
            ...
            <div>
              <h2 className="text-2xl font-semibold text-white">
                See your visibility. Then fix what AI cannot trust.
              </h2>
              <p className="mt-2 text-sm leading-7 text-white/60">
              ...
              <div className="rounded-2xl border border-[#22ff6e]/20 bg-[#22ff6e]/10 p-4">
                ...
                {baselineDelta && (
                  <div className="mt-2 space-y-2">
                    ...
                    <div
                      className={`text-xs font-semibold ${baselineDelta.scoreDelta >= 0 ? 'text-emerald-200' : 'text-rose-200'}`}
                    >
                      Score delta: {baselineDelta.scoreDelta >= 0 ? '+' : ''}
                      {baselineDelta.scoreDelta.toFixed(1)} points
                    </div>
