...
const KEYWORDS_FAQ = [
  ...
  },
  {
    ...
    answer:
      'The opportunity score (0\u2013100) is a composite signal that weighs search volume, competitive difficulty, AI extractability, and intent clarity. Higher scores indicate keywords where a targeted content improvement would produce the greatest visibility gain relative to the effort required. It prioritizes gaps where you have some existing content but poor answer structure, rather than areas where you have no content at all, since incremental improvements to existing pages typically yield faster citation gains.',
  },
  {
    question: 'Where do the keyword suggestions come from?',
    answer:
      "Keywords are derived from three sources: your own audit results (entities and topics that your content signals mention), AI-generated question variants that match your page's subject matter, and comparison data from competitor audits that identify the terms where competitors are being cited but you are not. Where Search Console or keyword enrichment data is available, real autocomplete and People Also Ask terms are layered in to validate volume and confirm actual user query patterns.",
  },
];
const INTENT_CONFIG = {
  informational: {
...
const VOLUME_CONFIG = {
  ...
  medium: { label: '1K–10K/mo', color: 'text-emerald-300' },
...
function OpportunityBar({ score }: { score: number }) {
  ...
  return (
    <div className="flex items-center gap-2 w-full min-w-[100px]">
      ...
      <span
        className={`text-xs w-7 text-right shrink-0 ${score >= 70 ? 'text-emerald-300' : score >= 40 ? 'text-amber-300' : 'text-rose-300'}`}
      >
        {score}
      </span>
    </div>
...
export default function KeywordsPage() {
  ...
  usePageMeta({
    ...
    structuredData: [
      buildWebPageSchema({
        ...
        description:
          'Keyword intent analysis, AI extractability scoring, and opportunity detection from your AI visibility audit results. Identify which query types drive AI citations and which need answer-block restructuring.',
      }),
  ...
  const [serverHosts, setServerHosts] = useState<
    Record<string, { keywords: any[]; auditCount: number; latestUrl: string }>
  >({});
  ...
  useEffect(() => {
    ...
    apiFetch('/api/keywords/from-audits')
      .then((r) => r.json())
      .then((d) => {
        if (d.success && d.hosts) setServerHosts(d.hosts);
      })
      .catch(() => {});
  }, [isAuthenticated]);
  ...
  const hostMap = useMemo(() => {
    const map = new Map<
      string,
      {
        keywords: Map<string, KeywordIntelligence>;
        latestUrl: string;
        auditCount: number;
    ...
    for (const entry of history) {
      ...
      let host: string;
      try {
        host = new URL(entry.result.url).hostname;
      } catch {
        continue;
      }
      if (!map.has(host))
      ...
      if (ts > record.latestTimestamp) {
        record.latestTimestamp = ts;
        record.latestUrl = entry.result.url;
      }
  ...
  useEffect(() => {
    ...
    if (selectedHost && !availableHosts.includes(selectedHost) && availableHosts.length > 0)
  ...
  const newKeywordCount = useMemo(() => {
    ...
    const hostAudits = history
      .filter((e) => {
        try {
          return new URL(e.result?.url || '').hostname === selectedHost;
        } catch {
          return false;
        }
      })
    ...
    const latestKws = new Set(
      (hostAudits[0].result?.keyword_intelligence || []).map((k) => k.keyword)
    );
    const prevKws = new Set(
      (hostAudits[1].result?.keyword_intelligence || []).map((k) => k.keyword)
    );
    let count = 0;
    latestKws.forEach((kw) => {
      if (!prevKws.has(kw)) count++;
    });
    return count;
  ...
  const verifiedCount = hasEnrichment
    ? [...enrichmentMap.values()].filter((e) => e.search_verified).length
    : 0;
  ...
  const filtered: KeywordIntelligence[] = useMemo(() => {
    ...
    list.sort((a, b) => {
      ...
      if (sortKey === 'opportunity') diff = (a.opportunity ?? 0) - (b.opportunity ?? 0);
        ...
        diff = (VOLUME_ORDER[a.volume_tier] ?? 0) - (VOLUME_ORDER[b.volume_tier] ?? 0);
      else if (sortKey === 'competition')
        diff = (COMP_ORDER[a.competition] ?? 0) - (COMP_ORDER[b.competition] ?? 0);
  ...
  const exportCSV = () => {
    ...
    const rows = filtered.map(
      (k) =>
        `"${k.keyword}",${k.intent},${k.volume_tier},${k.competition},${k.opportunity},${k.trend}`
    );
  ...
  const counts = useMemo(() => {
    ...
    keywords.forEach((k) => {
      (c as any)[k.intent] = ((c as any)[k.intent] || 0) + 1;
    });
    return c;
  }, [keywords]);

  const avgOpportunity = keywords.length
    ? Math.round(keywords.reduce((s, k) => s + k.opportunity, 0) / keywords.length)
    : 0;
  const risingCount = keywords.filter((k) => k.trend === 'rising').length;
  const highVolumeCount = keywords.filter(
    (k) => k.volume_tier === 'high' || k.volume_tier === 'very_high'
  ).length;
  ...
  const executionQueue = useMemo(() => {
    const rankScore = (keyword: KeywordIntelligence) => {
      const intentBoost =
        keyword.intent === 'transactional' || keyword.intent === 'commercial' ? 8 : 4;
      const trendBoost = keyword.trend === 'rising' ? 14 : keyword.trend === 'stable' ? 5 : 0;
      const competitionBoost =
        keyword.competition === 'low' ? 12 : keyword.competition === 'medium' ? 6 : 0;
    ...
    return [...keywords]
      .map((keyword) => ({
        keyword,
        rankScore: rankScore(keyword),
        plan: keywordPlans[keyword.keyword],
      }))
        ...
        <div className="flex items-center gap-2">
          {!hasData && history.length > 0 && (
            <span className="text-xs text-slate-500 hidden sm:inline">
              Re-run analysis for keyword data
            </span>
          )}
          <button
            onClick={exportCSV}
        ...
        {!history.length && (
          <div className="text-center py-20">
            ...
            <p className="text-slate-400 text-sm mt-2 mb-6">
              Run an analysis on your website to discover your topical keywords.
            </p>
            <button
              onClick={() => navigate('/app/analyze')}
              className="px-6 py-2.5 bg-emerald-500/10 border border-emerald-500/20 hover:bg-emerald-500/20 text-emerald-400 rounded-lg text-sm font-medium transition-colors"
              type="button"
            >
              Analyze a Website
            </button>
              ...
              <div className="flex items-center gap-3 flex-wrap">
                <div className="relative">
                  ...
                  >
                    ...
                    <ChevronDown
                      className={`w-4 h-4 text-white/50 transition-transform ${showHostDropdown ? 'rotate-180' : ''}`}
                    />
                  </button>
                  {showHostDropdown && (
                        ...
                        return (
                          <button
                            ...
                            className={`w-full text-left px-4 py-2.5 text-sm hover:bg-white/8 transition-colors flex items-center justify-between gap-3 ${
                              host === selectedHost
                                ? 'text-emerald-300 bg-emerald-500/8'
                                : 'text-white/80'
                            }`}
                            type="button"
                          >
                            ...
                            <span className="text-xs text-white/40 shrink-0">
                              {hd?.keywords.size ?? 0} kw · {hd?.auditCount ?? 0} audits
                            </span>
                          </button>
                        );
                      })}
                ...
                {hostData && (
                  <span className="text-xs text-white/40">
                    {hostData.auditCount} audit{hostData.auditCount > 1 ? 's' : ''} analyzed
                  </span>
                )}
              </div>
            )}
            ...
            {!hasData && (
              <div className="text-center py-16">
                ...
                <p className="text-white/60 text-sm mt-2 mb-6 max-w-md mx-auto">
                  Your audits didn't include keyword intelligence data. Run a new analysis to
                  discover AI-identified keywords with intent, volume, and opportunity scoring.
                </p>
                <button
                  onClick={() => navigate('/')}
                  className="px-6 py-2.5 bg-charcoal hover:bg-charcoal text-white rounded-full text-sm font-medium transition-colors"
                  type="button"
                >
                  Run New Analysis
                </button>
              ...
              <>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-5 gap-4">
                  {[
                    ...
                    },
                    {
                      label: 'Rising Trends',
                      value: risingCount,
                      color: risingCount > 0 ? 'text-emerald-300' : 'text-white/50',
                    },
                    {
                      label: hasEnrichment ? 'Search Verified' : 'High Volume',
                      value: hasEnrichment ? verifiedCount : highVolumeCount,
                      color: hasEnrichment
                        ? verifiedCount > 0
                          ? 'text-emerald-300'
                          : 'text-white/50'
                    ...
                    },
                    {
                      label: 'Starred',
                      value: starredCount,
                      color: starredCount > 0 ? 'text-yellow-300' : 'text-white/50',
                    },
                ...
                <div className="rounded-2xl border border-white/10 bg-charcoal-deep/60 p-4">
                  <div className="flex items-center justify-between gap-3 flex-wrap">
                    ...
                    <button
                      ...
                      className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-emerald-500/10 border border-emerald-500/25 hover:bg-emerald-500/20 text-emerald-300 text-sm font-medium transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
                      type="button"
                    >
                      {isEnriching ? (
                        <>
                  ...
                  {enrichError && <p className="text-xs text-rose-400 mt-2">{enrichError}</p>}
                </div>
                ...
                <div className="rounded-2xl border border-white/10 bg-charcoal-deep p-5">
                  <div className="flex items-center justify-between mb-4 gap-3">
                    <div>
                      ...
                      <p className="text-xs text-white/60">
                        Prioritized by opportunity, trend, intent, and competition
                      </p>
                    </div>
                  ...
                  <div className="space-y-3">
                    {executionQueue.map(({ keyword, rankScore, plan }) => (
                      <div
                        key={`queue-${keyword.keyword}`}
                        className="rounded-xl border border-white/10 bg-charcoal p-3"
                      >
                        <div className="flex flex-col lg:flex-row lg:items-center gap-3">
                          <div className="flex-1 min-w-0">
                            ...
                            <p className="text-xs text-white/60 mt-1">
                              ...
                              {INTENT_CONFIG[keyword.intent].label} • Opportunity{' '}
                              {keyword.opportunity}
                            </p>
                          </div>
                          ...
                          <input
                            ...
                            onChange={(e) =>
                              updateKeywordPlan(keyword.keyword, { targetUrl: e.target.value })
                            }
                          ...
                          <select
                            ...
                            onChange={(e) =>
                              updateKeywordPlan(keyword.keyword, {
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-3">
                  ...
                  >
                    ...
                    <p className="text-xs text-white/60 mt-1">
                      Prioritize saved targets for content updates
                    </p>
                  </button>
                  <button
                  ...
                  <div className="flex bg-charcoal rounded-lg p-1 gap-0.5 flex-wrap">
                    {(
                      ...
                      >
                        {intent === 'all'
                          ? `All (${counts.all})`
                          : `${intent.slice(0, 4)}. (${(counts as any)[intent]})`}
                      </button>
                    ))}
                  </div>
                  ...
                  <table className="w-full min-w-[700px]">
                    <thead>
                      <tr className="border-b border-white/10 bg-charcoal">
                        <th className="text-left px-5 py-3">
                          <SortHeader label="Keyword" col="keyword" />
                        </th>
                        ...
                        <th className="text-left px-4 py-3">
                          <SortHeader label="Competition" col="competition" />
                        </th>
                        <th className="text-left px-4 py-3 w-40">
                          <SortHeader label="Opportunity" col="opportunity" />
                        </th>
                        <th className="text-left px-4 py-3">
                          <span className="text-xs font-medium uppercase tracking-wide text-white/60">
                            Trend
                    ...
                    <tbody className="divide-y divide-white/20">
                          ...
                          <React.Fragment key={kw.keyword}>
                            <tr className="hover:bg-charcoal transition-colors">
                              <td className="px-5 py-3.5">
                                <div className="flex items-center gap-2">
                                  {hasEnrichment && enrichData?.real_suggestions?.length ? (
                                    <button
                                      onClick={() =>
                                        setExpandedEnrich(isExpanded ? null : kw.keyword)
                                      }
                              ...
                              <td className="px-4 py-3.5">
                                <span
                                  className={`text-xs px-2 py-0.5 rounded-full border whitespace-nowrap ${ic.color}`}
                                >
                                  {ic.label}
                                </span>
                              </td>
                              <td
                                className={`px-4 py-3.5 text-xs font-medium whitespace-nowrap ${vc.color}`}
                              >
                                {vc.label}
                              </td>
                              <td className="px-4 py-3.5">
                                <div className="flex items-center gap-1.5">
                                  <span
                                    className={`w-2 h-2 rounded-full flex-shrink-0 ${cc.dot}`}
                                  />
                                  ...
                                  {enrichData ? (
                                    enrichData.search_verified ? (
                                      <span
                                        ...
                                        title="Matches real search engine autocomplete"
                                      >
                                      ...
                                      <span
                                        ...
                                        title="Not found in search autocomplete"
                                      >
                            ...
                            {isExpanded &&
                              enrichData &&
                              (enrichData.real_suggestions.length > 0 ||
                                ...
                                <tr className="bg-charcoal-deep/40">
                                  <td colSpan={hasEnrichment ? 8 : 7} className="px-5 py-3">
                                    <div className="space-y-2 pl-5">
                                      {enrichData.related_queries.length > 0 && (
                                        <div>
                                          <p className="text-[10px] uppercase tracking-wide text-emerald-400/70 mb-1">
                                            Real multi-word searches people use
                                          </p>
                                          <div className="flex flex-wrap gap-1.5">
                                            {enrichData.related_queries.map((q) => (
                                              <span
                                                key={q}
                                                className="px-2 py-0.5 rounded-full border border-emerald-500/20 bg-emerald-500/8 text-xs text-emerald-200"
                                              >
                                                {q}
                                              </span>
                                            ))}
                                      ...
                                      {enrichData.real_suggestions.filter(
                                        (s) => !enrichData.related_queries.includes(s)
                                      ).length > 0 && (
                                        <div>
                                          <p className="text-[10px] uppercase tracking-wide text-white/40 mb-1">
                                            Other autocomplete suggestions
                                          </p>
                                          <div className="flex flex-wrap gap-1.5">
                                            {enrichData.real_suggestions
                                              .filter(
                                                (s) => !enrichData.related_queries.includes(s)
                                              )
                                              .slice(0, 8)
                                              .map((s) => (
                                                <span
                                                  key={s}
                                                  className="px-2 py-0.5 rounded-full border border-white/10 bg-white/4 text-xs text-white/55"
                                                >
                                                  {s}
                                                </span>
                                              ))}
                      ...
                      {filtered.length === 0 && (
                        <tr>
                          <td
                            colSpan={hasEnrichment ? 8 : 7}
                            className="text-center py-12 text-white/60 text-sm"
                          >
                ...
                <div className="flex flex-wrap gap-x-6 gap-y-2 text-xs text-white/60 px-1">
                  ...
                  {hasEnrichment && (
                    <>
                      ...
                      <span className="flex items-center gap-1.5">
                        <XCircle className="w-3 h-3 text-white/35" />
                        Not in autocomplete
                      </span>
                    </>
                  )}
