...
const TOOL_EXAMPLES: Record<Tool, { input: string; secondary?: string }> = {
  ...
  rewrite: {
    input:
      'We help businesses improve their online presence. Our team of experts works to optimize your website for better performance. Contact us to learn more about our services and how we can help you grow.',
...
function getVerdictFromProbability(probability?: number | null): {
  label: string;
...
function summarizeResult(
  ...
  let verdict: { label: string; tone: 'good' | 'warn' | 'neutral' } = {
    label: 'Mixed',
    tone: 'neutral',
  };
  ...
  if (tool === 'simulate') {
    const current =
      typeof result.currentProbability === 'number' ? result.currentProbability : null;
    const projected =
      typeof result.projectedProbability === 'number' ? result.projectedProbability : null;
    const uplift =
      typeof current === 'number' && typeof projected === 'number' ? projected - current : null;
    ...
    cards.push(
      ...
      {
        label: 'Current',
        value: current !== null ? `${clampPercent(current)}%` : '-',
        icon: Gauge,
      },
      {
        label: 'Projected',
        value: projected !== null ? `${clampPercent(projected)}%` : '-',
        tone: projected && projected >= 0.7 ? 'good' : 'neutral',
        icon: TrendingUp,
      },
      {
        label: 'Uplift',
        ...
        tone:
          uplift !== null && uplift > 0.15
            ? 'good'
            : uplift !== null && uplift < 0.05
              ? 'warn'
              : 'neutral',
        icon: Sparkles,
      }
    ...
    insights.push(
      {
        label: typeof uplift === 'number' && uplift > 0.15 ? 'High upside' : 'Moderate upside',
        tone: uplift && uplift > 0.15 ? 'good' : 'neutral',
      },
      {
        label: result.modelBreakdown ? 'Model-level detail available' : 'Model breakdown limited',
        tone: result.modelBreakdown ? 'good' : 'warn',
      },
      {
        label: result.implementationPlan
          ? 'Implementation path present'
          : 'Needs implementation plan',
        tone: result.implementationPlan ? 'good' : 'warn',
      }
    );
  ...
  if (tool === 'ghost') {
    const probability =
      typeof result.estimatedInclusionProbability === 'number'
        ? result.estimatedInclusionProbability
        : null;
    ...
    cards.push(
      ...
      {
        label: 'Inclusion',
        value: probability !== null ? `${clampPercent(probability)}%` : '-',
        icon: Gauge,
      },
      {
        label: 'Schema',
        value: result.schemaRecommendations
          ? `${collectStrings(result.schemaRecommendations, 8).length} signals`
          : '-',
        icon: Link2,
      },
      {
        label: 'Trust',
        value: result.trustSignals
          ? `${collectStrings(result.trustSignals, 8).length} layers`
          : '-',
        icon: Brain,
      }
    );
    ...
    insights.push(
      {
        label: result.idealBlueprint ? 'Blueprint ready' : 'Blueprint thin',
        tone: result.idealBlueprint ? 'good' : 'warn',
      },
      {
        label: result.entityGraph ? 'Entity graph mapped' : 'Entity clarity weak',
        tone: result.entityGraph ? 'good' : 'warn',
      },
      {
        label: result.internalLinkPattern
          ? 'Internal reinforcement present'
          : 'Needs internal link logic',
        tone: result.internalLinkPattern ? 'good' : 'warn',
      }
    );
  ...
  if (tool === 'decompile') {
    ...
    cards.push(
      ...
      {
        label: 'Blueprint',
        value: result.reconstructionBlueprint ? 'Ready' : 'Partial',
        tone: result.reconstructionBlueprint ? 'good' : 'warn',
        icon: Wand2,
      }
    );

    verdict = {
      label: result.reconstructionBlueprint ? 'Actionable' : 'Mixed',
      tone: result.reconstructionBlueprint ? 'good' : 'neutral',
    };

    insights.push(
      {
        label: result.answerShape ? 'Answer shape detected' : 'Weak answer shape',
        tone: result.answerShape ? 'good' : 'warn',
      },
      {
        label: structureCount >= 3 ? 'Reusable structure found' : 'Thin structure signals',
        tone: structureCount >= 3 ? 'good' : 'warn',
      },
      {
        label: citationCount >= 2 ? 'Citation vectors visible' : 'Citation patterns unclear',
        tone: citationCount >= 2 ? 'good' : 'warn',
      }
    );
  ...
  if (tool === 'model-diff') {
    ...
    cards.push(
      ...
      { label: 'Recommendations', value: String(recommendations), icon: Wand2 },
      {
        ...
        value: recommendations >= 3 ? 'Strong' : 'Partial',
        tone: recommendations >= 3 ? 'good' : 'warn',
        icon: ShieldCheck,
      }
    );

    verdict = {
      label: recommendations >= 3 ? 'Strategic' : 'Mixed',
      tone: (recommendations >= 3 ? 'good' : 'neutral') as 'good' | 'warn' | 'neutral',
    };

    insights.push(
      {
        label: models >= 2 ? 'Cross-model view present' : 'Limited model spread',
        tone: models >= 2 ? 'good' : 'warn',
      },
      {
        label: result.alignmentMatrix ? 'Alignment matrix available' : 'No alignment matrix',
        tone: result.alignmentMatrix ? 'good' : 'warn',
      },
      {
        label: recommendations >= 3 ? 'Usable optimization path' : 'Needs stronger recommendations',
        tone: recommendations >= 3 ? 'good' : 'warn',
      }
    );
...
function buildClientReportMarkdown(
  tool: Tool,
  input: string,
  secondaryInput: string,
  result: any
  ...
  const actions = summary.actions.length
    ? summary.actions
        .map(
          (action, i) =>
            `- [ ] ${i + 1}. ${action.title}${action.impact ? ` (${action.impact})` : ''}`
        )
        .join('\n')
  ...
  return [
    ...
    ...(summary.insights.length
      ? summary.insights.map((pill) => `- ${pill.label}`)
      : ['- No insight pills generated.']),
    '',
    `## Priority Actions`,
    actions,
    '',
...
function buildImplementationBrief(
  ...
  return [
    ...
    ...(summary.insights.length
      ? summary.insights.map((pill) => `- ${pill.label}`)
      : ['- Validate result quality and extract concrete implementation tasks.']),
    '',
    `## First 5 implementation moves`,
    ...(topActions.length
      ? topActions.map(
          (action, i) => `1.${i + 1}. ${action.title}${action.impact ? ` [${action.impact}]` : ''}`
        )
      ...
      <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
        <div>
          ...
          <p className="mt-1 text-xs leading-6 text-white/60">
            Move from analysis into implementation, export, or remediation without leaving the
            workflow cold.
            {!canGenerateFixPack && (
      ...
      <div className="flex items-start justify-between gap-3 px-5 py-4 bg-charcoal-deep">
        ...
        >
          <div>
            ...
            {whyItMatters && (
              <p className="mt-1 text-xs leading-6 text-white/55">Why it matters: {whyItMatters}</p>
            )}
          </div>
          {open ? (
            <ChevronDown className="mt-0.5 h-4 w-4 text-white/60" />
          ) : (
            <ChevronRight className="mt-0.5 h-4 w-4 text-white/60" />
          )}
        </button>
...
function ExecutiveSummary({ tool, input, result }: { tool: Tool; input: string; result: any }) {
  ...
  const interpretation = (() => {
    ...
    if (tool === 'ghost') {
      const prob =
        typeof result?.estimatedInclusionProbability === 'number'
          ? Math.round(result.estimatedInclusionProbability * 100)
          : null;
      return prob !== null
    ...
    if (tool === 'simulate') {
      const current =
        typeof result?.currentProbability === 'number'
          ? Math.round(result.currentProbability * 100)
          : null;
      const projected =
        typeof result?.projectedProbability === 'number'
          ? Math.round(result.projectedProbability * 100)
          : null;
          ...
          <div className="mt-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
            {summary.cards.map((card) => {
              ...
              return (
                <div
                  key={card.label}
                  className="rounded-xl border border-white/10 bg-charcoal-deep p-3.5"
                >
                  <div className="flex items-center gap-2">
        ...
        <div className="rounded-2xl border border-emerald-400/15 bg-emerald-400/5 p-5">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h4 className="text-sm font-semibold text-emerald-100">
              Start Here - Your Top {Math.min(3, summary.actions.length)} Moves
            </h4>
          </div>
          <div className="space-y-3">
            {summary.actions.slice(0, 3).map((action, index) => (
              <div key={`${action.title}-${index}`} className="flex items-start gap-3">
                ...
                {action.impact && (
                  <span
                    className={`rounded-full border px-2 py-0.5 text-[10px] uppercase ${getImpactTone(action.impact)}`}
                  >
                    {action.impact}
                  </span>
                )}
        ...
        <div className="mt-4 space-y-3">
          {blueprint.length ? (
            blueprint.map((item, idx) => (
              <div
                key={`${item}-${idx}`}
                className="rounded-xl border border-white/10 bg-charcoal-deep px-4 py-3 text-sm text-white/75"
              >
                {item}
              </div>
            ))
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <div>
            ...
            <div className="mt-3 space-y-2">
              {schema.length ? (
                schema.map((item, idx) => (
                  <div key={`${item}-${idx}`} className="text-xs leading-6 text-white/70">
                    • {item}
                  </div>
                ))
              ) : (
                <div className="text-xs text-white/50">No schema targets returned.</div>
              )}
            </div>
          </div>
          <div>
            ...
            <div className="mt-3 space-y-2">
              {trust.length ? (
                ...
                <div className="text-xs text-white/50">No trust layers returned.</div>
              )}
            </div>
          </div>
        </div>
        ...
        <div className="mt-4 rounded-xl border border-white/10 bg-charcoal-deep p-4">
          ...
          <div className="mt-3 space-y-2">
            {links.length ? (
              ...
              <div className="text-xs text-white/50">No internal link guidance returned.</div>
            )}
          </div>
        </div>
      </div>
...
function SimulateRichResults({ data }: { data: any }) {
  ...
  const projected =
    typeof data.projectedProbability === 'number' ? data.projectedProbability : null;
  const uplift =
    typeof current === 'number' && typeof projected === 'number' ? projected - current : null;
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
        <div className="rounded-xl border border-white/10 bg-charcoal p-4 text-center">
          ...
          <p className="text-3xl font-bold text-white/80">
            {current !== null ? `${clampPercent(current)}%` : '-'}
          </p>
        </div>
        <div className="rounded-xl border border-emerald-400/20 bg-emerald-400/10 p-4 text-center">
          ...
          <p className="text-3xl font-bold text-emerald-200">
            {projected !== null ? `${clampPercent(projected)}%` : '-'}
          </p>
        </div>
        <div className="rounded-xl border border-white/10 bg-charcoal p-4 text-center">
          <p className="mb-1 text-xs uppercase tracking-wide text-white/50">Uplift</p>
          <p className="text-3xl font-bold text-white/80">
            {uplift !== null ? `${uplift >= 0 ? '+' : ''}${Math.round(uplift * 100)}%` : '-'}
          </p>
        </div>
      </div>
        ...
        <div className="rounded-2xl border border-white/10 bg-charcoal p-4 sm:p-5">
          ...
          <div className="mt-4 space-y-3">
            {changes.length ? (
              ...
              <p className="text-sm text-white/55">No simulated changes returned.</p>
            )}
          </div>
        </div>
        ...
        <div className="rounded-2xl border border-white/10 bg-charcoal p-4 sm:p-5">
          ...
          <div className="mt-4 space-y-3">
            {plan.length ? (
              ...
              <p className="text-sm text-white/55">No implementation path returned.</p>
            )}
          </div>
        </div>
      </div>
...
function DecompileResult({ data }: { data: any }) {
  ...
  const blueprint =
    typeof data.reconstructionBlueprint === 'string' ? data.reconstructionBlueprint.trim() : '';
    ...
    <div className="space-y-5">
      ...
      {blueprint && (
        <div className="rounded-2xl border border-emerald-400/15 bg-gradient-to-br from-emerald-400/5 to-transparent p-5 sm:p-6">
          ...
          <p className="text-xs text-white/50 mb-4">
            This is your build guide. Follow this to replicate the answer structure on your own
            content.
          </p>
      ...
      {shape && (
        <div className="rounded-2xl border border-white/10 bg-charcoal p-5">
          ...
          <p className="text-xs text-white/50 mb-4">
            The structural fingerprint of this answer - how the model organized density, facts, and
            trust signals.
          </p>
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            {typeof shape.entityDensity === 'number' && (
              <div className="rounded-xl border border-white/10 bg-charcoal-deep p-3.5">
                ...
                <p className="mt-1 text-lg font-bold text-white/85">
                  {(shape.entityDensity * 100).toFixed(0)}%
                </p>
            ...
            {typeof shape.factDensity === 'number' && (
              <div className="rounded-xl border border-white/10 bg-charcoal-deep p-3.5">
                ...
                <p className="mt-1 text-lg font-bold text-white/85">
                  {(shape.factDensity * 100).toFixed(0)}%
                </p>
                <p className="mt-1 text-[11px] text-white/45">
                  Percentage that is verifiable claims
                </p>
              </div>
            )}
            ...
            {typeof shape.averageSentenceComplexity === 'number' && (
              <div className="rounded-xl border border-white/10 bg-charcoal-deep p-3.5">
                ...
                <p className="mt-1 text-lg font-bold text-white/85">
                  {shape.averageSentenceComplexity.toFixed(1)}
                </p>
      ...
      {patterns.length > 0 && (
        <div className="rounded-2xl border border-white/10 bg-charcoal p-5">
          ...
          <p className="text-xs text-white/50 mb-4">
            The answer formats the model relied on. Replicate these patterns in your own content.
          </p>
          <div className="grid gap-3 sm:grid-cols-2">
            {patterns.map((p, i) => {
              const name =
                typeof p === 'string' ? p : (p?.pattern ?? p?.name ?? `Pattern ${i + 1}`);
      ...
      {clusters.length > 0 && (
        <div className="rounded-2xl border border-white/10 bg-charcoal p-5">
          ...
          <p className="text-xs text-white/50 mb-4">
            Topic groups the model bundled together. Your content should cover these clusters to be
            citation-worthy.
          </p>
          <div className="space-y-3">
            {clusters.map((c, i) => {
              ...
              const relationships: string[] = Array.isArray(c?.relationships)
                ? c.relationships
                : [];
              const density = typeof c?.density === 'number' ? c.density : null;
              const contribution =
                typeof c?.answerContribution === 'number' ? c.answerContribution : null;
              return (
                <div key={i} className="rounded-xl border border-white/10 bg-charcoal-deep p-4">
                  <div className="flex items-center justify-between gap-2">
                  ...
                  {entities.length > 0 && (
                    <div className="mt-2 flex flex-wrap gap-1.5">
                      {entities.map((e, j) => (
                        <span
                          key={j}
                          className="rounded-md border border-white/10 bg-charcoal px-2 py-0.5 text-[11px] text-white/70"
                        >
                          {e}
                        </span>
                      ))}
      ...
      {citations.length > 0 && (
        <div className="rounded-2xl border border-white/10 bg-charcoal p-5">
          ...
          <p className="text-xs text-white/50 mb-4">
            What makes content quotable by AI. The "missing" items are gaps you should fill on your
            site.
          </p>
          <div className="space-y-3">
            {citations.map((cv, i) => {
              const contentType =
                typeof cv === 'string' ? cv : (cv?.contentType ?? `Vector ${i + 1}`);
              const probability = typeof cv?.probability === 'number' ? cv.probability : null;
              const required: string[] = Array.isArray(cv?.requiredSignals)
                ? cv.requiredSignals
                : [];
              const missing: string[] = Array.isArray(cv?.missingFromYourContent)
                ? cv.missingFromYourContent
                : [];
              return (
                <div key={i} className="rounded-xl border border-white/10 bg-charcoal-deep p-4">
                  <div className="flex items-center justify-between gap-2 mb-2">
                    <h5 className="text-sm font-medium text-white/90">
                      {patternLabel(contentType)}
                    </h5>
                    {probability !== null && (
                      <span
                    ...
                    <div className="mb-2">
                      ...
                      <div className="mt-1 flex flex-wrap gap-1.5">
                        {required.map((r, j) => (
                          <span
                            key={j}
                            className="rounded-md border border-emerald-400/15 bg-emerald-400/5 px-2 py-0.5 text-[11px] text-emerald-200/80"
                          >
                            {r}
                          </span>
                        ))}
                  ...
                  {missing.length > 0 && (
                    <div>
                      <span className="text-[10px] text-red-300/60 uppercase tracking-wide">
                      ...
                      <div className="mt-1 flex flex-wrap gap-1.5">
                        {missing.map((m, j) => (
                          <span
                            key={j}
                            className="rounded-md border border-red-400/20 bg-red-400/10 px-2 py-0.5 text-[11px] text-red-200/80"
                          >
                            {m}
                          </span>
                        ))}
      ...
      {sources.length > 0 && (
        <div className="rounded-2xl border border-white/10 bg-charcoal p-5">
          ...
          <p className="text-xs text-white/50 mb-4">
            The kinds of sources the AI drew from. Match these archetypes to increase your citation
            rate.
          </p>
          <div className="grid gap-3 sm:grid-cols-2">
            {sources.map((s, i) => {
                  ...
                  {indicators.length > 0 && (
                    <div className="mt-2 flex flex-wrap gap-1.5">
                      {indicators.map((ind, j) => (
                        <span key={j} className="text-xs text-white/55">
                          • {ind}
                        </span>
                      ))}
                    </div>
...
function GhostResult({ data }: { data: any }) {
  ...
  const schemas: any[] = Array.isArray(data.schemaRecommendations)
    ? data.schemaRecommendations
    : [];
  const linkPattern = data.internalLinkPattern;
  const prob =
    typeof data.estimatedInclusionProbability === 'number'
      ? data.estimatedInclusionProbability
      : null;

  const priorityColor = (p: string) => {
    if (p === 'critical' || p === 'required') return 'border-red-400/20 bg-red-400/10 text-red-200';
    if (p === 'important' || p === 'recommended')
    ...
    <div className="space-y-5">
        ...
        <div className="rounded-2xl border border-white/10 bg-charcoal p-5 flex flex-col sm:flex-row items-center gap-5">
          <div className="relative flex-shrink-0">
            <div className="flex h-24 w-24 items-center justify-center rounded-full border-4 border-white/10 bg-charcoal-deep">
              <span
                className={`text-3xl font-bold ${prob >= 0.7 ? 'text-emerald-200' : prob >= 0.4 ? 'text-amber-200' : 'text-white/70'}`}
              >
                {Math.round(prob * 100)}%
              </span>
            </div>
          ...
          <div>
            <h3 className="text-base font-semibold text-white">
              Estimated AI Inclusion Probability
            </h3>
      ...
      {bp && typeof bp === 'object' && (
        <div className="rounded-2xl border border-emerald-400/15 bg-gradient-to-br from-emerald-400/5 to-transparent p-5 sm:p-6">
          ...
          <p className="text-xs text-white/50 mb-5">
            Build this page exactly as described below. Each section has a purpose, word count
            target, and required entities.
          </p>
      ...
      {entities.length > 0 && (
        <div className="rounded-2xl border border-white/10 bg-charcoal p-5">
          ...
          <p className="text-xs text-white/50 mb-4">
            Entities and relationships your page must make explicit for AI models to extract
            cleanly.
          </p>
          <div className="grid gap-3 sm:grid-cols-2">
            {entities.map((e, i) => (
                  ...
                  <div className="mt-2 space-y-1">
                    {e.relationships.map((r: any, j: number) => (
                      <p key={j} className="text-xs text-white/55">
                        ...
                        {typeof r === 'string'
                          ? r
                          : `${r.relationship || 'relates to'} ${r.target || ''}`}
                      </p>
                    ))}
                  </div>
                )}
      ...
      {schemas.length > 0 && (
        <div className="rounded-2xl border border-white/10 bg-charcoal p-5">
          ...
          <p className="text-xs text-white/50 mb-4">
            Add these JSON-LD schema types to make your page machine-readable.
          </p>
          <div className="space-y-3">
            {schemas.map((s, i) => (
              <div key={i} className="rounded-xl border border-white/10 bg-charcoal-deep p-4">
                <div className="flex items-center justify-between gap-2 flex-wrap">
                  <h5 className="text-sm font-medium text-white/90">
                    {s.schemaType || `Schema ${i + 1}`}
                  </h5>
                  <div className="flex gap-2">
                    {s.priority && (
                      <span
                        className={`rounded-full border px-2 py-0.5 text-[10px] ${priorityColor(s.priority)}`}
                      >
                        {s.priority}
                      </span>
                    )}
      ...
      {trust.length > 0 && (
        <div className="rounded-2xl border border-white/10 bg-charcoal p-5">
          ...
          <p className="text-xs text-white/50 mb-4">
            Proof elements that increase AI confidence in your content.
          </p>
            ...
            {trust.map((t, i) => {
              ...
              return (
                <div key={i} className="rounded-xl border border-white/10 bg-charcoal-deep p-3.5">
                  ...
                  {t?.placement && (
                    <p className="mt-1 text-xs text-white/50">Place in: {t.placement}</p>
                  )}
                  {typeof t?.impact === 'number' && (
                    <div className="mt-2 h-1.5 rounded-full bg-white/10 overflow-hidden">
                      <div
                        className="h-full rounded-full bg-emerald-400/60"
                        style={{ width: `${Math.round(t.impact * 100)}%` }}
                      />
                    </div>
                  )}
                </div>
      ...
      {contentBands.length > 0 && (
        <div className="rounded-2xl border border-white/10 bg-charcoal p-5">
          ...
          <p className="text-xs text-white/50 mb-4">
            Word count zones for each section. Stay within optimal ranges.
          </p>
          <div className="space-y-2">
            {contentBands.map((b, i) => (
              <div
                key={i}
                className="rounded-xl border border-white/10 bg-charcoal-deep p-3.5 flex items-center justify-between gap-3 flex-wrap"
              >
                <div>
                  <h5 className="text-sm font-medium text-white/85">
                    {b.section || `Band ${i + 1}`}
                ...
                <div className="text-right">
                  <p className="text-sm font-bold text-white/85">
                    {b.optimalWords || b.minWords}–{b.maxWords} words
                  </p>
                </div>
              </div>
            ))}
      ...
      {linkPattern && typeof linkPattern === 'object' && (
        <div className="rounded-2xl border border-white/10 bg-charcoal p-5">
          ...
          <p className="text-xs text-white/50 mb-4">
            How to reinforce this page with your site's internal link structure.
          </p>
          ...
          <div className="grid gap-4 sm:grid-cols-2">
              ...
              <div>
                ...
                <div className="space-y-2">
                  {linkPattern.internalLinks.map((link: any, i: number) => (
                    <div
                      key={i}
                      className="rounded-lg border border-white/10 bg-charcoal-deep p-3 text-xs"
                    >
                      <span className="font-medium text-white/80">{link.anchor || 'Link'}</span>
                      {link.targetType && (
                        <span className="text-white/45"> → {link.targetType}</span>
                      )}
            ...
            {Array.isArray(linkPattern.externalAuthority) &&
              linkPattern.externalAuthority.length > 0 && (
                <div>
      ...
      {densityMap.length > 0 && (
        <div className="rounded-2xl border border-white/10 bg-charcoal p-5">
          ...
          <p className="text-xs text-white/50 mb-4">
            Subtopics that must be covered and their priority.
          </p>
          <div className="space-y-2">
            {densityMap.map((d, i) => {
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-2">
                    ...
                    {d.priority && (
                      <span
                        className={`rounded-full border px-2 py-0.5 text-[10px] ${priorityColor(d.priority)}`}
                      >
                        {d.priority}
                      </span>
                    )}
    ...
    <div className="space-y-5">
      ...
      {recs.length > 0 && (
        <div className="rounded-2xl border border-emerald-400/15 bg-gradient-to-br from-emerald-400/5 to-transparent p-5 sm:p-6">
          ...
          <p className="text-xs text-white/50 mb-4">
            Actionable changes based on how each model disagrees about your content.
          </p>
          <div className="space-y-3">
            {recs.map((r: any, i: number) => (
              <div key={i} className="rounded-xl border border-white/10 bg-charcoal-deep p-4">
                <div className="flex items-center justify-between gap-2 flex-wrap">
                  <div className="flex items-center gap-2">
                    <span className="flex h-6 w-6 items-center justify-center rounded-full bg-emerald-400/15 text-xs font-bold text-emerald-200">
                      {i + 1}
                    </span>
                    <h5 className="text-sm font-medium text-white/90">
                      {r.action || `Action ${i + 1}`}
                  ...
                  <div className="flex gap-2 flex-wrap">
                    ...
                    {r.effort && (
                      <span
                        className={`rounded-full border px-2 py-0.5 text-[10px] ${effortColor(r.effort)}`}
                      >
                        {r.effort} effort
                      </span>
                    )}
      ...
      {analyses.length > 0 && (
        <div className="rounded-2xl border border-white/10 bg-charcoal p-5">
          ...
          <p className="text-xs text-white/50 mb-4">
            How each AI model approaches the same query differently.
          </p>
          <div className="space-y-4">
            {analyses.map((entry: any, idx: number) => (
              <div key={idx} className="rounded-xl border border-white/10 bg-charcoal-deep p-5">
                <h5 className="text-sm font-semibold text-white/90 mb-4">
                  {entry.model || `Model ${idx + 1}`}
                </h5>
                ...
                <div className="grid gap-4 sm:grid-cols-2">
                  ...
                  {Array.isArray(entry.ignored) && entry.ignored.length > 0 && (
                    <div>
                      <p className="text-[10px] uppercase tracking-wide text-white/40 mb-2">
                        Ignores / De-prioritizes
                      </p>
                      <div className="flex flex-wrap gap-1.5">
                        {entry.ignored.map((e: string, i: number) => (
                          <span
                            key={i}
                            className="rounded-md border border-red-400/15 bg-red-400/10 px-2 py-0.5 text-[11px] text-red-200/80"
                          >
                            {e}
                          </span>
                        ))}
                    ...
                    <div>
                      ...
                      <div className="flex flex-wrap gap-1.5">
                        {entry.sourceBias.map((e: string, i: number) => (
                          <span
                            key={i}
                            className="rounded-md border border-white/10 bg-charcoal px-2 py-0.5 text-[11px] text-white/60"
                          >
                            {e}
                          </span>
                        ))}
                  ...
                  {Array.isArray(entry.structuralPreference) &&
                    entry.structuralPreference.length > 0 && (
                      <div>
                        <p className="text-[10px] uppercase tracking-wide text-white/40 mb-2">
      ...
      {matrix.length > 0 && (
        <div className="rounded-2xl border border-white/10 bg-charcoal p-5">
          ...
          <p className="text-xs text-white/50 mb-4">
            Per-model alignment score with gaps and strengths.
          </p>
          <div className="space-y-4">
            {matrix.map((m: any, i: number) => {
              ...
              const invisible: string[] = Array.isArray(m.invisibilityReasons)
                ? m.invisibilityReasons
                : [];
              return (
                <div key={i} className="rounded-xl border border-white/10 bg-charcoal-deep p-5">
                  ...
                  <div className="grid gap-3 sm:grid-cols-2">
                    {strengths.length > 0 && (
                      <div>
                        <p className="text-[10px] uppercase tracking-wide text-emerald-300/60 mb-2">
                          Strengths
                        </p>
                    ...
                    {gaps.length > 0 && (
                      <div>
                        <p className="text-[10px] uppercase tracking-wide text-red-300/60 mb-2">
                          Gaps
                        </p>
                        <div className="space-y-1">
                          {gaps.map((g, j) => (
                  ...
                  {invisible.length > 0 && (
                    <div className="mt-3">
                      <p className="text-[10px] uppercase tracking-wide text-amber-300/60 mb-2">
                        Why You're Invisible
                      </p>
                      <div className="space-y-1">
                        {invisible.map((r, j) => (
                          <p key={j} className="text-xs text-white/60 flex items-start gap-1.5">
                            <CircleAlert className="h-3 w-3 text-amber-300 mt-0.5 flex-shrink-0" />{' '}
                            {r}
                          </p>
                        ))}
                      </div>
...
function SimulateResult({ data }: { data: any }) {
  ...
  const probBar = (val: number, accent: string) => (
    <div className="flex items-center gap-2">
      ...
      <span className="text-sm font-bold text-white/85 min-w-[3rem] text-right">
        {Math.round(val * 100)}%
      </span>
    </div>
  );
    ...
    <div className="space-y-5">
        ...
        <div className="rounded-2xl border border-white/10 bg-charcoal p-5 sm:p-6">
          ...
          <div className="space-y-4">
            ...
            {projected !== null && (
              <div>
                ...
                {probBar(
                  projected,
                  delta !== null && delta > 0 ? 'bg-emerald-400/70' : 'bg-amber-400/70'
                )}
              </div>
            )}
          </div>
          {delta !== null && (
            ...
            >
              ...
              <span
                className={`text-sm font-semibold ${delta > 0 ? 'text-emerald-200' : delta < 0 ? 'text-red-200' : 'text-white/60'}`}
              >
                {delta > 0 ? '+' : ''}
                {Math.round(delta * 100)} percentage points
              </span>
              {ci && typeof ci.low === 'number' && typeof ci.high === 'number' && (
                <span className="text-xs text-white/40 ml-auto">
                  Confidence: {Math.round(ci.low * 100)}–{Math.round(ci.high * 100)}%
                </span>
              )}
            </div>
          )}
      ...
      {plan.length > 0 && (
        <div className="rounded-2xl border border-emerald-400/15 bg-gradient-to-br from-emerald-400/5 to-transparent p-5 sm:p-6">
          ...
          <p className="text-xs text-white/50 mb-4">
            Execute in this order for the best cumulative impact.
          </p>
          <div className="space-y-3">
            {plan.map((step: any, i: number) => {
              const cumProb =
                typeof step.cumulativeProbability === 'number' ? step.cumulativeProbability : null;
              return (
                <div key={i} className="rounded-xl border border-white/10 bg-charcoal-deep p-4">
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {step.element && (
                        <p className="text-xs text-white/50 mt-0.5">Element: {step.element}</p>
                      )}
                      {cumProb !== null && (
                        <div className="mt-2 flex items-center gap-2">
                          <div className="flex-1 h-1.5 rounded-full bg-white/10 overflow-hidden">
                            <div
                              ...
                              style={{ width: `${Math.round(cumProb * 100)}%` }}
                            />
                          </div>
                          <span className="text-[10px] text-white/50">
                            {Math.round(cumProb * 100)}% cumulative
                          </span>
                        </div>
                      )}
                      {Array.isArray(step.technicalRequirements) &&
                        step.technicalRequirements.length > 0 && (
                          <div className="mt-2 flex flex-wrap gap-1.5">
                            {step.technicalRequirements.map((req: string, j: number) => (
                              <span
                                key={j}
                                className="rounded-md border border-white/10 bg-charcoal px-2 py-0.5 text-[10px] text-white/50"
                              >
                                {req}
                              </span>
                            ))}
      ...
      {changes.length > 0 && (
        <div className="rounded-2xl border border-white/10 bg-charcoal p-5">
          ...
          <p className="text-xs text-white/50 mb-4">
            Each change and its estimated impact on visibility probability.
          </p>
          <div className="space-y-3">
                  ...
                  <div className="flex items-center justify-between gap-2 flex-wrap mb-2">
                    ...
                    <div className="flex gap-2">
                      ...
                      {c.effort && (
                        <span
                          className={`rounded-full border px-2 py-0.5 text-[10px] ${
                            /low|easy/i.test(c.effort)
                              ? 'border-emerald-400/20 bg-emerald-400/10 text-emerald-200'
                              : /high|hard/i.test(c.effort)
                                ? 'border-red-400/20 bg-red-400/10 text-red-200'
                                : 'border-amber-400/20 bg-amber-400/10 text-amber-200'
                          }`}
                        >
          ...
          <div className="space-y-2">
            {breakdown.map((m: any, i: number) => {
              ...
              return (
                <div
                  key={i}
                  className="rounded-xl border border-white/10 bg-charcoal-deep p-4 flex items-center justify-between gap-3 flex-wrap"
                >
                  ...
                  <div className="flex items-center gap-4 text-xs">
                    {before !== null && (
                      <span className="text-white/55">{Math.round(before * 100)}%</span>
                    )}
                    {before !== null && after !== null && <span className="text-white/30">→</span>}
                    {after !== null && (
                      <span className="text-white/80 font-medium">{Math.round(after * 100)}%</span>
                    )}
                    {change !== null && (
                      <span
                        className={`font-bold ${change > 0 ? 'text-emerald-300' : change < 0 ? 'text-red-300' : 'text-white/40'}`}
                      >
                        ({change > 0 ? '+' : ''}
...
export default function ReverseEngineerPage() {
  ...
  usePageMeta({
    ...
    structuredData: [
      buildWebPageSchema({
        ...
        description:
          'Reverse-engineer how AI models construct answers. Decompile answer structure, generate content blueprints, diff platform behavior, and simulate AI answer variants with the BRAG evidence framework.',
      }),
  ...
  const [recentInputs, setRecentInputs] = useState<
    Array<{ tool: Tool; input: string; secondary?: string }>
  >([]);
    ...
    try {
      ...
      switch (activeTool) {
        ...
        case 'rewrite': {
          ...
          const preset = VALID_PRESETS.includes(secondaryInput.trim())
            ? secondaryInput.trim()
            : 'authoritative';
      ...
      setRecentInputs((prev) => {
        ...
        const next = [
          ...
          ...prev.filter(
            (entry) =>
              !(
                entry.tool === activeTool &&
                entry.input === input.trim() &&
                entry.secondary === normalizedSecondary
              )
          ),
        ].slice(0, 6);
        try {
  ...
  const handleClientReport = () => {
    if (!result) return;
    const report = buildClientReportMarkdown(
      activeTool,
      primaryInput,
      secondaryInput.trim(),
      result
  ...
  const handleCreateSchema = () => {
    ...
    downloadTextFile(
      `${exportBasename}-schema-pack.json`,
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-1 text-sm text-slate-400">
          Deconstruct AI answers, generate page blueprints, compare models, and simulate
          probabilistic visibility impact.
        </p>
      </div>
        ...
        {!hasAccess ? (
          <UpgradeWall
            ...
            featurePreview={[
              ...
              "Simulate how structural changes affect your content's extractability and answer-space presence",
            ]}
          />
        ) : (
          <>
            <div className="grid gap-4 xl:grid-cols-[1.3fr_0.7fr]">
              <div className="rounded-2xl border border-white/10 bg-charcoal p-5 sm:p-6">
                ...
                <h2 className="mt-3 text-2xl brand-title">
                  Stop reading outputs like logs. Read them like leverage.
                </h2>
                <p className="mt-3 max-w-3xl text-sm leading-7 text-white/65">
                  This page is designed to turn model behavior into decisions. Run a query, extract
                  the signal, then route the result into blueprinting, remediation, or execution.
                </p>
              </div>
              ...
              <div className="rounded-2xl border border-white/10 bg-charcoal p-5 sm:p-6">
                ...
                <div className="mt-2 text-sm font-semibold text-white/85">
                  {isSignalTier
                    ? user?.tier === 'scorefix'
                      ? 'Score Fix advanced workflows'
                      : 'Signal advanced workflows'
                    : 'Alignment tier access'}
                </div>
            ...
            <div className="space-y-4 rounded-2xl border border-white/10 bg-charcoal p-4 sm:p-5">
              ...
              <div className="flex flex-wrap items-center justify-between gap-2 text-[11px] text-white/55">
                <span>
                  {currentTool.inputType === 'textarea'
                    ? 'Tip: Press Ctrl/Cmd + Enter to run quickly.'
                    : 'Tip: Press Enter to run quickly.'}
                </span>
                      ...
                      [
                        {
                          id: 'authoritative',
                          label: 'Authoritative',
                          desc: 'Confident, declarative, fact-forward',
                        },
                        {
                          id: 'conversational',
                          label: 'Conversational',
                          desc: 'Natural, direct, short sentences',
                        },
                        {
                          id: 'data_driven',
                          label: 'Data-Driven',
                          desc: 'Metrics and numbers for every claim',
                        },
              ...
              <div className="space-y-4">
                ...
                <div className="rounded-2xl border border-white/10 bg-charcoal p-4 sm:p-5">
                  ...
                  <div className="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
                    <Link
                      to="/app/score-fix"
                      className="rounded-xl border border-white/10 bg-charcoal-deep p-4 text-sm text-white/75 transition-colors hover:text-white"
                    >
                      Turn this into a Score Fix path
                    </Link>
                    <Link
                      to="/pricing?source=reverse-engineer"
                      className="rounded-xl border border-white/10 bg-charcoal-deep p-4 text-sm text-white/75 transition-colors hover:text-white"
                    >
