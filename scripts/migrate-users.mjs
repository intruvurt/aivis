...
if (OLD_DB === NEW_DB) {
  console.error(
    "ERROR: OLD_DATABASE_URL and DATABASE_URL are the same string. Aborting.",
  );
  process.exit(1);
}
...
async function run() {
  ...
  oldUsers.forEach((u, i) => {
    console.log(
      `  ${i + 1}. ${u.email}  tier=${u.tier}  verified=${u.is_verified}  role=${u.role || "user"}  created=${u.created_at?.toISOString?.().slice(0, 10)}`,
    );
  });

  // 3. Check which emails already exist in new DB
  const { rows: newExisting } = await newPool.query(
    "SELECT id, email FROM users",
  );
  const existingEmails = new Set(
    newExisting.map((r) => r.email.toLowerCase().trim()),
  );
  console.log(`\nNew DB already has ${newExisting.length} users.`);

  const toMigrate = oldUsers.filter(
    (u) => !existingEmails.has(u.email.toLowerCase().trim()),
  );
  const skipped = oldUsers.filter((u) =>
  ...
  toMigrate.forEach((u) =>
    console.log(`    + ${u.email} (${u.tier}, verified=${u.is_verified})`),
  );
    ...
    for (const u of toMigrate) {
      ...
      `,
        [
          ...
          u.org_phone,
          u.org_address,
          u.org_verified || false,
          u.org_verification_confidence,
          u.org_verification_reasons
            ? JSON.stringify(u.org_verification_reasons)
            : null,
          u.trial_ends_at,
          u.trial_used || false,
          u.trial_tier,
          u.trial_started_at,
          u.trial_converted || false,
        ],
      );
      ...
      if (res.rowCount > 0) {
        ...
        try {
          const { rows: usageRows } = await oldPool.query(
            "SELECT date, requests FROM usage_daily WHERE user_id = $1",
            [u.id],
          );
          ...
          console.warn(
            `    ⚠ usage_daily skip (table may not exist in old DB): ${e.message}`,
          );
        }
        ...
        try {
          const { rows: auditRows } = await oldPool.query(
            "SELECT id, url, result, source, created_at FROM audits WHERE user_id = $1",
            [u.id],
          );
