...
DO $$
BEGIN
  -- Guard: claims/entities may not exist yet (created by 010_graph_knowledge_layer.sql)
  -- This block is safe to run before or after 010 is applied.
  IF EXISTS (
    ...
    IF EXISTS (
      ...
      EXECUTE 'ALTER TABLE public.claims DROP CONSTRAINT claims_subject_id_fkey';
    END IF;

    -- Re-add with ON DELETE RESTRICT
    -- Entity cannot be deleted while claims reference it (safe, auditable)
    EXECUTE '
    ...
    -- Confirm NOT NULL is enforced (defensive)
    EXECUTE 'ALTER TABLE public.claims ALTER COLUMN subject_id SET NOT NULL';

    RAISE NOTICE '[013][2] claims.subject_id FK fixed: ON DELETE RESTRICT applied.';
  ELSE
    RAISE NOTICE '[013][2] SKIPPED: public.claims/entities do not exist yet. Run 010_graph_knowledge_layer.sql first, then re-run 013.';
  END IF;
END $$;
...
-- Backfill safety: add owner_id if table pre-existed without it
...
DO $$ BEGIN
  IF EXISTS (
    ...
    WHERE table_schema = 'public'
      ...
      AND constraint_name = 'fk_scans_workspace'
  ) THEN
    ALTER TABLE public.scans
      ADD CONSTRAINT fk_scans_workspace
      FOREIGN KEY (workspace_id) REFERENCES public.workspaces(id) ON DELETE CASCADE
      NOT VALID;
    RAISE NOTICE '[013][8] fk_scans_workspace FK added (NOT VALID — validate after backfill).';
  ELSIF NOT EXISTS (
    SELECT 1 FROM information_schema.tables
    WHERE table_schema = 'public' AND table_name = 'scans'
  ) THEN
    RAISE NOTICE '[013][8] SKIPPED fk_scans_workspace: public.scans does not exist yet.';
  END IF;
END $$;

DO $$ BEGIN
  -- Guard: claims table may not exist yet (created by 010_graph_knowledge_layer.sql)
  IF EXISTS (
    SELECT 1 FROM information_schema.tables
    WHERE table_schema = 'public' AND table_name = 'claims'
  ) THEN
    IF NOT EXISTS (
      ...
      WHERE table_schema = 'public'
        AND table_name = 'claims'
        AND constraint_name = 'fk_claims_workspace'
    ) THEN
      EXECUTE '
        ALTER TABLE public.claims
          ...
          NOT VALID
      ';
      RAISE NOTICE '[013][8] fk_claims_workspace FK added (NOT VALID — validate after backfill).';
    ELSE
      RAISE NOTICE '[013][8] fk_claims_workspace already exists, skipped.';
    END IF;
  ELSE
    RAISE NOTICE '[013][8] SKIPPED fk_claims_workspace: public.claims does not exist yet.';
  END IF;
END $$;
...
DECLARE
  inserted INTEGER := 0;
BEGIN
  -- Guard: if claims or claim_edges don't exist yet (pre-010 environment),
  -- return 0 cleanly rather than erroring. Run 010_graph_knowledge_layer.sql first.
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.tables
    WHERE table_schema = 'public' AND table_name = 'claims'
  ) OR NOT EXISTS (
    ...
    RAISE NOTICE 'generate_conflict_edges: public.claims/claim_edges missing. Returning 0.';
    RETURN 0;
  END IF;
  ...
  raw_pairs AS (
    SELECT
      ...
      1.0::REAL             AS w,
      -- Deterministic rank within each cluster; cap at 500 pairs per cluster
      ROW_NUMBER() OVER (
...
-- ============================================================
-- POST-RUN CHECKLIST (manual — do not automate these)
-- ============================================================
--
-- 1. REINDEX ivfflat indexes after 10K+ entity/claim rows loaded:
--      REINDEX INDEX CONCURRENTLY idx_entities_embedding;
--      REINDEX INDEX CONCURRENTLY idx_claims_embedding;
--      ANALYZE entities;
--      ANALYZE claims;
--
-- 2. Update application code before migration 014:
--      - All verification token READS must use verification_token_hash
--      - All verification token WRITES must SHA-256 hash before insert
--      - Stripe webhook handler must call record_stripe_webhook() as
--        first step and skip processing if it returns FALSE
--      - ingestion_jobs worker must use the UPDATE...RETURNING pattern
--        with worker_id and locked_at (see comment in section [3])
--
-- 3. After application is confirmed using verification_token_hash,
--    create migration 014 to drop the plaintext column:
--      ALTER TABLE users DROP COLUMN verification_token;
--
-- 4. For multi-tenant launch:
--    - VALIDATE the workspace FKs after backfilling workspace_id:
--        ALTER TABLE scans VALIDATE CONSTRAINT fk_scans_workspace;
--        ALTER TABLE claims VALIDATE CONSTRAINT fk_claims_workspace;
--
-- ============================================================
