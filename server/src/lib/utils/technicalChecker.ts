import { buildEvidence, normalizeEvidenceArray, attachEvidenceToClaim } from './evidenceUtils.js';
...
export const performTechnicalChecks = (crawlData: any, extractedData: any, url: string) => {
  ...
  try {
    ...
    // Check Twitter Cards
    const twitterTags = Object.keys(extractedData.metaTags).filter((key) =>
    ...
    if (extractedData.jsonLd.length > 0) {
      ...
      technicalData.schema.types = extractedData.jsonLd.map((ld: Record<string, unknown>) => ld['@type'] || 'Unknown');
      evidence.push(
        buildEvidence({
    ...
    evidence.push(
      buildEvidence({
        proof: null,
        source: url,
        description: `Technical checks error: ${(error as Error).message}`,
