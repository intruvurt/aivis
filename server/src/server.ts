...
import {
  ...
  buildPlatformIntelligencePromptBlock,
...
import {
  ...
  startMcpAuditLoop,
  stopMcpAuditLoop,
} from "./services/mcpAuditProcessor.js";
import {
  extractDeployHookPayload,
  resolveDeployHookBySecret,
...
import {
  ...
  startAutoScoreFixPostMergeLoop,
...
import {
  renderPlatformNewsletterEmail,
  sendPlatformNewsletterEmail,
...
import {
  enforceEffectiveTier,
  getAllowlistedElevatedEmails,
} from "./services/entitlementGuard.js";
import {
...
import {
  evaluateSSFRRules,
  buildSSFRSummary,
} from "./services/ssfrRuleEngine.js";
...
import {
  ...
  generateFingerprint,
  ocrCrossCheck,
} from "./services/deepAnalysisClient.js";
import {
  buildOcrEvidenceBlock,
  computeOcrCoverage,
...
function backgroundWithTimeout<T>(
  label: string,
  promise: Promise<T>,
  ...
  const timer = setTimeout(() => {
    console.warn(
      `[Background] ${label} exceeded ${timeoutMs}ms deadline — abandoned`,
    );
  }, timeoutMs);
  promise
    .catch((err: any) =>
      console.warn(`[Background] ${label} failed (non-fatal):`, err?.message),
    )
...
// Runs BEFORE all other middleware to ensure CORS headers are set on every response,
// including errors, OPTIONS preflight, and redirects.
// Must run BEFORE Helmet/applySecurityMiddleware so that security headers don't
...
const ANALYZE_REENTRANCY_GUARD_ENABLED =
  String(
    process.env.ANALYZE_REENTRANCY_GUARD_ENABLED ?? "true",
  ).toLowerCase() !== "false";
const ANALYZE_LOCK_TTL_MS = Number(process.env.ANALYZE_LOCK_TTL_MS || 120_000);
const STRICT_LIVE_SOFT_FAIL_ENABLED =
  String(process.env.STRICT_LIVE_SOFT_FAIL_ENABLED ?? "true").toLowerCase() !==
  "false";
...
function fireMeasurementEvent(
  ...
  sendMeasurementEvent({
    ...
    console.warn(
      `[GA4] Failed to send event ${eventName}:`,
      err?.message || err,
    );
  });
}
...
function verifyRecommendationEvidence<T extends { evidence_ids?: string[] }>(
  ...
  const enriched = recommendations.map((rec) => {
    ...
    const candidateEvidenceIds = Array.from(
      new Set([...rawEvidenceIds, ...inferredEvidenceIds]),
    );
    const validRefs = candidateEvidenceIds.filter((id) =>
  ...
  const evidenceCoveragePercent =
    total > 0 ? Math.round((recommendationsWithEvidence / total) * 100) : 0;
  const evidenceRefIntegrityPercent =
    totalEvidenceRefs > 0
...
function detectPeerCritiqueEvidenceConflicts(
  ...
  const mentionsMissingTldr =
    /(missing|no|lack(?:ing)?|without|add|include).{0,40}(tl;?dr|summary block|summary section|key takeaways)/i.test(
      normalized,
    ) ||
    /(tl;?dr|summary block|summary section|key takeaways).{0,40}(missing|not detected|absent)/i.test(
      normalized,
    );
  const mentionsBadMeta =
    /(truncated|incomplete|too long|too short).{0,40}meta description/i.test(
      normalized,
    ) ||
    /meta description.{0,40}(truncated|incomplete|too long|too short)/i.test(
      normalized,
    ) ||
  ...
  const mentionsNeedMoreQuestionHeadings =
    /(question[- ]format|question style|q\&a).{0,40}(h2|h3|heading)/i.test(
      normalized,
    ) ||
    /(convert|add|more).{0,40}(h2|h3|heading).{0,40}(question|q\&a)/i.test(
      normalized,
    );
  const mentionsNeedMoreSchema =
    /(add|more|increase|enhance).{0,50}(structured data|schema|json-ld)/i.test(
      normalized,
    ) ||
    /(structured data|schema|json-ld).{0,50}(missing|insufficient|limited|increase)/i.test(
      normalized,
    );
  const mentionsNeedMoreExternalCitations =
    /(add|more|increase|strengthen).{0,50}(authoritative|external).{0,40}(citation|citations|links?)/i.test(
      normalized,
    ) ||
    /(external).{0,30}(citation|citations|links?).{0,30}(missing|weak|insufficient)/i.test(
      normalized,
    );
  const mentionsNeedMoreContentDepth =
    /(enhance|increase|expand|improve).{0,40}(content depth|word count)/i.test(
      normalized,
  ...
  if (
    facts.metaDescriptionLength >= 120 &&
    facts.metaDescriptionLength <= 155 &&
    mentionsBadMeta
  ) {
    conflicts.add(
      `Meta description already within target range (${facts.metaDescriptionLength} chars)`,
    );
  }
  ...
  if ((facts.questionH2Count || 0) >= 2 && mentionsNeedMoreQuestionHeadings) {
    conflicts.add(
      `Question-format headings already strong (${facts.questionH2Count})`,
    );
  }

  if (
    (facts.schemaJsonLdCount || 0) >= 5 &&
    facts.hasFaqSchema &&
    mentionsNeedMoreSchema
  ) {
    conflicts.add(
      `Schema coverage already strong (${facts.schemaJsonLdCount} JSON-LD blocks incl. FAQPage)`,
    );
  }

  if (
    (facts.externalLinkCount || 0) >= 3 &&
    mentionsNeedMoreExternalCitations
  ) {
    conflicts.add(
      `External citation/link signals already present (${facts.externalLinkCount} external links)`,
    );
  }

  if ((facts.wordCount || 0) >= 1200 && mentionsNeedMoreContentDepth) {
    conflicts.add(
      `Content depth already within target range (${facts.wordCount} words)`,
    );
  }
...
function evaluateAi2PenaltyGate(facts: {
  hasTldr: boolean;
  metaDescriptionLength: number;
  h1Count: number;
  questionH2Count?: number;
  ...
  const checks = [
    ...
    {
      pass:
        facts.metaDescriptionLength >= 110 &&
        facts.metaDescriptionLength <= 170,
      label: `Meta description in target range (${facts.metaDescriptionLength} chars)`,
    },
    { pass: facts.h1Count === 1, label: `Exactly one H1 (${facts.h1Count})` },
    {
      pass: (facts.questionH2Count || 0) >= 1,
      label: `Question-format heading(s) present (${facts.questionH2Count || 0})`,
    },
    {
      pass: (facts.schemaJsonLdCount || 0) >= 4,
      label: `Schema JSON-LD depth adequate (${facts.schemaJsonLdCount || 0})`,
    },
    { pass: !!facts.hasFaqSchema, label: "FAQ schema present" },
    {
      pass: (facts.externalLinkCount || 0) >= 3,
      label: `External link/citation signals present (${facts.externalLinkCount || 0})`,
    },
    {
      pass: (facts.wordCount || 0) >= 1200,
      label: `Content depth threshold met (${facts.wordCount || 0} words)`,
    },
  ];
...
>(recommendations: T[]): T[] {
  ...
  const score = (rec: T): number => {
    const evidence = Array.isArray(rec.evidence_ids)
      ? rec.evidence_ids.length
      : 0;
    const descriptionLen = String(rec.description || "").length;
    return (
      evidence * 100 +
      priorityRank(rec.priority) * 10 +
      Math.min(9, Math.floor(descriptionLen / 80))
    );
  };
  ...
  for (const rec of recommendations) {
    ...
    const mergedEvidence = Array.from(
      new Set([...existingEvidence, ...currentEvidence]),
    );
    ...
    grouped.set(key, {
      ...
      impact:
        String(base.impact || "").length >= String(fallback.impact || "").length
          ? base.impact
          : fallback.impact,
...
function buildPublicSharedResult(
  result: any,
  ownerTierRaw: CanonicalTier | LegacyTier | string,
) {
  const ownerTier = uiTierFromCanonical(
    (ownerTierRaw as CanonicalTier | LegacyTier) || "observer",
  );
  ...
  const textSummary = result
    ? generateTextSummary(result, textSummaryDepth)
    : undefined;
  ...
  const recommendations = Array.isArray(safeResult.recommendations)
    ...
    : [];
  const redactedRecommendations = recommendations
    .slice(0, 3)
    .map((item: any) => ({
      ...item,
      implementation:
        "Redacted on Observer public links. Upgrade to view full implementation guidance.",
    }));

  // Observer share gets minimal text summary (2 findings, no fixes)
  const observerTextSummary = result
    ? generateTextSummary(result, "minimal")
    : undefined;
  ...
  return {
    ...
    redacted: true,
    redaction_note:
      "Observer public links include a redacted snapshot for safe distribution.",
  };
}
...
function stripObserverResult(result: any): any {
  ...
  const recommendations = Array.isArray(result.recommendations)
    ...
    : [];
  const strippedRecommendations = recommendations
    .slice(0, 3)
    .map((rec: any) => ({
      id: rec.id,
      title: rec.title,
  ...
  return {
    ...
    upgrade_cta: {
      message:
        "Unlock full implementation fixes, all recommendations, competitor tracking, and advanced tools.",
...
function applyTierResultStripping(result: any, tier: string): any {
  const canonicalTier = uiTierFromCanonical(
    (tier || "observer") as CanonicalTier | LegacyTier,
  );
  const tierLabel = getTierDisplayName(
    (tier || "observer") as CanonicalTier | LegacyTier,
  );
  const tierMeta = {
    analysis_tier: canonicalTier,
    analysis_tier_display: tierLabel,
  };
  if (tier === "observer")
    return { ...stripObserverResult(result), ...tierMeta };
  if (tier === "starter") return { ...stripStarterResult(result), ...tierMeta };
  if (tier === "alignment")
    return { ...stripAlignmentResult(result), ...tierMeta };
...
function assertCriticalEnvForProduction(): void {
  ...
  if (!hasAiProviderKey)
    missing.push("OPENROUTER_API_KEY (or OPEN_ROUTER_API_KEY)");
...
const matchesRateLimitPath = (req: Request, target: string): boolean => {
  ...
  return getRateLimitPathCandidates(req).some(
...
function refreshRateLimitOwnerCache(): void {
  ...
  getPool()
    .query(`SELECT id FROM users WHERE LOWER(email) = ANY($1::text[])`, [
      emails,
    ])
...
const apiLimiter = rateLimit({
  ...
  skip: (req) => {
    ...
    if (
      matchesRateLimitPath(req, "/api/health") ||
      matchesRateLimitPath(req, "/health")
    )
      return true;
    if (
      req.method === "GET" &&
      (matchesRateLimitPath(req, "/api/features/status") ||
        matchesRateLimitPath(req, "/features/status"))
    )
      return true;
    if (
      req.method === "GET" &&
      (startsWithRateLimitPath(req, "/api/features/notifications") ||
        startsWithRateLimitPath(req, "/features/notifications"))
    )
      return true;
    if (
      req.method === "GET" &&
      (matchesRateLimitPath(req, "/api/auth/profile") ||
        matchesRateLimitPath(req, "/auth/profile"))
    )
      return true;
    return false;
  },
...
const heavyActionLimiter = rateLimit({
  ...
  handler: (_req, res) => {
    res.status(429).json({
      error: "Too many high-cost requests",
      code: "HIGH_COST_RATE_LIMIT",
      retryAfter: 60,
    });
  },
});
...
const publicToolLimiter = rateLimit({
  ...
  handler: (_req, res) => {
    res.status(429).json({
      error: "Too many tool requests, please try again later",
...
const JSON_MW = express.json({
  ...
  verify: (req: Request, _res, buf) => {
    ...
    if (!STRIPE_WEBHOOK_PATHS.has(req.originalUrl))
...
app.use((req, res, next) => {
  ...
  if (req.originalUrl === "/api/analyze/upload")
    return JSON_UPLOAD_MW(req, res, next);
...
// OPTIONS fallback handler (catches any non-CORS preflight)
app.use((req, res, next) => {
...
app.use("/api/scorefix", ScoreFixRoutes);
...
app.get(
  "/api/audits/:auditId/deep-analysis",
  authRequired,
    ...
    try {
      ...
      if (!deepAnalysis) {
        return res.json({
          available: false,
          message:
            "Deep analysis not yet available for this audit. It runs as a background enrichment after the main audit completes.",
        });
      }
    ...
    return res.json({
      ...
      user: {
        ...
        trial_active: Boolean(
          ...
          new Date((user as any).trial_ends_at) > new Date(),
        ),
...
app.get("/api/analytics", authRequired, async (req: Request, res: Response) => {
  try {
    ...
    const analyticsTier = uiTierFromCanonical(
      ((req as any).user?.tier || "observer") as CanonicalTier,
    );
    if (!meetsMinimumTier(analyticsTier, "alignment")) {
      return res.status(403).json({
        success: false,
        error: "Analytics requires Alignment tier or higher.",
    ...
    for (const audit of audits) {
      ...
      if (
        ...
        new Date(existing.latestAudit.created_at).getTime()
      ) {
      ...
      const isHigher =
        !isNaN(score) &&
        (isNaN(existingBestScore) || score > existingBestScore);
      const isTieButNewer =
        !isNaN(score) &&
    ...
    const latestScore = latestScores.length
      ? latestScores[latestScores.length - 1]
      : 0;
    ...
    for (const a of latestPerUrlAudits) {
      ...
      if (ps && typeof ps === "object") {
        for (const [canonicalPlatform, aliases] of Object.entries(
          platformAliases,
        ) as Array<[keyof typeof platformAliases, string[]]>) {
    ...
    const platformAverages = Object.fromEntries(
      Object.entries(platformTotals).map(([k, vals]) => [
        k,
        vals.length
    ...
    const seoDiagnosticsSummary: Record<
      string,
    ...
    const scoreDistribution = Object.entries(scoreBuckets).map(
      ([bucket, count]) => ({
        bucket,
        count,
        pct: latestScores.length
          ? Math.round((count / latestScores.length) * 100)
          : 0,
      }),
    );
    ...
    const improvementDeltas = urlAggregates
      .map((item) => {
        ...
        const domain = (() => {
          try {
            return new URL(item.representativeUrl).hostname.replace(
              /^www\./,
              "",
            );
          } catch {
            return item.representativeUrl;
          }
        })();
        return {
          domain,
          ...
          delta:
            !isNaN(firstVal) && !isNaN(latestVal) ? latestVal - firstVal : null,
          audits: item.audits,
        };
      })
    ...
    try {
      ...
      const familyScoresList = familyScoresResult.rows
        .map((r) =>
          typeof r.family_scores === "string"
            ? JSON.parse(r.family_scores)
            : r.family_scores,
        )
        .filter(Boolean);
      ...
      if (familyScoresList.length > 0) {
        ...
        for (const f of families) {
          const vals = familyScoresList
            .map((s: Record<string, number>) => s[f])
      ...
      const rulePassRates = ruleStatsResult.rows.map((r) => ({
        ...
        pass_rate:
          Number(r.total) > 0
            ? Math.round((Number(r.pass_count) / Number(r.total)) * 100)
            : 0,
      }));
      ...
      deterministicData = {
        ...
        hard_blocker_stats: {
          ...
          avg_blockers_per_audit:
            Math.round(Number(blockerRow?.avg_blockers ?? 0) * 100) / 100,
        },
      };
    } catch (detErr: any) {
      console.warn(
        "[/api/analytics] Deterministic pipeline data query failed:",
        detErr?.message,
      );
    }
    ...
    const recommendationInsights = {
      ...
      avg_per_audit:
        auditsWithRecs > 0
          ? Math.round((totalRecommendations / auditsWithRecs) * 10) / 10
          : 0,
      ...
      if (sm && typeof sm === "object") {
        ...
        if (Array.isArray(types)) {
          for (const t of types) {
            ...
            if (name)
              schemaTypeCounts[String(name)] =
                (schemaTypeCounts[String(name)] || 0) + 1;
          }
        }
      }
    }
    const schemaInsights = {
      coverage_pct:
        latestPerUrlAudits.length > 0
          ? Math.round((auditsWithSchema / latestPerUrlAudits.length) * 100)
          : 0,
      avg_json_ld:
        latestPerUrlAudits.length > 0
          ? Math.round((totalJsonLd / latestPerUrlAudits.length) * 10) / 10
          : 0,
    ...
    const contentAndTechInsights = {
      ...
      https_pct:
        latestPerUrlAudits.length > 0
          ? Math.round((httpsCount / latestPerUrlAudits.length) * 100)
          : 0,
    ...
    const volatilityEntries = Object.entries(domainScoreSeries)
      ...
      .map(([key, scores]) => {
        ...
        const domain = (() => {
          try {
            return new URL(
              urlAggregateMap[key]?.representativeUrl || key,
        ...
        return {
          ...
          stddev,
          label: stddev < 5 ? "stable" : stddev < 15 ? "moderate" : "volatile",
        };
      })
    ...
    return res.json({
      ...
      data: {
        summary: {
          ...
          total_passes: Object.values(seoDiagnosticsSummary).reduce(
            (s, v) => s + v.pass,
            0,
          ),
          total_warns: Object.values(seoDiagnosticsSummary).reduce(
            (s, v) => s + v.warn,
            0,
          ),
          total_fails: Object.values(seoDiagnosticsSummary).reduce(
            (s, v) => s + v.fail,
            0,
          ),
        },
...
app.get(
  ...
  async (req: Request, res: Response) => {
    try {
      const userId = (req as any).user?.id || (req as any).userId;
      const miniTier = uiTierFromCanonical(
        ((req as any).user?.tier || "observer") as CanonicalTier,
      );
      ...
      const scores = rows
        .map((r) => Number(r.visibility_score))
        .filter((s) => !isNaN(s));
      ...
      const trendPts = rows.slice(-50).map((r) => ({
        ...
        chatgpt: (() => {
          try {
            ...
            return ps ? Number((ps as any).chatgpt) || null : null;
          } catch {
            return null;
          }
        })(),
        perplexity: (() => {
          try {
            const ps = r.platform_scores;
            return ps ? Number((ps as any).perplexity) || null : null;
          } catch {
            return null;
          }
        })(),
        google_ai: (() => {
          try {
            const ps = r.platform_scores;
            return ps
              ? Number((ps as any).google_ai ?? (ps as any).gemini_ai) || null
              : null;
          } catch {
            return null;
          }
        ...
        claude: (() => {
          try {
            const ps = r.platform_scores;
            return ps ? Number((ps as any).claude) || null : null;
          } catch {
            return null;
          }
        })(),
      ...
      return res.json({
        ...
        data: {
          ...
          trend: trendPts,
          distribution: Object.entries(dist).map(([bucket, count]) => ({
            bucket,
            count,
          })),
        },
      ...
      return res
        .status(500)
        .json({ success: false, error: "Failed to load mini analytics" });
    }
  },
);

app.get(
  "/api/analytics/domains.csv",
  authRequired,
  async (req: Request, res: Response) => {
    try {
      const analyticsUser = (req as any).user;
      const analyticsTier = (analyticsUser?.tier || "observer") as
        | CanonicalTier
        | LegacyTier;
      if (!meetsMinimumTier(analyticsTier, "alignment")) {
        return res.status(403).json({
      ...
      const audits = historyResult.rows as Array<{
        url: string;
      ...
      for (const audit of audits) {
        ...
        if (
          safeScore !== null &&
          (aggregate.bestScore === null || safeScore > aggregate.bestScore)
        ) {
      ...
      res.setHeader(
        "Content-Disposition",
        `attachment; filename=aivis-analytics-domains-${range}-${stamp}.csv`,
      );
      ...
      return res.status(500).json({
        success: false,
        error: "Failed to export analytics domains CSV",
      });
    }
  },
);
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const isUndefinedTable =
        ...
        message.includes('relation "user_sessions"');

      if (isUndefinedTable) {
        console.error(
          "[/api/analytics/platform-metrics] Missing analytics meta tables, returning empty metrics",
        );
        return res.json({
          success: true,
          data: {
      ...
      return res
        .status(500)
        .json({ success: false, error: "Failed to load platform metrics" });
    }
  },
);
...
app.get("/api/contacts", authRequired, async (req: Request, res: Response) => {
  try {
    ...
    return res.json({
      success: true,
      data: { contacts, range_days: days, max_score: maxScore },
    });
  } catch (err: any) {
...
app.get(
  ...
  async (_req: Request, res: Response) => {
    try {
      ...
      console.error(
        "[benchmarks] Public benchmark query failed:",
        err?.message,
      );
      res
        .status(500)
        .json({ success: false, error: "Benchmark data unavailable" });
    }
  },
);
...
const keywordEnrichCache = new Map<
  string,
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (!Array.isArray(keywords) || keywords.length === 0) {
        return res
          .status(400)
          .json({ success: false, error: "keywords array is required" });
      }
      ...
      if (sanitized.length === 0) {
        return res
          .status(400)
          .json({ success: false, error: "No valid keywords provided" });
      }
      ...
      if (toFetch.length > 0) {
        ...
        for (const r of freshResults) {
          ...
          if (key)
            keywordEnrichCache.set(key, {
              result: r,
              expiresAt: now + KEYWORD_CACHE_TTL,
            });
        }
      }
      ...
      return res.json({
        success: true,
        enriched: [...cached, ...freshResults],
      });
    } catch (err: any) {
      console.error("[keywords/enrich] Error:", err?.message);
      return res
        .status(500)
        .json({ success: false, error: "Keyword enrichment failed" });
    }
  },
);
...
Team updates
- New member: L.L.M. - Marketing Specialist (UTC-5)
  ...
  try {
    ...
    const fetchHeaders = async (method: "HEAD" | "GET") => {
      ...
      try {
        ...
        const body =
          method === "GET" ? (await response.text()).slice(0, 250_000) : "";
    ...
    if (!security.hsts && targetUrl.startsWith("https://")) {
      ...
      if (hstsMaxAge < 31536000)
        insights.push(
          `HSTS max-age is ${hstsMaxAge}s - recommend at least 31536000 (1 year).`,
        );
      if (!hstsIncludesSubs)
        insights.push("HSTS missing includeSubDomains directive.");
      if (!hstsPreload)
        insights.push(
          "HSTS missing preload directive - not eligible for browser preload list.",
        );
    }
    if (!security.csp) {
      ...
      if (!cspHasDefaultSrc)
        insights.push(
          "CSP missing default-src directive - incomplete fallback policy.",
        );
      if (!cspHasScriptSrc)
        insights.push(
          "CSP missing script-src directive - scripts unrestricted.",
        );
      if (cspHasUnsafeInline)
        insights.push(
          "CSP contains unsafe-inline - weakens XSS protection significantly.",
        );
    }
    if (!security.xFrameOptions) {
      ...
      insights.push(
        `X-Frame-Options value "${xfoRaw.trim()}" is non-standard - use DENY or SAMEORIGIN.`,
      );
    }
    if (!security.xContentTypeOptions) {
      ...
      insights.push(
        `X-Content-Type-Options value "${xctRaw.trim()}" is incorrect - must be "nosniff".`,
      );
    }
    if (!security.referrerPolicy) {
      insights.push(
        "Referrer-Policy header missing - browser may leak full URL on cross-origin navigations.",
      );
    } else if (!rpStrict) {
      insights.push(
        `Referrer-Policy "${rpRaw.trim()}" is permissive - consider strict-origin-when-cross-origin or no-referrer.`,
      );
    }
    if (!security.permissionsPolicy)
      insights.push(
        "Permissions-Policy header missing - browser features unrestricted by default.",
      );
    if (!cacheControl && !expires)
      insights.push("No cache lifetime headers detected.");
    else if (typeof maxAgeSeconds === "number" && maxAgeSeconds <= 0)
      insights.push("Cache-Control max-age is zero or negative.");
    if (!headers["server"]) insights.push("Server header hidden or stripped.");
    if (headers["x-robots-tag"])
      insights.push(`X-Robots-Tag present: ${headers["x-robots-tag"]}`);
    ...
    const hasTwitterCards =
      isHtml && /<meta[^>]+name=["']twitter:/i.test(htmlBody);
    const hasMetaDescription =
      isHtml &&
      /<meta[^>]+name=["']description["'][^>]+content=/i.test(htmlBody);
    const hasCanonical =
      isHtml && /<link[^>]+rel=["']canonical["'][^>]+href=/i.test(htmlBody);

    const richResultsRecommendations: string[] = [];
    if (isHtml && jsonLdCount === 0)
      richResultsRecommendations.push(
        "Add JSON-LD schema markup (Organization + WebPage/Article).",
      );
    if (isHtml && h1Count !== 1)
      richResultsRecommendations.push(
        "Use exactly one H1 to improve entity extraction consistency.",
      );
    if (isHtml && !hasMetaDescription)
      richResultsRecommendations.push(
        "Add a clear meta description for snippet control.",
      );
    if (isHtml && !hasCanonical)
      richResultsRecommendations.push(
        "Add canonical link tag to avoid duplicate-content ambiguity.",
      );
    if (isHtml && !hasOpenGraph)
      richResultsRecommendations.push(
        "Add Open Graph tags for richer social/AI previews.",
      );
    if (headers["x-robots-tag"]?.toLowerCase().includes("noindex")) {
      richResultsRecommendations.push(
        "Remove noindex directives for pages intended to rank in search and AI answers.",
      );
    }
    ...
    const richResultsScore = Math.max(
      0,
      Math.min(
        100,
        (!isHtml ? 30 : 0) +
    ...
    const riskLevel: "low" | "medium" | "high" =
      overallScore >= 80 ? "low" : overallScore >= 65 ? "medium" : "high";
    ...
    const topActions = [
      ...insights
        .filter((item) => item.toLowerCase().includes("missing"))
        .slice(0, 3),
    ...
    if (!security.csp)
      xssReasons.push("Missing CSP increases script injection risk.");
    if (!security.xContentTypeOptions)
      xssReasons.push(
        "Missing X-Content-Type-Options permits MIME sniffing vectors.",
      );
    if (!security.xFrameOptions)
      xssReasons.push(
        "Missing clickjacking protection can amplify XSS social engineering paths.",
      );
    if (!targetUrl.startsWith("https://"))
      xssReasons.push(
        "HTTP transport can enable content tampering in transit.",
      );
    const xssScore = Math.max(
      0,
      Math.min(
        100,
        (security.csp ? 45 : 10) +
    ...
    const xssRisk: "low" | "medium" | "high" =
      xssScore >= 80 ? "low" : xssScore >= 60 ? "medium" : "high";

    const ddosReasons: string[] = [];
    const hasEdgeSignal = Boolean(
      headers["cf-ray"] ||
    ...
    if (!hasEdgeSignal)
      ddosReasons.push(
        "No clear CDN/edge shielding signal detected from response headers.",
      );
    if (!hasCachePolicy)
      ddosReasons.push(
        "No cache policy detected, increasing origin load under bursts.",
      );
    if (!hasCompression)
      ddosReasons.push(
        "No compression signal detected, potentially increasing bandwidth pressure.",
      );
    if (!hasRateLimitHeaders)
      ddosReasons.push(
        "No visible rate-limit headers detected (may still exist internally).",
      );
    ...
    const ddosRisk: "low" | "medium" | "high" =
      ddosScore >= 80 ? "low" : ddosScore >= 60 ? "medium" : "high";

    const remediationChecklist = [
      ...missingHeaders.map(
        (header) => `Set ${header} with strict production values.`,
      ),
    ...
    const allowPrivateTargets =
      process.env.ALLOW_PRIVATE_ANALYZE_TARGETS === "true";
    try {
      ...
      return res.status(400).json({
        success: false,
        error: "Invalid final redirect URL",
    ...
    const code = message.toLowerCase().includes("abort")
...
app.post(
  "/api/tools/server-headers-check",
  ipRateLimit({ maxRequests: 10, windowMs: 60_000 }),
  handleServerHeadersCheck,
);
app.get(
  "/api/tools/server-headers-check",
  ipRateLimit({ maxRequests: 10, windowMs: 60_000 }),
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (!valid || !targetUrl) {
        return res
          .status(400)
          .json({ success: false, error: error || "Invalid URL" });
      }
      ...
      return res
        .status(500)
        .json({ success: false, error: "Failed to complete risk assessment" });
    }
  },
);

app.post(
  "/api/seo/audit/onpage",
  authRequired,
  heavyActionLimiter,
  usageGate,
  incrementUsage,
  async (req: Request, res: Response) => {
    try {
      ...
      if (sd.structuredData) {
        ...
        if (
          sd.structuredData.hasLocalBusiness &&
          !schemaMarkup.has_organization_schema
        ) {
      ...
      try {
        ...
        schemaMarkup.schema_score = scoreSchema(
          sd.html || "",
          pageContentSignals,
        );
      } catch {
      ...
      const questionReFaqOnpage =
        /\?|^(who|what|when|where|why|how|is|are|does|do|can|should|which)\b/i;
      const questionH3CountOnpage = (sd.headings?.h3 || []).filter(
        (t: string) => questionReFaqOnpage.test(t),
      ).length;
      ...
      return res
        .status(500)
        .json({ success: false, error: "Failed to run on-page SEO audit" });
    }
  },
);
...
function extractSchemaSignalsFromHtml(html: string): SchemaMarkup {
  ...
  while ((match = ldRegex.exec(html || "")) !== null) {
    try {
      ...
      schemaMarkup.validation_errors.push(
        "Malformed JSON-LD block (invalid JSON)",
      );
    }
  }
  ...
  for (const entity of allEntities) {
    ...
    const typeStr = Array.isArray(eType)
      ? eType.join(",")
      : String(eType || "");
    ...
    if (/review/i.test(typeStr) && entity.itemReviewed) {
      ...
      if (ir && typeof ir === "object" && !ir["@type"]) {
        schemaMarkup.validation_errors.push(
          "Review.itemReviewed missing @type - Google requires explicit type, not just @id reference",
        );
      }
    }
    ...
    if (/^product$/i.test(typeStr) && entity.offers) {
      if (!entity.image) {
        schemaMarkup.validation_errors.push(
          "Product missing image - required for Google Product snippets",
        );
      }
    }
    ...
    if (/article|blogposting|newsarticle/i.test(typeStr)) {
      if (
        ...
        schemaMarkup.validation_errors.push(
          `${typeStr} author missing @type or @id - required for article rich results`,
        );
      }
    }
    ...
    if (/faqpage/i.test(typeStr) && entity.mainEntity) {
      ...
      for (const q of questions) {
        if (!q.acceptedAnswer) {
          schemaMarkup.validation_errors.push(
            "FAQPage Question missing acceptedAnswer",
          );
          break;
        }
      }
  ...
  const typeCounts = topLevelTypes.reduce(
    (acc, t) => {
      acc[t] = (acc[t] || 0) + 1;
      return acc;
    },
  ...
  for (const [t, count] of Object.entries(typeCounts)) {
    if (count > 1) {
      schemaMarkup.validation_errors.push(
        `Duplicate standalone ${t} schema blocks (${count}) - may confuse structured data interpretation`,
      );
    }
  }

  schemaMarkup.schema_types = Array.from(new Set(collectedSchemaTypes));
  schemaMarkup.has_faq_schema =
    schemaMarkup.has_faq_schema ||
    schemaMarkup.schema_types.some((entry) =>
      /faqpage|faq/i.test(String(entry)),
    );
  schemaMarkup.has_organization_schema =
    schemaMarkup.has_organization_schema ||
    schemaMarkup.schema_types.some((entry) =>
      /organization|localbusiness/i.test(String(entry)),
    );
  return schemaMarkup;
}
...
function collectSeoIssueLabels(
  diagnostics: ReturnType<typeof computeSeoDiagnostics>,
): string[] {
  const issues: string[] = [];
  ...
  async (req: Request, res: Response) => {
    ...
    try {
      ...
      const normalizedTier = uiTierFromCanonical(
        (user?.tier || "observer") as CanonicalTier | LegacyTier,
      );
      if (!meetsMinimumTier(normalizedTier, "alignment")) {
        return res.status(403).json({
          success: false,
          error:
            "SEO crawl is available on Alignment, Signal, and Score Fix plans.",
          code: "TIER_INSUFFICIENT",
        });
      }
      ...
      const queue: Array<{ url: string; depth: number }> = [
        { url: normalizeCrawlUrl(rootUrl), depth: 0 },
      ];
        ...
        try {
          ...
          if (!sd) {
            pages.push({
              ...
              depth: current.depth,
              status: "error",
              seo_diagnostics: computeSeoDiagnostics(
                {},
                { json_ld_count: 0 },
                {
          ...
          const diagnostics = computeSeoDiagnostics(
            sd,
            schemaMarkup,
            technicalSignals,
            "url",
      ...
      const averageWordCount =
        pages.length > 0 ? Math.round(totalWordCount / pages.length) : 0;
      ...
      return res.status(500).json({
        success: false,
        error: "Failed to run SEO crawl. Please try again.",
        code: "SEO_CRAWL_FAILED",
      });
    } finally {
      client.release();
    }
    ...
    try {
      ...
      return res.json({
        ...
        data: result.rows.map((row) => ({
          ...
          issue_counts: (row.summary || {}).issue_counts || {
            pass: 0,
            warn: 0,
            fail: 0,
          },
      ...
      return res.status(500).json({
        success: false,
        error: "Failed to load crawl history",
        code: "SEO_CRAWL_LIST_FAILED",
      });
    }
  },
);
...
app.get(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      return res.status(500).json({
        success: false,
        error: "Failed to load SEO crawl",
        code: "SEO_CRAWL_FETCH_FAILED",
      });
    }
  },
);
...
function getServerApiKey(): string | null {
  return (
    process.env.OPENROUTER_API_KEY || process.env.OPEN_ROUTER_API_KEY || null
  );
}
...
const MAX_UPLOAD_FILES_BY_TIER: Record<
  "alignment" | "signal" | "scorefix",
  number
> = {
  alignment: 5,
  signal: 10,
...
const MAX_UPLOAD_TOTAL_BYTES_BY_TIER: Record<
  "alignment" | "signal" | "scorefix",
  number
> = {
...
async function runOcrOnBuffer(raw: Uint8Array): Promise<string> {
  ...
  try {
    const ocrPromise = Tesseract.recognize(Buffer.from(raw), "eng", {
      logger: () => undefined,
    });
...
async function extractUploadData(
  ...
  if (kind === "pdf") {
    ...
    try {
      ...
      const hasEnoughText =
        body.split(/\s+/).filter(Boolean).length >= PDF_TEXT_WORD_FLOOR;
      if (!hasEnoughText) {
        const imageResult = await parser
          .getImage({ imageBuffer: true, imageThreshold: 300 })
          .catch(() => null);
...
function mergeUploadData(
  files: UploadExtractedData[],
...
function detectWritingContentType(
  parsedFiles: ParsedUploadFile[],
  mergedBody: string,
): WritingContentType {
  const joinedNames = parsedFiles
  ...
  if (
    /research|whitepaper|paper|study|dataset|findings|report/.test(
      joinedNames,
    ) ||
    /methodology|data set|sample size|findings|limitations/.test(bodyLower)
  )
    return "research";
  if (
    /blog|post|newsletter/.test(joinedNames) ||
    /read time|subscribe|published|author/.test(bodyLower)
  )
    return "blog";
...
function detectUploadAnalysisMode(
  parsedFiles: ParsedUploadFile[],
  sd: UploadExtractedData,
): {
  ...
  if (hasHtml) {
    reasons.push(
      "HTML/page-template detected; using main website analysis path.",
    );
  ...
  if (hasCodeLike) {
    reasons.push(
      "Code/source file detected; using main website analysis path.",
    );
    return { mode: "main_analyze", contentType, reasons };
  }

  if (
    ["blog", "article", "ebook", "research"].includes(contentType) ||
    wordCount >= 120
  ) {
    reasons.push(
      "Prose/document content detected; using writing quality analysis path.",
    );
...
function normalizeWritingAudit(raw: any, contentType: WritingContentType) {
  ...
  const diff = diffRaw
    .map((item: any) => ({
      ...
      original:
        typeof item?.original === "string" ? stripEmDashes(item.original) : "",
      revised:
        typeof item?.revised === "string" ? stripEmDashes(item.revised) : "",
      reason:
        typeof item?.reason === "string"
          ? item.reason
          : "Clarity and quality improvement.",
    }))
    .filter((item: any) => item.original || item.revised);

  const rewrittenText = stripEmDashes(
    typeof rewriteRaw?.rewritten_text === "string"
      ? rewriteRaw.rewritten_text
      : "",
  );
  const deAiFindings = Array.isArray(raw?.de_ai_findings)
    ? raw.de_ai_findings.map((item: any) => String(item)).filter(Boolean)
    : [];
  const freshnessFindings = Array.isArray(raw?.freshness_findings)
    ? raw.freshness_findings.map((item: any) => String(item)).filter(Boolean)
    : [];
  ...
  const verdict = validVerdicts.has(rawVerdict)
    ? (rawVerdict as "keep" | "refresh" | "rebuild" | "merge" | "kill")
    : "refresh";
  ...
  return {
    ...
    seo_title:
      typeof raw?.seo_title === "string" ? stripEmDashes(raw.seo_title) : "",
    ...
    information_gain_score: clampScore(
      Number(raw?.information_gain_score || 0),
    ),
    citation_readiness_score: clampScore(
      Number(raw?.citation_readiness_score || 0),
    ),
    fact_check_items: factCheckItems,
    readability_level:
      typeof raw?.readability_level === "string"
        ? raw.readability_level.slice(0, 50)
        : "Unknown",
...
function computeGoalAlignment(
  ...
  const matched_goals = goals.filter((goal) =>
    textCorpus.includes(goal.toLowerCase()),
  );
  ...
  const score_adjustment = Math.max(
    -6,
    Math.min(6, Math.round((coverage - 0.5) * 12)),
  );
...
function computeMockDataScan(
  ...
  const patterns: Array<{ key: string; regex: RegExp }> = [
    ...
    {
      key: "your-company-template",
      regex: /\b(your company|your brand|your name)\b/i,
    },
    { key: "test-email", regex: /\b(test@example\.com|you@example\.com)\b/i },
    { key: "example-domain", regex: /\bexample\.com\b/i },
    {
      key: "under-construction",
      regex: /\b(coming soon|under construction)\b/i,
    },
  ];
...
function buildAnalysisIntegrity(params: {
  ...
  const executionClass = (() => {
    ...
    if (params.mode === "deterministic-fallback")
      return "DETERMINISTIC_FALLBACK";
    if (params.mode === "scrape-only") return "SCRAPE_ONLY";
    if (
      typeof params.fallbackMode === "string" &&
      params.fallbackMode.trim().length > 0
    )
      return "DETERMINISTIC_FALLBACK";
    return "LIVE";
  })();
  ...
  return {
    ...
    recommendation_evidence_coverage_percent: Math.max(
      ...
      Math.min(
        100,
        Number(
          params.recommendationEvidenceSummary?.evidence_coverage_percent ?? 0,
        ),
      ),
    ),
    recommendation_evidence_integrity_percent: Math.max(
      0,
      Math.min(
        100,
        Number(
          params.recommendationEvidenceSummary
    ...
    ...(params.normalizedTargetUrl
      ? { normalized_target_url: params.normalizedTargetUrl }
      : {}),
...
function buildGeoSignalProfile(result: any) {
  ...
  const brandEntityCount = Array.isArray(result?.brand_entities)
    ? result.brand_entities.length
    : 0;
  const entityClarity = Number(
    result?.domain_intelligence?.entity_clarity_score || 0,
  );
  const seo = result?.seo_diagnostics || {};
  const uniquenessStatus = String(
    seo?.content_uniqueness?.status || "",
  ).toLowerCase();
  ...
  const informationGain = (() => {
    ...
    if (
      wordCount >= 400 &&
      (uniquenessStatus === "pass" || uniquenessStatus === "warn")
    )
      return "standard";
    return "redundant";
  })();
...
function buildContradictionReport(
  result: any,
  geo: ReturnType<typeof buildGeoSignalProfile>,
) {
  const issues: Array<{
    id: string;
  ...
  if (!isUpload && !result?.technical_signals?.has_canonical) {
    issues.push({
      ...
      detail:
        "Canonical is absent, which creates index and citation ambiguity across duplicate URLs.",
      blocking: true,
  ...
  if (schemaCount === 0) {
    issues.push({
      ...
      detail:
        "No JSON-LD schema was extracted, reducing machine-verified identity and trust.",
      blocking: true,
  ...
  if (!geo.source_verified) {
    issues.push({
      ...
      detail:
        "Brand/entity clarity is too low for high-confidence source attribution.",
      blocking: true,
  ...
  if (
    String(seo?.title?.status || "").toLowerCase() === "fail" ||
    String(seo?.h1?.status || "").toLowerCase() === "fail"
  ) {
    issues.push({
      ...
      detail:
        "Critical heading metadata is missing or malformed, which weakens extractive answer confidence.",
      blocking: true,
  ...
  if (!geo.fact_unique) {
    issues.push({
      ...
      detail:
        "Detected content appears redundant or too generic for preferential citation.",
      blocking: false,
  ...
  if (!geo.relationship_anchored) {
    issues.push({
      ...
      detail:
        "Schema/entity linkage is incomplete, reducing graph-level trust signals for answer engines.",
      blocking: false,
  ...
  return {
    status:
      blockerCount > 0 ? "critical" : issues.length > 0 ? "attention" : "clean",
...
function derivePlatformScoresLegacy(baseScore: number): {
  chatgpt: number;
  perplexity: number;
  google_ai: number;
  claude: number;
...
function derivePlatformScores(
  ...
  try {
    ...
    console.warn(
      "Platform intelligence scoring failed, falling back to legacy:",
      e?.message,
    );
...
function deriveModelScores(params: {
  ...
  return METHODOLOGY_MODEL_PROFILES.map((profile) => {
    const weightedCategoryScore = Object.entries(
      profile.categoryWeights,
    ).reduce((sum, [label, weight]) => {
      const categoryScore = gradeMap.has(label)
        ? Number(gradeMap.get(label))
        : clampedBase;
      return sum + categoryScore * weight;
    }, 0);
...
function normalizeAnalysis(
  ...
  const normalizedGrades = REQUIRED_CATEGORIES.map(({ label }) => {
    ...
    const baseFallback =
      typeof ai.visibility_score === "number" && isFinite(ai.visibility_score)
        ...
        : 0;
    let score: number;
    const useDeterministicEvidenceScore =
      DETERMINISTIC_CATEGORY_LABELS.has(label) && !!evidenceBounds?.[label];
    ...
    if (existing) {
      return {
        ...
        improvements: Array.isArray(existing.improvements)
  ...
  const finalScore =
    Math.abs(aiScore - derivedScore) <= 5 ? aiScore : derivedScore;
  return {
    ...ai,
    visibility_score: finalScore,
    category_grades: normalizedGrades,
  };
}
...
function isUsableAiAnalysisPayload(payload: any): boolean {
  ...
  const hasScore =
    typeof payload.visibility_score === "number" &&
    Number.isFinite(payload.visibility_score);
  const hasSummary =
    typeof payload.summary === "string" && payload.summary.trim().length >= 20;
  const hasCategories =
    Array.isArray(payload.category_grades) &&
    payload.category_grades.length >= 3;
...
function computeEvidenceScores(
  ...
  const b: Record<
    string,
    { floor: number; ceiling: number; reasons: string[] }
  > = {};
  const normalizeBound = (bound: {
    ...
    return {
      ...
      reasons: [
        ...bound.reasons,
        "Bound normalization applied (floor exceeded ceiling)",
      ],
    };
  };
  ...
  const lexicalDiversity =
    bodyWords.length > 0 ? new Set(bodyWords).size / bodyWords.length : 0;
  ...
  if (wc < 100) {
    cF = 0;
    cC = 15;
    cR.push(`${wc} words - critically thin`);
  } else if (wc < 300) {
    cF = 5;
    cC = 35;
    cR.push(`${wc} words - insufficient for AI citation`);
  } else if (wc < 600) {
    cF = 15;
    cC = 55;
    cR.push(`${wc} words - sparse`);
  } else if (wc < 1000) {
    cF = 30;
    cC = 75;
    cR.push(`${wc} words - adequate`);
  } else if (wc < 1200) {
    cF = 45;
    cC = 88;
    cR.push(`${wc} words - good depth`);
  } else {
    cF = 55;
    cC = 100;
    cR.push(`${wc} words - strong depth`);
  }
  if (wc >= 2200 && lexicalDiversity >= 0.42 && schemaTypes.length >= 6) {
    cF = Math.max(cF, 82);
    cR.push(
      "Exceptional depth, lexical diversity, and rich structured context",
    );
  } else if (wc >= 2000 && lexicalDiversity >= 0.4 && schemaTypes.length >= 5) {
    cF = Math.max(cF, 78);
    cR.push("Substantial depth with broad vocabulary and structured context");
  } else if (
    wc >= 1800 &&
    lexicalDiversity >= 0.38 &&
  ...
  b["Content Depth & Quality"] = normalizeBound({
    floor: cF,
    ceiling: cC,
    reasons: cR,
  });
  ...
  if (h1 === 0) {
    hF = 0;
    hC = 20;
    hR.push("No H1 - critical");
  } else if (h1 > 1) {
    hF = 5;
    hC = 45;
    hR.push(`${h1} H1 tags - should be 1`);
  } else {
    hF = 40;
    hR.push("Single H1");
  }
  if (h2 === 0 && h3 === 0) {
    hC = Math.min(hC, 55);
    hR.push("No subheadings");
  } else if (h2 > 0 && h3 > 0) {
    hF = Math.max(hF, 50);
    hR.push("H2+H3 hierarchy");
  } else if (h2 > 0) {
    hF = Math.max(hF, 35);
    hR.push(`${h2} H2 tags`);
  }
  if (h1 === 1 && h2 >= 14 && h3 >= 14) {
    hF = Math.max(hF, 86);
    hR.push(
      "Exceptional heading architecture with deep subsection granularity",
    );
  } else if (h1 === 1 && h2 >= 10 && h3 >= 10) {
    hF = Math.max(hF, 82);
    hR.push(
      "Comprehensive heading architecture with extensive subsection depth",
    );
  ...
  b["Heading Structure & H1"] = normalizeBound({
    floor: hF,
    ceiling: hC,
    reasons: hR,
  });
  ...
  if (sc === 0) {
    sF = 0;
    sC = 12;
    sR.push("No JSON-LD - critical gap");
  } else if (sc === 1 && !hasOrg) {
    sF = 8;
    sC = 40;
    sR.push("1 schema, no Organization");
  } else if (hasOrg && hasFaq && sc >= 2) {
    sF = 55;
    sC = 100;
    sR.push(`${sc} schemas w/ Org+FAQ`);
  } else if (hasOrg) {
    sF = 30;
    sC = 75;
    sR.push(`${sc} schemas w/ Organization`);
  } else {
    sF = 12;
    sC = 55;
    sR.push(`${sc} schema blocks`);
  }
  if (
    hasOrg &&
    hasFaq &&
  ...
  if (schemaScore && typeof schemaScore.total === "number") {
    ...
    if (sq >= 85) {
      sF = Math.max(sF, 88);
      sR.push(
        `Schema quality ${sq}/100 - exceptional (entity graph, properties, vocabulary)`,
      );
    } else if (sq >= 70) {
      sF = Math.max(sF, 75);
  ...
  b["Schema & Structured Data"] = normalizeBound({
    floor: sF,
    ceiling: sC,
    reasons: sR,
  });
  ...
  if (!hasTitle) {
    ...
    mC = Math.min(mC, 70);
    mR.push(`Title ${titleLen}ch (ideal 30-60)`);
  } else {
    mF = Math.max(mF, 30);
    mR.push(`Title OK (${titleLen}ch)`);
  }
  if (!hasDesc) {
    mC = Math.min(mC, 40);
    mR.push("No meta description");
  } else if (descLen < 50 || descLen > 165) {
    mC = Math.min(mC, 75);
    mR.push(`Desc ${descLen}ch (ideal 120-155)`);
  } else {
    mF = Math.max(mF, 35);
    mR.push(`Desc OK (${descLen}ch)`);
  }
  if (!hasOg) {
    mC = Math.min(mC, 65);
    mR.push("No Open Graph");
  } else {
    mF = Math.max(mF, 25);
    mR.push("OG present");
  }
  ...
  b["Meta Tags & Open Graph"] = normalizeBound({
    floor: mF,
    ceiling: mC,
    reasons: mR,
  });
  ...
  if (!hasCan) {
    tC = Math.min(tC, 70);
    tR.push("No canonical");
  } else {
    tF = Math.max(tF, 20);
    tR.push("Canonical present");
  }
  if (intLinks === 0) {
    tC = Math.min(tC, 55);
    tR.push("No internal links");
  } else if (intLinks >= 5) {
    tF = Math.max(tF, 35);
    tR.push(`${intLinks} internal links`);
  }
  if (responseMs > 0) {
    if (responseMs <= 1800) {
      tF = Math.max(tF, 45);
  ...
  if (wc < 200) {
    rF = 0;
    rC = 20;
    rR.push("Content too thin for AI citation");
  } else if (wc < 500) {
    rC = Math.min(rC, 55);
    rR.push("Limited depth for citability");
  }
  if (h1 === 0 && h2 === 0) {
    rC = Math.min(rC, 30);
    rR.push("No heading structure");
  }
  if (sc > 0 && wc > 600) {
    rF = Math.max(rF, 35);
    rR.push("Schema + content depth aids AI");
  }
  if (hasFaq) {
    rF = Math.max(rF, 40);
    rR.push("FAQ schema boosts citability");
  }
  ...
  b["AI Readability & Citability"] = normalizeBound({
    floor: rF,
    ceiling: rC,
    reasons: rR,
  });
...
function buildDeterministicAnalysis(
  ...
  const categoryGrades = REQUIRED_CATEGORIES.map(({ label, weight }) => {
    const bound = evidenceBounds[label] || {
      floor: 20,
      ceiling: 55,
      reasons: ["Limited evidence"],
    };
    const spread = Math.max(0, bound.ceiling - bound.floor);
    const score = Math.round(
      Math.min(100, Math.max(0, bound.floor + spread * 0.55 + weight * 5)),
    );
    return {
      grade: scoreToGrade(score),
      label,
  ...
  const visibilityScore = Math.round(
    Math.min(
      ...
      Math.max(
        0,
        REQUIRED_CATEGORIES.reduce(
          (sum, { weight }, i) => sum + categoryGrades[i].score * weight,
          0,
        ),
      ),
    ),
  ...
  if (wordCount < 800) {
    recommendations.push({
      ...
      difficulty: "medium",
      implementation:
        "Add complete sections for proof points, FAQs, and differentiators directly in server-rendered HTML.",
    ...
    recommendations.push({
      ...
      description:
        h1Count === 0
          ...
          : `${h1Count} H1 headings detected; keep exactly one.`,
      impact: "Improves topic clarity for AI and search parsing.",
      difficulty: "easy",
      implementation:
        "Use one descriptive H1, then organize sections under H2/H3.",
  ...
  if (schemaCount === 0) {
    recommendations.push({
      ...
      difficulty: "medium",
      implementation:
        "Add Organization and WebPage schema first, then FAQ where applicable.",
  ...
  if (!hasMetaDescription) {
    recommendations.push({
      ...
      difficulty: "easy",
      implementation:
        "Write a 120-155 character description with value proposition + entity terms.",
  ...
  if (!hasCanonical) {
    recommendations.push({
      ...
      difficulty: "easy",
      implementation:
        "Add a canonical link element in the head for each canonical page.",
  ...
  if (recommendations.length < 6 && (!hasFaqSchema || wordCount < 1200)) {
    recommendations.push({
      ...
      description:
        "Strengthen machine-readable Q&A around services, pricing, and differentiators.",
    ...
    recommendations.push({
      ...
      description:
        "Add short Q&A blocks with evidence-backed facts in core sections to increase extractability.",
      impact: "Improves citation precision and answer snippet quality.",
      difficulty: "medium",
      implementation:
        "Add 3-5 concise answer blocks with concrete stats/examples and consistent entity naming.",
  ...
  return {
    ...
    ai_platform_scores: {
      ...
      perplexity: Math.round(
        Math.min(100, Math.max(0, visibilityScore * 0.82)),
      ),
    ...
    content_highlights: [
      {
        area: "heading",
        found: `${h1Count} H1 detected`,
        status: h1Count === 1 ? "good" : "warning",
        note: "Heading hierarchy impacts AI parsing.",
        source_id: "ev_h1_count",
      },
      {
        area: "schema",
        found: `${schemaCount} JSON-LD blocks`,
        status: schemaCount > 0 ? "good" : "critical",
        note: "Structured data supports citation confidence.",
        source_id: "ev_schema",
      },
      {
        area: "meta-tags",
        found: hasMetaDescription
          ? "Meta description present"
          : "Meta description missing",
        status: hasMetaDescription ? "good" : "warning",
        note: "Metadata improves contextual relevance.",
        source_id: "ev_meta_description",
      },
      {
        area: "technical",
        found: isHttps ? "HTTPS enabled" : "HTTPS missing",
        status: isHttps ? "good" : "critical",
        note: "HTTPS is a baseline trust signal.",
        source_id: "ev_https",
      },
      {
        area: "content",
        found: `${wordCount} words`,
        status: wordCount >= 800 ? "good" : "warning",
        note: "Depth directly affects answer quality and citations.",
        source_id: "ev_word_count",
      },
    ],
    crypto_intelligence: {
...
function computeSeoDiagnostics(
  ...
  const formInputCount = (html.match(/<(input|textarea|select)\b/gi) || [])
    .length;
  const labelTagCount = (html.match(/<label\b/gi) || []).length;
  const ariaLabelCount = (
    html.match(/\baria-label\s*=\s*['"][^'"]+['"]/gi) || []
  ).length;
...
function buildEvidenceFixPlan(
  ...
  const diagnostics =
    seoDiagnostics || ({} as ReturnType<typeof computeSeoDiagnostics>);
  ...
  for (const item of diagnosticConfig) {
    const check = diagnostics[item.key] as
      | { status?: "pass" | "warn" | "fail"; detail?: string }
      | undefined;
    if (!check || check.status === "pass") continue;

    const severity =
      check.status === "fail" ? item.severityWhenFail : item.severityWhenWarn;
    const relatedRecommendations = recommendationList
      .filter((rec) => {
        ...
        const mentionsArea =
          title.includes(item.area.toLowerCase()) ||
          description.includes(item.area.toLowerCase()) ||
          implementation.includes(item.area.toLowerCase());
        const linkedEvidence =
          Array.isArray(rec.evidence_ids) &&
          rec.evidence_ids.includes(item.evidenceId);
    ...
    issues.push({
      ...
      related_recommendation_ids: relatedRecommendations.length
...
function buildStrictRubricSystem(args: {
  ...
  const diagnostics =
    args.seoDiagnostics || ({} as ReturnType<typeof computeSeoDiagnostics>);
  const recSummary = args.recommendationEvidenceSummary || {};
  const fixPlan = args.evidenceFixPlan || {};
  const parityScore = Number(
    args.citationParityAudit?.ai_visibility_score_0_100 || 0,
  );
  ...
  const recommendationsWithEvidence = Number(
    recSummary.recommendations_with_evidence || 0,
  );
  ...
  const createFixpack = (
    ...
    const validation_steps = matchedIssues
      .flatMap((issue) =>
        Array.isArray(issue.validation_steps) ? issue.validation_steps : [],
      )
      .filter(
  ...
  const fixpackLibrary: StrictRubricSystem["required_fixpacks"] = [
    createFixpack(
      ...
      12,
      (issue) =>
        ["fix_https", "fix_robots", "fix_indexability"].includes(
          String(issue.id || ""),
        ),
    ),
    createFixpack(
      ...
      10,
      (issue) =>
        ["fix_title", "fix_meta_description", "fix_canonical"].includes(
          String(issue.id || ""),
        ),
    ),
    createFixpack(
      ...
      14,
      (issue) =>
        ["fix_h1", "fix_schema", "fix_content_uniqueness"].includes(
          String(issue.id || ""),
        ),
    ),
    createFixpack(
  ...
  const passRate =
    gates.length > 0 ? Number((passCount / gates.length).toFixed(3)) : 0;
  const projectedLift = requiredFixpacks.reduce(
    (sum, pack) => sum + pack.estimated_score_lift_min,
    0,
  );
...
function buildRailEvidenceAudit(args: {
  ...
  evidenceFixPlan?: {
    issue_count?: number;
    issues?: Array<{ evidence_ids?: string[] }>;
  };
  citationParityAudit?: {
    ai_visibility_score_0_100?: number;
    evidence?: unknown[];
  ...
  const scoreFor = (value: number): number =>
  ...
  const pickEvidence = (...preferred: string[]) => {
    ...
    const fallback = evidenceIds
      .filter((id) =>
        preferred.some((needle) => id.includes(needle.replace(/^ev_/, ""))),
      )
      .slice(0, 3);
    return fallback;
  };
  ...
  const headingCount =
    Number(headings.h1 || 0) +
    Number(headings.h2 || 0) +
    Number(headings.h3 || 0);
  ...
  const hasStructuredSchema =
    schemaCount > 0 ||
    (Array.isArray(schema.schema_types) && schema.schema_types.length > 0);
  const hasTitle = Boolean(String(domain.page_title || "").trim());
  const hasDescription = Boolean(String(domain.page_description || "").trim());
  const hasCanonical =
    Boolean(technical.has_canonical) ||
    Boolean(String(domain.canonical_url || "").trim());
  const hasHttps =
    args.sourceType === "upload" ? true : Boolean(technical.https_enabled);
  const coverage = Number(recSummary.evidence_coverage_percent || 0);
  const integrity = Number(recSummary.evidence_ref_integrity_percent || 0);
  const recommendationsWithEvidence = Number(
    recSummary.recommendations_with_evidence || 0,
  ...
  const claudeChecks: RailCheck[] = [
    ...
    createCheck(
      ...
      hasStructuredSchema
        ? `Schema markers detected (${schemaCount} JSON-LD blocks).`
        : "No schema markers detected in evidence manifest.",
  ...
  const trackingChecks: RailCheck[] = [
    createCheck(
      ...
      coverage >= 70 && integrity >= 85
        ? "pass"
        : coverage >= 45 && integrity >= 70
          ? "warn"
          : "fail",
    ...
    createCheck(
      ...
      fixIssueCount >= 3 && fixPlanEvidenceRefs >= fixIssueCount
        ? "pass"
        : fixIssueCount > 0
          ? "warn"
          : "fail",
    ...
    createCheck(
      ...
      parityScore >= 65 && parityEvidenceCount >= 5
        ? "pass"
  ...
  const scoreTrack = (checks: RailCheck[]) => {
    ...
    return scoreFor(
      checks.reduce((sum, check) => sum + check.score_0_100, 0) / checks.length,
    );
  };
  ...
  const overallScore = scoreFor(
    (claudeScore + googleScore + perplexityScore + trackingScore) / 4,
  );
  ...
  if (coverage < 70)
    bragReasons.push(
      `Evidence coverage below target (current ${coverage}%, target >= 70%).`,
    );
  if (integrity < 85)
    bragReasons.push(
      `Evidence integrity below target (current ${integrity}%, target >= 85%).`,
    );
  if (fixIssueCount <= 0)
    bragReasons.push("No evidence-linked fix issues were generated.");
  if (bragReasons.length === 0) {
    bragReasons.push(
      "BRAG thresholds satisfied: recommendation evidence coverage/integrity and fix traceability are in acceptable range.",
    );
  }
...
app.get(
  ...
  async (req: Request, res: Response) => {
    ...
    if (entry.owner_user_id !== viewerUserId) {
      return res
        .status(403)
        .json({ error: "Forbidden", code: "PROGRESS_FORBIDDEN" });
    }
    ...
    res.write(
      `data: ${JSON.stringify({ step: entry.step, percent: entry.percent })}\n\n`,
    );
...
app.get(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const hosts: Record<
        string,
        { keywords: any[]; auditCount: number; latestUrl: string }
      > = {};
      for (const row of rows) {
        ...
        if (!hosts[host])
          hosts[host] = { keywords: [], auditCount: 0, latestUrl: row.url };
      ...
      return res
        .status(500)
        .json({ success: false, error: "Failed to load keyword data" });
    }
  },
);
...
app.get("/api/audits", authRequired, async (req: Request, res: Response) => {
  try {
    ...
    const auditTier = uiTierFromCanonical(
      (auditUser?.tier || "observer") as CanonicalTier | LegacyTier,
    );
    ...
    const rawWsHeader =
      String(req.headers["x-workspace-id"] || "").trim() || null;
    ...
    if (teamView) {
      ...
      const countResult = await pool.query(
        "SELECT COUNT(*) AS total FROM audits WHERE user_id = $1",
        [userId],
      );
      total = Number(countResult.rows[0]?.total || 0);
    }
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const total = rows.filter(
        (row: any) =>
          normalizeAuditTargetKey(String(row.url || "")) === targetKey,
      ).length;
      ...
      return res
        .status(500)
        .json({ error: "Failed to fetch target scan count" });
    }
  },
);
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const { rowCount } = await pool.query(
        "DELETE FROM audits WHERE id = $1 AND user_id = $2",
        [auditId, userId],
      );
...
async function loadOwnedAuditWithResult(
  pool: any,
  userId: string,
  auditId: string,
) {
...
async function seedFixpackLifecycleRows(
  pool: any,
  userId: string,
  auditId: string,
  auditResult: Record<string, unknown> | null,
) {
  const strictRubric = auditResult?.strict_rubric as
...
app.get(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      await seedFixpackLifecycleRows(
        pool,
        userId,
        auditId,
        auditRow.result as Record<string, unknown> | null,
      );
      ...
      return res
        .status(500)
        .json({ error: "Failed to fetch fixpack statuses" });
    }
  },
);

app.patch(
  "/api/audits/:auditId/fixpacks/:packId",
  authRequired,
  async (req: Request, res: Response) => {
    try {
      const userId = (req as any).user?.id;
      ...
      if (!FIXPACK_ALLOWED_STATUSES.has(status)) {
        return res.status(400).json({
          error:
            "status must be one of open, in-progress, blocked, resolved, validated, verification_failed, or closed",
        });
      }
      const lifecycleStateRaw = String(body?.lifecycleState || "").trim();
      const owner = body?.owner
        ? String(body.owner).trim().slice(0, 255)
        : null;
      const blockerReason = body?.blockerReason
        ? String(body.blockerReason).trim().slice(0, 1000)
        : null;
      const verificationNotes = body?.verificationNotes
        ? String(body.verificationNotes).trim().slice(0, 2000)
        : null;
      ...
      if (reAuditId) {
        ...
        if (!reAuditRows.length) {
          return res
            .status(400)
            .json({ error: "reAuditId must reference one of your audits" });
        }
      }
...
app.get(
  "/api/audits/:auditId/score-roadmap",
  authRequired,
  async (req: Request, res: Response) => {
    try {
      const userId = (req as any).user?.id;
      ...
      const [ruleResults, scoreSnap, fixpacks, brag] = await Promise.all([
        pool.query(
          "SELECT * FROM audit_rule_results WHERE audit_id = $1 ORDER BY rule_id",
          [auditId],
        ),
        pool.query(
          "SELECT * FROM audit_score_snapshots WHERE audit_id = $1 ORDER BY created_at DESC LIMIT 1",
          [auditId],
        ),
        pool.query(
          "SELECT * FROM fixpacks WHERE audit_run_id = $1 ORDER BY fixpack_type",
          [auditId],
        ),
        pool.query(
          "SELECT * FROM brag_trail WHERE audit_run_id = $1 ORDER BY created_at",
          [auditId],
        ),
      ]);
...
app.post(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const shareLinkTier = (shareLinkUser?.tier || "observer") as
        | CanonicalTier
        | LegacyTier;
      ...
      if (!TIER_LIMITS[shareLinkUiTier].hasShareableLink) {
        return res.status(403).json({
          error:
            "Shareable report links are not enabled for your current plan.",
          code: "TIER_INSUFFICIENT",
      ...
      if (!auditId && (!analyzedAt || Number.isNaN(Date.parse(analyzedAt)))) {
        return res
          .status(400)
          .json({ error: "Missing or invalid analyzedAt timestamp" });
      }
      ...
      if (!rows[0]?.id && auditId && allowCrossWorkspaceFallback) {
        ...
        if (rows[0]?.id) {
          console.log(
            `[Share-link] Fallback matched audit ${rows[0].id} by url+timestamp (workspace mismatch resolved)`,
          );
        }
      }
      ...
      if (!rows[0]?.id && allowCrossWorkspaceFallback) {
        ...
        const match = broadFallback.rows.find(
          (row: any) =>
        ...
        if (match?.id) {
          rows = [match];
          console.log(
            `[Share-link] Broad fallback matched audit ${match.id} by normalized URL key`,
          );
        }
      }
      ...
      if (!audit?.id) {
        console.error(
          "[Share-link] Audit NOT FOUND even after all fallbacks. Params:",
          JSON.stringify({
        ...
        return res
          .status(404)
          .json({ error: "Matching audit not found for this report" });
      }
      ...
      const scanOrdinalIndex = matchingTargetAudits.findIndex(
        (row: any) => String(row.id) === String(audit.id),
      );
      const scanOrdinal = scanOrdinalIndex >= 0 ? scanOrdinalIndex + 1 : 1;
      const scanLabel =
        scanOrdinal <= 1 ? "First scan" : `Scan #${scanOrdinal}`;
...
app.post(
  "/api/public/deploy-hooks/:hookId",
  ipRateLimit({ maxRequests: 5, windowMs: 60_000 }),
  async (req: Request, res: Response) => {
    try {
      ...
      if (!hookId || !providedSecret) {
        return res
          .status(401)
          .json({ success: false, error: "Deploy hook secret required" });
      }
      ...
      if (!hook) {
        return res
          .status(401)
          .json({ success: false, error: "Invalid deploy hook credentials" });
      }
      ...
      const normalized = targetUrl
        ? normalizePublicHttpUrl(targetUrl)
        : { ok: false, error: "No deployment URL found in hook payload" };
...
app.get(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (!decoded) {
        console.warn(
          `[public-report] Reference resolution returned null (len=${token.length}, isLegacy=${token.includes(".")})`,
        );
        return res
          .status(401)
          .json({ error: "Invalid or expired public report token" });
      }
      ...
      const shared = buildPublicSharedResult(
        audit.result,
        audit.owner_tier || "observer",
      );
      const ownerCanonical = uiTierFromCanonical(
        (audit.owner_tier || "observer") as CanonicalTier | LegacyTier,
      );
      ...
      return res.json({
        ...
        analysis_tier_display: getTierDisplayName(
          (audit.owner_tier || "observer") as CanonicalTier | LegacyTier,
        ),
      });
    } catch (err: any) {
...
app.post(
  ...
  async (req: Request, res: Response) => {
    ...
    try {
      ...
      if (!zParsed.success) {
        logMalformedPayload(req, ownerUserId, "Zod validation failed", {
          issues: zParsed.error.issues,
          route: "/api/analyze",
        });
        return res.status(400).json({
      ...
      const {
        url: rawUrl,
        apiKey: clientKey,
        findabilityGoals: rawFindabilityGoals,
      } = (req.body || {}) as any;
      if (clientKey) {
        logInvalidApiKey(req, "client-provided-provider-key");
        return res.status(400).json({
          error: "Client-provided API keys not allowed",
      ...
      if (!valid || !targetUrl) {
        ...
        if (/private|localhost|loopback/i.test(reason)) {
          ...
          logMalformedPayload(req, ownerUserId, reason, {
            rawUrl,
            route: "/api/analyze",
          });
        }
      ...
      const normalizedRequestTier = uiTierFromCanonical(
        (userTier || "observer") as CanonicalTier | LegacyTier,
      );
      const forceRefreshRequested =
        Boolean((req.body as any)?.forceRefresh) || req.query.force === "1";
      ...
      const forceRefresh =
        ...
        TIER_LIMITS[normalizedRequestTier].hasForceRefresh;
      const retryRequested =
        Boolean((req.body as any)?.retryRequested) || forceRefresh;
      ...
      const auditCreditPlan = buildAuditCreditPlan({
        retryRequested,
        scanMockData,
        hasFindabilityGoals,
      });
      const effectiveMonthlyLimit =
        Number((req as any).monthlyLimit ?? 0) ||
        TIER_LIMITS[normalizedRequestTier]?.scansPerMonth ||
        0;
      const effectiveCurrentUsage = Number((req as any).currentUsage ?? 0);
      const remainingMonthlyScans = Math.max(
        0,
        effectiveMonthlyLimit - effectiveCurrentUsage,
      );

      if (auditCreditPlan.total > 0) {
        const availableCredits = await getAvailablePackCredits(ownerUserId);
        if (
          availableCredits < auditCreditPlan.total &&
      ...
      const consumeAuditCreditsOrThrow = async () => {
        ...
        if (availableCredits < auditCreditPlan.total) {
          ...
          throw Object.assign(
            new Error(
              `This audit requires ${auditCreditPlan.total.toFixed(2)} credits from your balance. Available: ${availableCredits.toFixed(2)}.`,
            ),
            {
              code: "INSUFFICIENT_PACK_CREDITS",
              status: 402,
        ...
        const charge = await consumePackCredits(
          ownerUserId,
          auditCreditPlan.total,
          "audit_advanced_validation",
          {
            url: targetUrl,
            requestId,
            retryRequested,
        ...
        if (!charge.consumed) {
          throw Object.assign(
            new Error(
              "Advanced audit credits were no longer available. Please refresh your balance and retry.",
            ),
            {
              code: "INSUFFICIENT_PACK_CREDITS",
              status: 402,
      ...
      const cached =
        forceRefresh || hasFindabilityGoals || scanMockData || requireLiveAi
          ? null
          : await AnalysisCacheService.get(cacheKey);
      console.log(
        `[${requestId}] Cache check took ${Date.now() - cacheStart}ms`,
      );
      ...
      if (cached) {
        ...
        const hasFallbackMode =
          typeof (cached as any).fallback_mode === "string" &&
          (cached as any).fallback_mode.length > 0;
        const cacheTier = String((cached as any).cache_tier || "")
          .trim()
          .toLowerCase();
        ...
        if (
          ...
          req.usageSkipIncrement = true;
          console.log(
            `[${requestId}] Cache hit for ${cacheKey} (score: ${cachedScore})`,
          );
          ...
          try {
            ...
            console.warn(
              `[${requestId}] Cache-hit audit insert failed (non-fatal):`,
          ...
          const cachedWithIntegrity = {
            ...cached,
            analysis_integrity:
              (cached as any).analysis_integrity ||
              buildAnalysisIntegrity({
                mode: "live",
                evidenceManifest: (cached as any).evidence_manifest,
                modelCount: Number((cached as any).model_count || 1),
                tripleCheckEnabled: Boolean(
                  (cached as any).triple_check_enabled,
                ),
                recommendationEvidenceSummary: (cached as any)
                  .recommendation_evidence_summary,
                normalizedTargetUrl: targetUrl,
                warnings: [],
              }),
          };
          const cachedTextSummary = generateTextSummary(
            cachedWithIntegrity as any,
            getTextSummaryDepth(normalizedRequestTier as CanonicalTier),
          );
          const cachedPayload = {
            ...cachedWithIntegrity,
            cached: true,
          ...
          return res.json(
            applyTierResultStripping(cachedPayload, normalizedRequestTier),
          );
        }
        console.log(
      ...
      if (!req.usageSkipIncrement && !req.usingPackCredits) {
        try {
          ...
          console.error(
            `[${requestId}] CRITICAL: Metering write failed:`,
            meterErr?.message || meterErr,
          );
          return res.status(503).json({
      ...
      try {
        ...
        console.warn(
          `[${requestId}] DNS pre-check failed (non-fatal): ${dnsErr.message}`,
        );
      }
      ...
      try {
        ...
        if (requireLiveAi) {
          if (!STRICT_LIVE_SOFT_FAIL_ENABLED) {
            return res.status(503).json({
              error:
                "Live crawl failed. Demo strict mode requires successful live crawl + AI analysis. Retry on a crawlable URL.",
              code: "LIVE_CRAWL_REQUIRED",
              request_id: requestId,
            });
          }
          console.warn(
            `[${requestId}] Strict live mode soft-fail: crawl failed, returning degraded domain-only output`,
          );
        }
        console.warn(
          `[${requestId}] Scrape failed - using domain-only fallback: ${scrapeErr.message}`,
        );
      ...
      const _metaDescGrade =
        _metaDescLen === 0
          ? "MISSING - CRITICAL"
          : _metaDescLen < 70
            ? `${_metaDescLen} chars - TOO SHORT (target 120–160)`
            : _metaDescLen > 165
              ? `${_metaDescLen} chars - TOO LONG (will truncate in SERPs/AI previews)`
      ...
      const _titleGrade =
        _titleLen === 0
          ? "MISSING - CRITICAL"
          : _titleLen < 30
            ? `${_titleLen} chars - TOO SHORT (target 50–60)`
            : _titleLen > 65
              ? `${_titleLen} chars - TOO LONG (may truncate in SERPs)`
              : `${_titleLen} chars - OK`;

      const _h1Grade =
        _h1s.length === 0
          ? "MISSING - CRITICAL (AI models need a primary heading to anchor citations)"
          : _h1s.length === 1
            ? `1 H1: "${_h1s[0].substring(0, 80)}" - OK`
      ...
      const _lcpGrade =
        _lcpMs <= 0
          ...
          : _lcpMs < 1200
            ? `${_lcpMs}ms - FAST`
            : _lcpMs < 2500
              ? `${_lcpMs}ms - NEEDS IMPROVEMENT (target <2.5s)`
      ...
      const _imgAltGrade =
        _imgTags.length === 0
          ? "No images detected"
          : _imgsNoAlt === 0
            ? `${_imgTags.length} images - all have alt text - OK`
      ...
      const _robotsAiGrade = (() => {
        const robotsPart =
          _robotsAllowsGpt === false
            ? "GPTBot/AI crawlers BLOCKED in robots.txt - CRITICAL: AI indexing prevented"
            : _robotsAllowsGpt === true
              ? "No AI crawler blocks in robots.txt - OK"
              : "robots.txt not fetched or unknown";
        const lt = (sd as any).llmsTxt;
        const llmsPart = !lt?.fetched
          ? ""
          : lt.present
            ? " | llms.txt PRESENT - AI guidance file found"
      ...
      const _ogComplete =
        sd.meta?.ogTitle && sd.meta?.ogDescription
          ? "og:title + og:description present - OK"
          : !sd.meta?.ogTitle && !sd.meta?.ogDescription
            ? "og:title MISSING, og:description MISSING - no social/AI preview coverage"
            : !sd.meta?.ogTitle
              ? "og:title MISSING"
      ...
      const _wordCountGrade =
        (sd.wordCount || 0) < 200
          ? `${sd.wordCount || 0} words - CRITICAL (below 200 - insufficient for AI citation eligibility)`
          : (sd.wordCount || 0) < 500
            ? `${sd.wordCount || 0} words - LOW (target 800+ for knowledge-base eligibility)`
            : (sd.wordCount || 0) < 800
              ? `${sd.wordCount || 0} words - MODERATE (target 800+ preferred)`
              : `${sd.wordCount || 0} words - adequate`;

      const _intLinkGrade =
        (sd.links?.internal || 0) < 3
          ? `${sd.links?.internal || 0} internal links - SPARSE (hinders AI crawler graph navigation)`
          : `${sd.links?.internal || 0} internal links - OK`;

      const _canonicalGrade = sd.canonical
        ? `present: ${sd.canonical}`
        : "MISSING - duplicate content not resolved, canonical tag absent";
      ...
      const icpPatterns =
        /\b(for|built for|designed for|ideal for|perfect for|made for|helps?)\s+(seo|marketing|content|product|dev|founder|startup|agenc|team|manager|executive|cmo|cto|marketer|writer|freelance|small business|enterprise|saas|ecommerce|b2b|b2c)/i;
      const hasIcp = icpPatterns.test(_bodyText);
      diagnosticFlags.push(
        `Target audience (ICP): ${hasIcp ? "DETECTED - page identifies a specific audience" : 'NOT DETECTED - no explicit target audience statement found. Pages that name their ICP (e.g. "built for SEO managers") convert and get cited more reliably'}`,
      );

      // Unexplained acronyms / jargon detection
      const acronymRe = /\b[A-Z]{2,6}\b/g;
      const foundAcronyms = new Set(
        (_bodyText.match(acronymRe) || []).filter(
          (a) =>
            ![
              "AI",
              "SEO",
      ...
      for (const acr of foundAcronyms) {
        ...
        const expandedRe = new RegExp(
          `${acr}\\s*[:(–-]|stands for|meaning|abbreviated|also known as`,
          "i",
        );
        if (!expandedRe.test(_bodyText)) unexplainedAcronyms.push(acr);
      }
      diagnosticFlags.push(
        `Unexplained acronyms: ${unexplainedAcronyms.length === 0 ? "none found - OK" : `${unexplainedAcronyms.length} found (${unexplainedAcronyms.slice(0, 8).join(", ")}) - consider expanding on first use for non-technical readers and AI extraction`}`,
      );

      // Benefit vs feature analysis (pricing / feature sections)
      const featureOnlyPatterns =
        /\b(api access|csv export|pdf export|white.?label|scheduled|triple.?check|competitor track|citation test|brand mention|force.?refresh)\b/i;
      const benefitPatterns =
        /\b(save time|increase|improve|boost|reduce|find out|discover|track .*(progress|changes)|share with|measure|monitor|automate|get (cited|found|visible))\b/i;
      const hasFeatureOnly = featureOnlyPatterns.test(_bodyText);
      const hasBenefitLanguage = benefitPatterns.test(_bodyText);
      diagnosticFlags.push(
        `Benefit orientation: ${hasBenefitLanguage ? "benefit language DETECTED" : "NO benefit language found"} | ${hasFeatureOnly && !hasBenefitLanguage ? "Features listed without outcome context - consider framing features as user benefits" : "OK"}`,
      );

      // Schema completeness (Review, Product, FAQ count)
      // These get updated later after schema extraction, but we flag the intent here
      diagnosticFlags.push(
        `Schema completeness intent: check for Review/AggregateRating, Product with Offer pricing, FAQPage with 8+ items, HowTo steps`,
      );
      ...
      const evidenceManifest: Record<string, string> = {
        ...
        ev_https: targetUrl.startsWith("https")
          ? "HTTPS enabled - OK"
          : "NOT HTTPS - CRITICAL",
        ...
        ev_icp: hasIcp
          ? "Target audience explicitly identified on page"
          : "No explicit target audience / ICP statement found on page",
        ev_jargon:
          unexplainedAcronyms.length === 0
            ? "No unexplained acronyms detected"
            : `${unexplainedAcronyms.length} unexplained acronym(s): ${unexplainedAcronyms.slice(0, 8).join(", ")}`,
        ...
        ev_llms_txt: (() => {
          const lt = (sd as any).llmsTxt;
          if (!lt?.fetched)
            return "llms.txt: not fetched (timeout or not attempted)";
          if (!lt.present)
            return "llms.txt: NOT PRESENT - add /llms.txt to guide AI crawlers on what to index and cite";
          const preview = (lt.raw || "").substring(0, 120).replace(/\n/g, " ");
          return `llms.txt: PRESENT - "${preview}${lt.raw?.length > 120 ? "…" : ""}"`;
        })(),
        ev_lcp_ms: _lcpGrade,
        ev_tldr: sd.hasTldr
          ? `TLDR/summary detected: "${(sd.tldrText || "").substring(0, 100)}"`
          : "No TLDR/summary block detected",
      ...
      const [citationStrengths, clarity, sentiment] = (await Promise.all([
        assessCitationStrength(externalDomains, sd.body || "", apiKey).catch(
          (e: any) => {
            console.warn(
              `[${requestId}] citation strength heuristic failed (non-fatal):`,
              e?.message,
            );
            return null;
          },
        ),
        assessEntityClarity(sd.body || "", apiKey).catch((e: any) => {
          console.warn(
            `[${requestId}] entity clarity heuristic failed (non-fatal):`,
            e?.message,
          );
          return null;
        }),
        analyzeReviewSentiment(sd.body || "", apiKey).catch((e: any) => {
          console.warn(
            `[${requestId}] review sentiment heuristic failed (non-fatal):`,
            e?.message,
          );
          return null;
        }),
      ...
      if (clarity) {
        ...
        if (clarity.excerpt)
          domainIntelligence.entity_clarity_excerpt = clarity.excerpt;
      }
      if (sentiment) {
      ...
      const technicalSignals = {
        response_time_ms: Number(
          sd.pageLoadMs || sd.lcpMs || Date.now() - startTime,
        ),
        status_code: 200,
        content_length: (sd.html || "").length,
        image_count: sd.images || 0,
      ...
      try {
        ...
        console.warn(
          `[${requestId}] Schema quality scoring failed (non-fatal):`,
          schemaScoreErr?.message,
        );
      }
      ...
      const questionReFaq =
        /\?|^(who|what|when|where|why|how|is|are|does|do|can|should|which)\b/i;
      const questionH3Count = (sd.headings?.h3 || []).filter((t: string) =>
        questionReFaq.test(t),
      ).length;
      ...
      const headingSignalCount =
        (sd.headings?.h1?.length || 0) + (sd.headings?.h2?.length || 0);
      const hasStructuredSignals =
        schemaMarkup.json_ld_count > 0 || schemaMarkup.schema_types.length > 0;
      const hasMetaSignals =
        !!sd.title || !!sd.meta?.description || !!sd.canonical;
      const hasLinkSignals =
        (sd.links?.internal || 0) + (sd.links?.external || 0) > 0;
      const hasRenderableSignals =
        hasStructuredSignals ||
        hasMetaSignals ||
        hasLinkSignals ||
        headingSignalCount > 0;
      const shouldReturnThinFallback =
        bodyWordCount < 30 && !hasRenderableSignals;
      const goalAlignment = computeGoalAlignment(findabilityGoals, sd);
      const mockDataScan = computeMockDataScan(sd, scanMockData);
      const scoreAdjustmentTotal =
        goalAlignment.score_adjustment + mockDataScan.score_adjustment;
      ...
      if (shouldReturnThinFallback) {
        ...
        console.warn(
          `[${requestId}] Extremely thin content detected (${bodyWordCount} words, no crawl signals) - building scrape-only result`,
        );
        ...
        const thinSeoDiagnostics = computeSeoDiagnostics(
          sd,
          schemaMarkup,
          technicalSignals,
          "url",
        ...
        const thinResult = attachTruthSignals({
          ...
          ai_platform_scores: derivePlatformScores(
            thinFinalScore,
            sd,
            schemaMarkup,
            targetUrl,
          ...
          analysis_integrity: buildAnalysisIntegrity({
            ...
            warnings: [
              `Only ${bodyWordCount} crawl-accessible words available`,
            ],
          }),
        ...
        try {
          ...
          if (userId) {
            ...
            settleReferralCreditsIfEligible(userId).catch((err: any) => {
              console.error(
                `[${requestId}] Referral settlement after thin audit failed:`,
                err?.message || err,
              );
            });
          ...
          console.error(
            `[${requestId}] THIN-AUDIT PERSIST FAILED:`,
            thinPersistErr?.message,
          );
        }
        ...
        const thinPayload = {
          ...thinResult,
          request_id: requestId,
          text_summary: generateTextSummary(
            thinResult as any,
            getTextSummaryDepth(normalizedRequestTier as CanonicalTier),
          ),
        };
        return res.json(
          applyTierResultStripping(thinPayload, normalizedRequestTier),
        );
      }
      ...
      let cryptoScanResult: Awaited<ReturnType<typeof runCryptoScan>> | null =
        null;
      let privateExposureScanResult: Awaited<
        ReturnType<typeof runPrivateExposureScan>
      > | null = null;
      let threatIntelResult: Awaited<ReturnType<typeof assessUrlRisk>> | null =
        null;
      ...
      try {
        ...
        const [cryptoRes, privateExposureRes, threatRes] = await Promise.all([
          withTimeout(
            ...
            console.warn(
              `[${requestId}] Crypto scan skipped (non-fatal):`,
              e.message,
            );
            return null;
          }),
          ...
          withTimeout(
            ...
            console.warn(
              `[${requestId}] Threat intel skipped (non-fatal):`,
              e.message,
            );
            return null;
          }),
        ...
        console.warn(
          `[${requestId}] Security scans skipped (non-fatal):`,
          parallelErr.message,
        );
      }
      ...
      if (isTripleCheck && sd.ocrData && sd.ocrData.images.length > 0) {
        try {
          ...
          const scrapeCoverage =
            crossCheck?.scrape_coverage_ratio ?? coverage.overlapRatio;
          const pythonEvidenceQuality = crossCheck?.evidence_quality ?? 0;
          const usePythonPayload =
            scrapeCoverage < 0.6 && pythonEvidenceQuality >= 85;

          ocrValidationResult = { ocrData, crossCheck, usePythonPayload };

          console.log(
            `[${requestId}] OCR validation: ${ocrData.images.length} images, coverage=${(scrapeCoverage * 100).toFixed(1)}%, python_quality=${pythonEvidenceQuality}, fallback=${usePythonPayload ? "PYTHON" : "SCRAPE"}`,
          );
        } catch (ocrValErr: any) {
          console.warn(
            `[${requestId}] OCR validation failed (non-fatal):`,
            ocrValErr?.message,
          );
        }
      }
      ...
      evidenceManifest["ev_schema"] =
        schemaMarkup.json_ld_count === 0
      ...
      evidenceManifest["ev_schema_completeness"] =
        schemaGaps.length === 0
      ...
      diagnosticFlags.push(
        `Schema completeness: ${evidenceManifest["ev_schema_completeness"]}`,
      );
      ...
      if (ocrValidationResult.usePythonPayload && sd.ocrData) {
        ...
        console.log(
          `[${requestId}] Dynamic OCR fallback ACTIVE: body augmented with ${sd.ocrData.totalOcrWords} OCR words`,
        );
      }
      ...
      try {
        ...
        console.warn(
          `[${requestId}] LLM readability validation failed (non-fatal):`,
          llmValErr?.message,
        );
      }

      // ── Compute evidence-based scoring bounds ──
      const evidenceBounds = computeEvidenceScores(
        sd,
        schemaMarkup,
        targetUrl,
        llmReadabilityResult,
      );
      const boundsBlock = REQUIRED_CATEGORIES.map(({ label }) => {
      ...
      try {
        const platformSignals = extractPlatformSignals(
          sd,
          schemaMarkup,
          targetUrl,
        );
        ...
        console.warn(
          `[${requestId}] Platform intelligence prompt block failed (non-fatal):`,
          platErr?.message,
        );
      }
...
`
      ...
      const remainingPipelineMs = Math.max(
        0,
        PIPELINE_DEADLINE_MS - elapsedBeforeAI,
      );
        ...
        new Promise<never>((_, reject) => {
          activeDeadlineTimer = setTimeout(
            () =>
              reject(
                new Error(
                  `Pipeline deadline (${label}) - ${budgetMs}ms budget exhausted`,
                ),
              ),
            budgetMs,
          );
      ...
      try {
        ...
        for (let pi = 0; pi < providers.length; pi++) {
          ...
          if (providerBudgetMs < MIN_FALLBACK_BUDGET_MS && pi > 0) {
            console.warn(
              `[${requestId}] AI1 fallback budget exhausted (${providerBudgetMs}ms < ${MIN_FALLBACK_BUDGET_MS}ms) after ${pi} providers - stopping`,
            );
            break;
          }
          ...
          try {
            ai1Raw = await Promise.race([
              callAIProvider({
                ...
                opts: {
                  max_tokens: isscorefixTier ? 5000 : 3000,
                  temperature: 0.1,
                  timeoutMs: effectiveBudgetMs,
                },
              }),
            ...
            if (ai1Raw) {
              ...
              if (parsed.ok && isUsableAiAnalysisPayload(parsed.value)) {
                ...
                lastErr = new Error(
                  `AI1 ${prov.model} payload incomplete (missing summary/categories/recommendations)`,
                );
              } else {
                lastErr = new Error(
                  `AI1 ${prov.model} JSON parse failed: ${(parsed as any).error}`,
                );
              }
            } else {
            ...
            if (
              errMsg.includes("pipeline deadline") ||
              errMsg.includes("budget exhausted") ||
          ...
          if (pi < providers.length - 1) {
            console.warn(
              `[${requestId}] AI1 provider ${prov.model} failed, trying fallback ${providers[pi + 1].model}...`,
            );
        ...
        const isPipelineDeadline =
          msg.includes("pipeline deadline") ||
        ...
        console.error(
          `[${requestId}] *** AI1 FAILED - falling back to deterministic ***`,
          {
            reason: isPipelineDeadline
              ? "pipeline-deadline"
              : "provider-failure",
        ...
        aiAnalysis = buildDeterministicAnalysis(
          sd,
          schemaMarkup,
          targetUrl,
          evidenceBounds,
      ...
      if (
        aiFallbackWarning &&
        !scrapeWarning &&
        (aiAnalysis?.visibility_score ?? 0) === 0
      ) {
      ...
      if (requireLiveAi && aiFallbackWarning) {
        if (!STRICT_LIVE_SOFT_FAIL_ENABLED) {
          return res.status(503).json({
            error:
              "Strict live mode rejected deterministic fallback output. Retry when AI providers are healthy for full live pipeline execution.",
        ...
        scrapeWarning = aiFallbackWarning;
        console.warn(
          `[${requestId}] Strict live mode soft-fail: accepted deterministic fallback due to provider instability`,
        );
      }
      ...
      if (isTripleCheck && !aiFallbackWarning) {
        ...
        const tcRemainingMs = Math.max(
          0,
          PIPELINE_DEADLINE_MS - (Date.now() - startTime),
        );
        ...
        try {
          ...
          if (ai2Raw) {
            ...
            if (ai2Parsed.ok) {
              ...
              const clampedAdj =
                typeof adj === "number"
                  ? Math.max(-15, Math.min(10, Math.round(adj)))
                  : 0;
              const ai2Critique =
                typeof ai2Parsed.value.critique === "string"
                  ? ai2Parsed.value.critique
                  : "";
              const ai2MissedIssues = Array.isArray(
              ...
              const filteredAi2ExtraRecommendations =
                ai2ExtraRecommendations.filter(
                  (recommendation: string) =>
                    detectPeerCritiqueEvidenceConflicts(
                      recommendation,
                      peerCritiqueFacts,
                    ).length === 0,
                );
              const penaltyGate = evaluateAi2PenaltyGate(peerCritiqueFacts);
              const gateNeutralizedNegative =
                clampedAdj < 0 && !penaltyGate.allowNegativePenalty;
              const conflictNeutralizedNegative =
                clampedAdj < 0 && critiqueConflicts.length > 0;
              const ai2PenaltyNeutralized =
                gateNeutralizedNegative || conflictNeutralizedNegative;

              const ai2PenaltyReasons: string[] = [];
              if (conflictNeutralizedNegative) {
                ai2PenaltyReasons.push(
                  `Conflict with crawl evidence: ${critiqueConflicts.join("; ")}`,
                );
              }
              ...
              tripleCheckResult.ai2_adjustment = ai2PenaltyNeutralized
                ? 0
                : clampedAdj;
              tripleCheckResult.ai2_critique =
                critiqueConflicts.length > 0
              ...
              if (
                filteredAi2ExtraRecommendations.length !==
                ai2ExtraRecommendations.length
              ) {
                console.warn(
              ...
              if (Array.isArray(ai2Parsed.value.category_overrides)) {
                for (const override of ai2Parsed.value.category_overrides) {
                  if (
                    !override?.label ||
                    typeof override.adjusted_score !== "number"
                  )
                    continue;
                  if (DETERMINISTIC_CATEGORY_LABELS.has(String(override.label)))
                    continue;
                  const catIdx = aiAnalysis.category_grades.findIndex(
                    (g: any) => g.label === override.label,
                  );
                  if (catIdx >= 0) {
              ...
              console.log(
                `[${requestId}] AI2 peer critique: adjustment=${clampedAdj}, critique="${tripleCheckResult.ai2_critique.substring(0, 80)}..."`,
              );
              ai2StageCompleted = true;
            } else {
              throw new Error(
                `AI2 JSON parse failed: ${(ai2Parsed as any).error}`,
              );
            }
          }
        } catch (ai2Err: any) {
          ...
          console.warn(
            `[${requestId}] AI2 peer critique failed (non-fatal): ${ai2Err.message}`,
          );
        }
        ...
        if (ai3BudgetMs > 4_000) {
          ...
          try {
            ...
            if (ai3Raw) {
              ...
              if (ai3Parsed.ok) {
                tripleCheckResult.ai3_validated =
                  ai3Parsed.value.validated === true;
                tripleCheckResult.ai3_verdict =
                  typeof ai3Parsed.value.verdict === "string"
                    ? ai3Parsed.value.verdict
                    : "";
                ...
                if (ai3Parsed.value.validated) {
                  ...
                  tripleCheckResult.confidence =
                    ai3Parsed.value.confidence || tripleCheckResult.confidence;
                } else {
                  ...
                  tripleCheckResult.confidence =
                    ai3Parsed.value.confidence || "medium";
                }
                console.log(
                  `[${requestId}] AI3 validation: validated=${tripleCheckResult.ai3_validated}, final=${tripleCheckResult.final_score}, verdict="${tripleCheckResult.ai3_verdict.substring(0, 80)}..."`,
                );
                ai3StageCompleted = true;
              } else {
                throw new Error(
                  `AI3 JSON parse failed: ${(ai3Parsed as any).error}`,
                );
              }
            }
          } catch (ai3Err: any) {
            ...
            console.warn(
              `[${requestId}] AI3 validation failed (non-fatal): ${ai3Err.message}`,
            );
            // Fall back to AI1 + AI2 adjustment
            tripleCheckResult.final_score = Math.max(
              0,
              Math.min(100, ai1Score + tripleCheckResult.ai2_adjustment),
            );
          ...
          if (strictTripleCheck) {
            throw new Error(
              `TRIPLE_CHECK_AI3_SKIPPED_INSUFFICIENT_BUDGET: ${ai3BudgetMs}ms`,
            );
          }
          console.warn(
            `[${requestId}] Skipping AI3 - insufficient budget (${ai3BudgetMs}ms)`,
          );
          tripleCheckResult.final_score = Math.max(
            0,
            Math.min(100, ai1Score + tripleCheckResult.ai2_adjustment),
        ...
        if (
          ...
          (!ai2StageCompleted ||
            !ai3StageCompleted ||
            !tripleCheckResult.enabled)
        ) {
          throw new Error(
            "TRIPLE_CHECK_INCOMPLETE: Required Signal/scorefix 3-model pipeline did not complete successfully",
          );
        }
      }

      const evidenceDerivedScore = computeWeightedCategoryScore(
        aiAnalysis.category_grades,
        evidenceBounds,
        ai1Score,
      );
      ...
      const rawFinalVisibilityScore = tripleCheckResult.enabled
        ? tripleCheckResult.final_score
        : ai1Score;
      ...
      if (
        tripleCheckResult.enabled &&
        typeof tripleCheckResult.ai2_adjustment === "number"
      ) {
        ...
        const boundedAi2Adjustment = Math.max(
          -6,
          Math.min(8, tripleCheckResult.ai2_adjustment),
        );
      ...
      const evidenceAnchoredScore = clampScore(
        evidenceDerivedScore + scoreAdjustmentTotal,
      );
      let finalVisibilityScore = clampScore(
        evidenceAnchoredScore + tripleCheckScoreDelta,
      );
      ...
      if (tripleCheckResult.enabled && isTripleCheck) {
        ...
        finalVisibilityScore = Math.max(
          minAllowed,
          Math.min(maxAllowed, finalVisibilityScore),
        );
      }
      ...
      if (elapsedTotal > PIPELINE_DEADLINE_MS && aiFallbackWarning) {
        ...
        emitProgress(requestId, "complete", 100);
        const deterministicFinalScore = clampScore(
          deterministic.visibility_score + scoreAdjustmentTotal,
        );
        ...
        const deterministicSeoDiagnostics = computeSeoDiagnostics(
          sd,
          schemaMarkup,
          technicalSignals,
          "url",
        ...
        const deterministicStrictRubric = buildStrictRubricSystem({
          ...
          evidenceFixPlan: {
            issues: deterministic.recommendations.map((rec, index) => ({
              ...
              validation_steps: [
                "Re-run deterministic audit and confirm recommendation is resolved.",
              ],
        ...
        AnalysisCacheService.set(cacheKey, partial as any).catch((e: any) =>
          console.error(
            `[${requestId}] Deterministic fallback cache write failed:`,
            e?.message,
          ),
        );
        ...
        return res.json(
          applyTierResultStripping(partialPayload, normalizedRequestTier),
        );
      } else if (elapsedTotal > PIPELINE_DEADLINE_MS) {
        console.warn(
          `[${requestId}] Soft deadline exceeded (${elapsedTotal}ms) but preserving successful AI analysis output`,
        );
      }
      ...
      const derivedPlatformScores = derivePlatformScores(
      ...
      const rawPlatformScores = {
        chatgpt:
          ...
          derivedPlatformScores.chatgpt,
        perplexity:
          aiAnalysis.ai_platform_scores?.perplexity ??
          derivedPlatformScores.perplexity,
        google_ai:
          aiAnalysis.ai_platform_scores?.google_ai ??
          derivedPlatformScores.google_ai,
        claude:
          aiAnalysis.ai_platform_scores?.claude ?? derivedPlatformScores.claude,
      };
      ...
      const ai1RecommendationsFiltered = (
        aiAnalysis.recommendations || []
      ).filter((rec: any) => {
        const conflicts = detectPeerCritiqueEvidenceConflicts(
          rec.title + " " + rec.description,
          ai1EvidenceConflictFacts,
        );
        if (conflicts.length > 0) {
          console.warn(
            `[${requestId}] Filtered AI1 recommendation conflicting with evidence: ${rec.title || rec.description || "unnamed"}`,
          );
          return false;
        }
        return true;
      ...
      const recommendations = ai1RecommendationsFiltered.map(
        (rec: any, idx: number) => ({
          id: `rec_${idx + 1}`,
          ...
          scorefix_category:
            typeof rec.scorefix_category === "string"
              ? rec.scorefix_category
              : undefined,
          consequenceStatement:
            typeof rec.consequenceStatement === "string"
              ? rec.consequenceStatement
              : undefined,
          estimatedVisibilityLoss:
            typeof rec.estimatedVisibilityLoss === "string"
              ? rec.estimatedVisibilityLoss
              : undefined,
          estimatedTimeMinutes:
            typeof rec.estimatedTimeMinutes === "number"
      ...
      const categoryGrades = (aiAnalysis.category_grades || []).map(
        (g: any) => ({
          ...
          strengths: Array.isArray(g.strengths)
            ...
            : [],
          improvements: Array.isArray(g.improvements)
            ? g.improvements.filter((s: any) => typeof s === "string")
            : [],
        }),
      );
      ...
      const contentHighlights = (aiAnalysis.content_highlights || []).map(
        (h: any) => ({
          ...
          source_id:
            typeof h.source_id === "string" && validEvidenceIds.has(h.source_id)
              ? h.source_id
              : undefined,
        }),
      );
      ...
      const verifiedRecommendationBundle = verifyRecommendationEvidence(
      ...
      if (
        requireLiveAi &&
        finalizedRecommendationBundle.summary.recommendations_with_evidence <= 0
      ) {
        return res.status(503).json({
          error:
            "Strict live mode rejected output with no evidence-linked recommendations. Retry for a fully grounded run.",
      ...
      const result = attachTruthSignals({
        ...
        crypto_intelligence: (() => {
          ...
          if (scan) {
            return {
              has_crypto_signals:
                scan.has_crypto_signals ||
                aiCrypto?.has_crypto_signals ||
                false,
              summary: scan.summary,
              detected_assets: [
                ...new Set([
                  ...(scan.detected_assets || []),
                  ...(aiCrypto?.detected_assets || []),
                ]),
              ],
              keywords: [
                ...new Set([
              ...
              wallet_addresses: [
                ...new Set([
                  ...(scan.wallet_addresses || []),
                  ...(aiCrypto?.wallet_addresses || []),
                ]),
              ],
        ...
        triple_check_summary: {
          ...
          ai2_penalty_reasons:
            tripleCheckResult.ai2_penalty_reasons.length > 0
          ...
          ai2_extra_recommendations:
            tripleCheckResult.ai2_extra_recommendations.length > 0
              ? tripleCheckResult.ai2_extra_recommendations
              : undefined,
          final_score: tripleCheckResult.enabled
            ? tripleCheckResult.final_score
            : undefined,
          ai3_recommended_score: tripleCheckResult.enabled
            ? tripleCheckResult.ai3_recommended_score
            : undefined,
        },
      ...
      try {
        ...
        console.error(
          `[${requestId}] AUDIT PERSIST FAILED - audit will not be shareable:`,
          preInsertErr?.message,
          preInsertErr?.stack?.split("\n").slice(0, 3).join(" | "),
        );
      }
      ...
      try {
        const hintUser = (req as any).user as
          | import("./models/User.js").User
          | undefined;
        if (hintUser?.id) {
          ...
          let analyzedDomain = "";
          try {
            analyzedDomain = new URL(targetUrl).hostname
              .replace(/^www\./, "")
              .toLowerCase();
          } catch {
            /* skip */
          ...
          if (userWebsite) {
            try {
              const candidate = /^https?:\/\//i.test(userWebsite)
                ? userWebsite
                : `https://${userWebsite}`;
              userDomain = new URL(candidate).hostname
                .replace(/^www\./, "")
                .toLowerCase();
            } catch {
              /* skip */
          ...
          const userEntityFp = await getEntityFingerprint(hintUser.id).catch(
            () => null,
          );
          ...
          const isSelf =
            (userDomain && analyzedDomain && userDomain === analyzedDomain) ||
          ...
          if (!isSelf && analyzedDomain && profileComplete) {
            ...
            const topicalKeywords: string[] = Array.isArray(
              (result as any).topical_keywords,
            )
              ? (result as any).topical_keywords
              : [];
            const brandEntities: string[] = Array.isArray(
              (result as any).brand_entities,
            )
              ? (result as any).brand_entities
              : [];
            ...
            try {
              ...
              if (
                userAudit[0]?.keywords &&
                Array.isArray(userAudit[0].keywords)
              ) {
                userKeywords = userAudit[0].keywords.map((k: unknown) =>
            ...
            const analyzedKeywordsLower = topicalKeywords.map((k) =>
              k.toLowerCase(),
            );
            const sharedKeywords = userKeywords.filter((k) =>
              analyzedKeywordsLower.includes(k),
            );
            const validUserKeywords = userKeywords.filter(
              (k) => k && typeof k === "string",
            );
            const sharedBrands = brandEntities.filter((b) => {
              ...
              if (
                userBrandName &&
                (bLow === userBrandName ||
                  textMentionsBrand(bLow, userBrandName) ||
                  textMentionsBrand(userBrandName, bLow))
              ) {
                return false;
              }
              // Use word-boundary matching instead of substring includes()
              return validUserKeywords.some((uk) =>
                textMentionsBrand(uk, bLow),
              );
            });
            const sharedSignals = [
              ...new Set([...sharedKeywords, ...sharedBrands]),
            ].slice(0, 8);

            // Determine if potential competitor: at least 2 shared signals OR same niche keywords overlap >20%
            const overlapRatio =
              userKeywords.length > 0
                ? sharedKeywords.length / userKeywords.length
                : 0;
            const isPotentialCompetitor =
              sharedSignals.length >= 2 || overlapRatio >= 0.2;

            if (isPotentialCompetitor || !alreadyTracked) {
              const matchReasons: string[] = [];
              if (sharedKeywords.length >= 2)
                matchReasons.push(
                  `${sharedKeywords.length} shared topical keywords`,
                );
              if (overlapRatio >= 0.2)
                matchReasons.push(
                  `${Math.round(overlapRatio * 100)}% keyword overlap with your site`,
                );
              if (sharedBrands.length > 0)
                matchReasons.push(`Shared brand/entity signals`);
        ...
        console.warn(
          `[${requestId}] Competitor hint detection failed (non-fatal):`,
          hintErr?.message,
        );
      }

      // ── Platform benchmark comparison ──────────────────────────────────────
      let platformBenchmark: {
        global_avg: number;
        total_audits: number;
        percentile: number;
      } | null = null;
      try {
        ...
        console.warn(
          `[${requestId}] Platform benchmark query failed (non-fatal):`,
          benchErr?.message,
        );
      }

      // ── Text summary narrative (tier-gated) ──────────────────────────────────
      const textSummaryDepth = getTextSummaryDepth(
        normalizedRequestTier as CanonicalTier,
      );
      ...
      const fullPayload = {
        ...result,
        request_id: requestId,
        ...(dbAuditId ? { audit_id: dbAuditId } : {}),
        ...(!dbAuditId ? { _persist_warning: "audit_not_saved" } : {}),
        ...(competitorHint ? { competitor_hint: competitorHint } : {}),
        ...(platformBenchmark ? { platform_benchmark: platformBenchmark } : {}),
        text_summary: textSummary,
      };
      const responsePayload = applyTierResultStripping(
        fullPayload,
        normalizedRequestTier,
      );
      ...
      if (!result.scrape_warning) {
        AnalysisCacheService.set(cacheKey, result as any).catch((e: any) =>
      ...
      (async () => {
        try {
          ...
          if (userId && dbAuditId) {
            ...
            settleReferralCreditsIfEligible(userId).catch((err: any) => {
              console.error(
                `[${requestId}] Referral settlement after audit failed:`,
                err?.message || err,
              );
            });
            ...
            if (auditId) {
              ...
              if (workspaceId) {
                dispatchWebhooks(userId, workspaceId, "audit.completed", {
                  ...
                  console.warn(
                    `[${requestId}] Webhook dispatch failed:`,
                    e?.message,
                  ),
                );
                ...
                dispatchAuditReportDeliveries({
                  ...
                  console.warn(
                    `[${requestId}] Report delivery failed:`,
                    e?.message,
                  ),
                );
              }
          ...
          if (canRank) {
            ...
            const niche =
              ...
              "SaaS tools and online software";
            if (brandName) {
              backgroundWithTimeout(
                `niche-ranking:${requestId}`,
                runNicheRanking({
                  targetUrl,
                  brandName,
                  niche,
          ...
          if (dbAuditId && scraped) {
            backgroundWithTimeout(
              ...
              runDeterministicAuditLayer(dbAuditId, userId, scraped).then(
                async (deterministicResult) => {
                  if (deterministicResult) {
                    const additions =
                      buildDeterministicResponseAdditions(deterministicResult);
                    await attachDeterministicToAudit(
                      dbAuditId!,
                      additions,
                    ).catch((e: any) => {
                      console.warn(
                        `[${requestId}] Deterministic attach failed (non-fatal):`,
                        e?.message,
                      );
                    });
                  }
          ...
          if (dbAuditId && scraped) {
            backgroundWithTimeout(
              ...
              (async () => {
                ...
                const enriched = enrichEvidenceFromAnalysis(
                  ssfrEvidence,
                  result as any,
                );
                const ruleResults = evaluateSSFRRules(enriched);
                const fixpacks = generateFixpacks(
                  ruleResults,
                  enriched,
                  targetUrl,
                );
                const pool = getPool();
                await persistSSFRResults(
                  pool,
                  dbAuditId!,
                  enriched,
                  ruleResults,
                ...
                console.log(
                  `[${requestId}] SSFR pipeline complete: ${summary.passed_rules}/${summary.total_rules} rules passed, ${fixpacks.length} fixpacks, cap=${summary.effective_score_cap ?? "none"}`,
                );
              })(),
            );
          }
          ...
          if (dbAuditId && scraped) {
            ...
            backgroundWithTimeout(
              ...
              (async () => {
                ...
                const scrapedText =
                  scraped.data?.body || scraped.data?.html || "";
                ...
                const deepResult = await analyzeContentDeep({
                  ...
                  json_ld_blocks:
                    scraped.data?.structuredData?.raw?.map((block: any) =>
                ...
                if (deepResult) {
                  ...
                  const ledgerResult = await recordEvidenceLedger({
                    audit_id: dbAuditId!,
                    url: targetUrl,
                    evidence_entries: evidenceEntries as Array<
                      Record<string, unknown>
                    >,
                  });
                  ...
                  console.log(
                    `[${requestId}] Python deep analysis complete: readability=${deepResult.readability?.reading_level}, entities=${deepResult.entities?.total_entity_count}, ledger_root=${ledgerResult?.root_hash?.slice(0, 12) ?? "none"}`,
                  );
                }
              })(),
            );
          ...
          console.error(
            `[${requestId}] Failed to store audit trail:`,
            auditErr.message,
          );
        }
      })();
      ...
      const status =
        Number(err?.status) ||
        (code === "INVALID_URL" ? 400 : isStrictTripleCheckFailure ? 503 : 500);
...
app.post(
  ...
  async (req: Request, res: Response) => {
    ...
    try {
      ...
      const userTier = ((req as any).user?.tier || "observer") as
        | CanonicalTier
        | LegacyTier;
      if (!meetsMinimumTier(userTier, "alignment")) {
        ...
        return res.status(403).json({
          error:
            "Document upload analysis requires Alignment, Signal, or Score Fix.",
          code: "TIER_INSUFFICIENT",
          requiredTier: "alignment",
          request_id: requestId,
        });
      ...
      if (validIncomingFiles.length === 0) {
        logMalformedPayload(req, ownerUserId, "Missing files payload", {
          route: "/api/analyze/upload",
        });
        return res.status(400).json({
          error:
            "Missing files payload. Provide fileName, mimeType, and content.",
      ...
      if (validIncomingFiles.length > maxFiles) {
        logInvalidUpload(
          req,
          ownerUserId,
          "upload batch exceeds tier file limit",
          {
            route: "/api/analyze/upload",
            tier: normalizedTier,
            maxFiles,
      ...
      for (const item of validIncomingFiles) {
        ...
        const encoding: UploadEncoding =
          item.encoding === "base64" ? "base64" : "utf8";

        if (!fileName || !mimeType || !content) {
          logMalformedPayload(
            req,
            ownerUserId,
            "Missing upload file metadata",
            { fileName, mimeType, route: "/api/analyze/upload" },
          );
          return res.status(400).json({
            error:
              "Each uploaded file requires fileName, mimeType, and content.",
            code: "INVALID_UPLOAD_PAYLOAD",
            request_id: requestId,
          });
        }
        ...
        if (!uploadKind) {
          ...
          return res.status(415).json({
            error:
              "Unsupported file type. Supported: docs and source files (.txt, .md, .html, .pdf, .docx, .png, .jpg, .jpeg, .webp, .js, .ts, .py, .php, .css, .json, etc.)",
        ...
        if (byteSize > maxBytesPerFile) {
          logInvalidUpload(
            req,
            ownerUserId,
            "upload file exceeds tier size limit",
            {
              fileName,
              mimeType,
              byteSize,
      ...
      if (totalByteSize > maxTotalBytes) {
        logInvalidUpload(
          req,
          ownerUserId,
          "combined upload exceeds request size limit",
          {
            totalByteSize,
            maxTotalBytes,
      ...
      const extractedByFile: Array<
        ParsedUploadFile & { extracted: UploadExtractedData }
      > = [];
      ...
      for (const parsedFile of parsedFiles) {
        try {
          ...
          return res.status(422).json({
            error:
              parseErr?.message ||
              `Failed to parse uploaded file ${parsedFile.fileName}`,
      ...
      const sd = mergeUploadData(
        ...
        parsedFiles.length === 1
          ? primaryFileName
          : `${parsedFiles.length} uploaded files`,
      );
      ...
      for (const file of extractedByFile) {
        ...
        schemaMarkup.has_organization_schema =
          schemaMarkup.has_organization_schema ||
          fileSchema.has_organization_schema;
        schemaMarkup.has_faq_schema =
          schemaMarkup.has_faq_schema || fileSchema.has_faq_schema;
        if (
          Array.isArray(fileSchema.schema_types) &&
          fileSchema.schema_types.length > 0
        ) {
          schemaMarkup.schema_types.push(...fileSchema.schema_types);
        }
      }

      schemaMarkup.schema_types = Array.from(
        new Set(
          schemaMarkup.schema_types
      ...
      const evidenceManifest: Record<string, string> = {
        ...
        up_file_ext:
          parsedFiles.length === 1
            ? getExtension(primaryFileName) || "(none)"
            : "mixed",
...
{"visibility_score":<0-100>,"ai_platform_scores":{"chatgpt":<0-100>,"perplexity":<0-100>,"google_ai":<0-100>,"claude":<0-100>},"summary":"<2-3 sentences>","key_takeaways":["<3-5 items>"],"topical_keywords":["<5-10>"],"brand_entities":["<entities>"],"primary_topics":["<3-5>"],"recommendations":[{"priority":"high|medium|low","category":"<cat>","title":"<short>","description":"<detail>","impact":"<impact>","difficulty":"easy|medium|hard","implementation":"<steps>","evidence_ids":["up_*"]}],"category_grades":[{"grade":"A|B|C|D|F","label":"<category>","score":<0-100>,"summary":"<1-2 sent>","strengths":[],"improvements":[]}],"content_highlights":[{"area":"heading|meta-tags|schema|content|technical|readability","found":"<quote>","status":"good|warning|critical|missing","note":"<why>","source_id":"up_*"}],"writing_audit":{"content_type":"blog|article|ebook|research|general","seo_title":"<improved SEO title>","hook":"<improved opening hook, max 2 sentences>","seo_title_score":<0-100>,"hook_score":<0-100>,"de_ai_score":<0-100>,"freshness_findings":["<issues or passes>"],"de_ai_findings":["<issues or passes>"],"verdict":"keep|refresh|rebuild|merge|kill","rubric_scores":{"content":<0-100>,"facts":<0-100>,"structure":<0-100>,"seo":<0-100>,"aeo":<0-100>,"business":<0-100>},"entity_clarity":{"name":"<subject name>","what":"<what it covers>","who":"<author/creator>","why":"<why it matters>","score":<0-100>},"information_gain_score":<0-100>,"citation_readiness_score":<0-100>,"fact_check_items":[{"claim":"<claim text>","status":"verified|unverified|disputed|missing_source","note":"<context>"}],"readability_level":"<e.g. Grade 8>","chunking_issues":["<paragraph or section description if >150 words without break>"],"rewrite":{"rewritten_text":"<polished rewrite, no em dashes>","diff":[{"type":"title|hook|clarity|tone|seo|freshness|de_ai|structure|fact","original":"<before>","revised":"<after>","reason":"<why>"}]}}}`;
      ...
      if (!parsed.ok)
        throw new Error((parsed as any).error || "AI JSON parse failed");
      ...
      const verifiedUploadRecommendationBundle = verifyRecommendationEvidence(
        dedupedUploadRecommendations,
        validUploadEvidenceIds,
      );
      ...
      const uploadDerivedPlatformScores = derivePlatformScores(
        uploadVisibilityScore,
        sd,
        schemaMarkup,
        syntheticUrl,
      );
      ...
      const uploadCitationParityAudit = buildCitationParityAudit({
        ...
        contentHighlights: Array.isArray(aiAnalysis.content_highlights)
          ? aiAnalysis.content_highlights
          : [],
        evidenceManifest,
        schemaMarkup,
        contentAnalysis: {
        ...
        domainIntelligence: {
          entity_clarity_score: 0,
          primary_topics: Array.isArray(aiAnalysis.primary_topics)
            ? aiAnalysis.primary_topics
            : [],
          citation_domains: [],
        },
        technicalSignals: {
        ...
        topicalKeywords: Array.isArray(aiAnalysis.topical_keywords)
          ? aiAnalysis.topical_keywords
          : [],
        brandEntities: Array.isArray(aiAnalysis.brand_entities)
          ? aiAnalysis.brand_entities
          : [],
        recommendationEvidenceSummary:
          verifiedUploadRecommendationBundle.summary,
      });
      ...
      const result = attachTruthSignals({
        ...
        recommendations: verifiedUploadRecommendationBundle.recommendations,
        category_grades: Array.isArray(aiAnalysis.category_grades)
          ? aiAnalysis.category_grades
          : [],
        content_highlights: Array.isArray(aiAnalysis.content_highlights)
          ? aiAnalysis.content_highlights
          : [],
        schema_markup: schemaMarkup,
        content_analysis: {
        ...
        summary:
          aiAnalysis.summary ||
          `AI visibility analysis for uploaded ${parsedFiles.length === 1 ? `file ${primaryFileName}` : `${parsedFiles.length} files`}`,
        key_takeaways: Array.isArray(aiAnalysis.key_takeaways)
          ? aiAnalysis.key_takeaways
          : [],
        topical_keywords: Array.isArray(aiAnalysis.topical_keywords)
          ? aiAnalysis.topical_keywords
          : [],
        keyword_intelligence: Array.isArray(aiAnalysis.keyword_intelligence)
          ? aiAnalysis.keyword_intelligence
          : [],
        brand_entities: Array.isArray(aiAnalysis.brand_entities)
          ? aiAnalysis.brand_entities
          : [],
        url: syntheticUrl,
        analyzed_at: new Date().toISOString(),
      ...
      emitProgress(requestId, "complete", 100);
      const observerCheckTier = ((req as any).user?.tier ||
        "observer") as CanonicalTier;
      const uploadTextSummary = generateTextSummary(
        result as any,
        getTextSummaryDepth(observerCheckTier),
      );
      ...
      try {
        ...
        if (userId) {
          ...
          settleReferralCreditsIfEligible(userId).catch((err: any) => {
            console.error(
              `[${requestId}] Referral settlement after upload audit failed:`,
              err?.message || err,
            );
          });
        ...
        console.error(
          `[${requestId}] Failed to persist upload audit:`,
          dbErr?.message || dbErr,
        );
      }
...
function requireAdminKey(req: Request, res: Response): boolean {
  if (!process.env.ADMIN_KEY) {
    res.status(503).json({
      error: "Admin endpoint disabled - ADMIN_KEY not configured",
      code: "ADMIN_DISABLED",
    });
    return false;
  }
...
app.post("/api/admin/set-tier", adminLimiter, async (req, res) => {
  ...
  if (!validTiers.includes(tier))
    return res
      .status(400)
      .json({ error: `tier must be one of: ${validTiers.join(", ")}` });

  try {
    const result = await getPool().query(
      `UPDATE users SET tier = $1, updated_at = NOW() WHERE email = $2 RETURNING id, email, tier`,
...
app.get("/api/admin/tier-audit", adminLimiter, async (req, res) => {
  ...
  try {
    ...
    const users = result.rows.map((u: any) => {
      ...
      else if (!hasSubscription && !hasPaid && !isTrialing)
        status = "FREELOADER_NO_PAYMENT";
      else status = "unknown";
    ...
    const freeloaders = users.filter((u: any) =>
      u.status.startsWith("FREELOADER"),
    );
    const summary = {
      ...
      trial_active: users.filter((u: any) => u.status === "trial_active")
        .length,
...
app.post("/api/admin/downgrade-freeloaders", adminLimiter, async (req, res) => {
  ...
  try {
    ...
    if (!dryRun && candidates.length > 0) {
      ...
      console.log(
        `[Admin] Downgraded ${ids.length} freeloader(s): ${candidates.map((u: any) => u.email).join(", ")}`,
      );
    }
...
app.post("/api/admin/newsletter/preview", adminLimiter, async (req, res) => {
  ...
  const tier = String(body.tier || "observer")
    .trim()
    .toLowerCase();
  const validTiers = ["observer", "starter", "alignment", "signal", "scorefix"];
  ...
  const auditCount = Number.isFinite(auditCountRaw)
    ? Math.max(0, Math.floor(auditCountRaw))
    : 7;
  ...
  const editionLabel =
    String(body.editionLabel || "").trim() || getCurrentNewsletterEditionKey();
  const sendTest =
    body.sendTest === true ||
    String(body.sendTest || "").toLowerCase() === "true";
  ...
  try {
    ...
    return res
      .status(500)
      .json({ error: e?.message || "Newsletter preview failed" });
  }
});
...
app.get("/api/admin/newsletter/settings", adminLimiter, async (req, res) => {
  ...
  try {
    ...
    return res
      .status(500)
      .json({ error: e?.message || "Failed to read newsletter settings" });
  }
});
...
app.post("/api/admin/newsletter/settings", adminLimiter, async (req, res) => {
  ...
  try {
    const next = await upsertNewsletterDispatchSettings({
      automationEnabled:
        typeof body.automationEnabled === "boolean"
          ...
          : undefined,
      batchSize: Number.isFinite(Number(body.batchSize))
    ...
    return res
      .status(500)
      .json({ error: e?.message || "Failed to update newsletter settings" });
  }
});
...
app.post("/api/admin/newsletter/dispatch", adminLimiter, async (req, res) => {
  ...
  const options = {
    ...
    forceResend:
      ...
      String(body.forceResend || "").toLowerCase() === "true",
    batchSize: Number.isFinite(Number(body.batchSize))
      ? Number(body.batchSize)
      : undefined,
    delayMs: Number.isFinite(Number(body.delayMs))
  ...
  try {
    ...
    return res
      .status(500)
      .json({ error: e?.message || "Newsletter dispatch failed" });
  }
});
...
app.get("/api/admin/newsletter/editions", adminLimiter, async (req, res) => {
  ...
  try {
    ...
    return res
      .status(500)
      .json({ error: e?.message || "Failed to list newsletter editions" });
  }
});
...
app.post("/api/admin/newsletter/editions", adminLimiter, async (req, res) => {
  ...
  try {
    ...
    return res
      .status(500)
      .json({ error: e?.message || "Failed to create newsletter edition" });
  }
});
...
app.post("/api/admin/broadcast/preview", adminLimiter, async (req, res) => {
  ...
  if (!subject || !headline || !broadcastBody) {
    return res
      .status(400)
      .json({ error: "subject, headline, and body are required" });
  }
  ...
  try {
    ...
    return res
      .status(500)
      .json({ error: e?.message || "Broadcast preview failed" });
  }
});
...
app.post("/api/admin/broadcast/send", adminLimiter, async (req, res) => {
  ...
  const ctaUrl = String(body.ctaUrl || "").trim() || undefined;
  const dryRun =
    body.dryRun === true || String(body.dryRun || "").toLowerCase() === "true";
  const tierFilter: string[] = Array.isArray(body.tierFilter)
    ? body.tierFilter
    : [];
  ...
  try {
    ...
    if (tierFilter.length > 0) {
      ...
      whereClauses.push(
        `COALESCE(u.tier, 'observer') = ANY($${params.length}::text[])`,
      );
    }
    ...
    for (const row of users) {
      try {
        ...
        failed += 1;
        console.error(
          `[Broadcast] Failed for ${row.email}:`,
          err?.message || err,
        );
      }
    }
    ...
    return res
      .status(500)
      .json({ error: e?.message || "Broadcast send failed" });
  }
});
  ...
  try {
    ...
    return res.json({
      ...
      revenue: {
        ...
        last_30d_usd: +(
          Number(revenue.revenue_last_30d_cents || 0) / 100
        ).toFixed(2),
        mrr_estimate_usd: +(
          Number(revenue.revenue_last_30d_cents || 0) / 100
        ).toFixed(2),
...
app.get("/api/admin/payments", adminLimiter, async (req, res) => {
  ...
  const limit = Math.min(
    100,
    Math.max(1, Number.isFinite(limitRaw) ? Math.floor(limitRaw) : 25),
  );
  ...
  try {
    ...
    return res.json({
      ...
      summary: {
        ...
        activeSubscriptionCount: Number(
          summaryRow.active_subscription_count || 0,
        ),
    ...
    return res
      .status(500)
      .json({ error: e?.message || "Failed to fetch payment ledger" });
  }
});
...
app.get("/api/admin/validate-keys", adminLimiter, async (req, res) => {
  ...
  const results: Record<
    string,
    { configured: boolean; valid?: boolean; error?: string; latencyMs?: number }
  > = {};
  ...
  if (dsKey) {
    ...
    try {
      ...
      if (!r.ok)
        results.deepseek.error = body?.error?.message || `HTTP ${r.status}`;
    } catch (e: any) {
...
app.use((err: any, _req: any, res: any, _next: any) => {
  ...
  if (res.headersSent) {
    console.warn(
      "[GlobalErrorHandler] Headers already sent, error not delivered to client:",
      err?.message || err,
    );
    return;
  }
...
(async function start() {
  ...
  try {
    ...
    if (databaseReady) {
      ...
      console.warn(
        ...
        dbStatus.lastError || "health check failed",
      );
    }
  } catch (err: any) {
  ...
  const analyzeInternally = async (
    ...
    try {
      ...
      const providers =
        tier === "signal" || tier === "scorefix"
          ? PROVIDERS.slice(0, 3)
          : tier === "alignment" || tier === "starter"
            ? PROVIDERS.slice(0, 2)
      ...
      settleReferralCreditsIfEligible(userId).catch((err: any) => {
        console.error(
          `[rescan-internal] Referral settlement failed for ${userId}:`,
          err?.message || err,
        );
      });
      ...
      onCompleted: async ({
        ...
        try {
          ...
          if (recentAudits.length === 2) {
            ...
            if (drop >= 5) {
              const user = await getUserById(userId);
              if (user?.email) {
                const { sendScoreDropAlert } =
                  await import("./services/trendAlertEmails.js");
          ...
          console.warn(
            `[rescan] Trend alert failed (non-fatal):`,
            trendErr?.message,
          );
        }
        ...
        if (auditId) {
          try {
            ...
            console.warn(
              `[rescan] timeline record failed (non-fatal):`,
              tlErr?.message,
            );
          }
        }
