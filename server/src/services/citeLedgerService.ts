 ...
 *   2. Enqueue job
 *   3. Mark processing
 *   4. Anchor cite ledger + evidence to entity
 *   5. Record drift score
 *   6. Mark completed
 *
...
export async function runCiteLedgerPipeline(opts: {
    ...
    try {
        ...
        // 2. Enqueue + mark processing
        jobId = await enqueueJob(opts.url, entityId);
        await updateJobStatus(jobId, 'processing');

        // 3. Anchor existing cite_ledger + evidence rows to this entity
        await anchorCiteLedgerToEntity(opts.auditRunId, entityId);
        await anchorEvidenceToEntity(opts.auditRunId, entityId);

        // 4. Record drift score snapshot
        await recordDriftScore(
            entityId,
            opts.score,
            opts.evidenceCount,
            opts.scoreSource,
        ...
        console.log(
            `[cite-ledger] Pipeline complete: entity=${entityId}, job=${jobId}, score=${opts.score}, evidence=${opts.evidenceCount}`,
        );
    } catch (err: unknown) {
        const msg = err instanceof Error ? err.message : String(err);
        console.error(`[cite-ledger] Pipeline error: ${msg}`);
        if (jobId) {
            await updateJobStatus(jobId, 'failed', msg).catch(() => { });
        }
    }
}
