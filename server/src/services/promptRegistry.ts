...
{"visibility_score":<0-100>,"ai_platform_scores":{"chatgpt":<0-100>,"perplexity":<0-100>,"google_ai":<0-100>,"claude":<0-100>},"summary":"<2-3 sentences>","key_takeaways":["<3-5 items>"],"topical_keywords":["<5-10>"],"keyword_intelligence":[{"keyword":"<from topical_keywords>","intent":"informational|commercial|navigational|transactional","volume_tier":"low|medium|high|very_high","competition":"low|medium|high","opportunity":<0-100>,"trend":"rising|stable|declining"}],"brand_entities":["<found on page>"],"primary_topics":["<3-5>"],"faq_count":<int>,"category_grades":[{"grade":"A|B|C|D|F","label":"<category>","score":<0-100>,"summary":"<1-2 sent>","strengths":[],"improvements":[]}],"content_highlights":[{"area":"heading|meta|schema|content|technical|readability","found":"<quote actual text from page>","status":"good|warning|critical|missing","note":"<specific why with measured value>","source_id":"<ev_* ID>"}],"recommendations":[{"priority":"high|medium|low","category":"<category_grade label>","scorefix_category":"<one of the 9 above>","title":"<short specific title>","description":"Measured: <actual value>. Target: <recommended value>. <explanation>","impact":"<specific projected impact with estimated score lift>","difficulty":"easy|medium|hard","implementation":"<specific steps with code/markup example>","evidence_ids":["<ev_*"],"consequenceStatement":"<real business cost in survival language>","estimatedVisibilityLoss":"<X-Y%>","estimatedTimeMinutes":<5|15|30|60|120>}],"crypto_intelligence":{"has_crypto_signals":false,"summary":"<or detected>","detected_assets":[],"keywords":[],"wallet_addresses":[],"sentiment":"neutral","risk_notes":[],"chain_networks":[]},"cite_ledger":{"cite_ledger_score":<0-100 — must equal visibility_score>,"data_confidence":"high|medium|low","dimensions":[{"name":"Schema & Structured Data","score":<0-100>,"weight":20,"status":"pass|partial|fail","evidence":["<real page element or API signal>"],"issues":["<specific missing or broken element>"],"impact":"<effect on citation probability>","fix":"<specific implementable action>"},{"name":"Content Depth","score":<0-100>,"weight":18,"status":"pass|partial|fail","evidence":["<real page element or API signal>"],"issues":["<specific missing or broken element>"],"impact":"<effect on citation probability>","fix":"<specific implementable action>"},{"name":"Technical Trust","score":<0-100>,"weight":15,"status":"pass|partial|fail","evidence":["<real page element or API signal>"],"issues":["<specific missing or broken element>"],"impact":"<effect on citation probability>","fix":"<specific implementable action>"},{"name":"Meta & Open Graph","score":<0-100>,"weight":15,"status":"pass|partial|fail","evidence":["<real page element or API signal>"],"issues":["<specific missing or broken element>"],"impact":"<effect on citation probability>","fix":"<specific implementable action>"},{"name":"AI Readability","score":<0-100>,"weight":12,"status":"pass|partial|fail","evidence":["<real page element or API signal>"],"issues":["<specific missing or broken element>"],"impact":"<effect on citation probability>","fix":"<specific implementable action>"},{"name":"Heading Structure","score":<0-100>,"weight":10,"status":"pass|partial|fail","evidence":["<real page element or API signal>"],"issues":["<specific missing or broken element>"],"impact":"<effect on citation probability>","fix":"<specific implementable action>"},{"name":"Security & Trust","score":<0-100>,"weight":10,"status":"pass|partial|fail","evidence":["<real page element or API signal>"],"issues":["<specific missing or broken element>"],"impact":"<effect on citation probability>","fix":"<specific implementable action>"}],"brag_entries":[{"id":"CL-<deterministic 4-digit number>","signal":"<signal name>","status":"pass|fail","evidence":["<real page element, API signal, or confirmed absence>"],"impact":"<citation probability effect>","fix":"<specific implementable action>"}],"hard_blockers":[{"type":"<blocker type>","description":"<description>","impact":"<score cap and citation effect>"}],"verdict":"<max 3 sentences: (1) why AI would or would not cite the page, (2) what is missing at a structural level, (3) what kind of page this appears to be>","top_fixes":[{"action":"<specific implementable action — not vague>","why":"<maps to which BRAG failure>","expected_impact":"high|medium|low"}],"citation_probability":{"score":<0-100>,"confidence":"low|medium|high","reasoning":"<1-2 sentences grounded in dimension evidence>"}}}
...
CITE LEDGER rules: cite_ledger_score must equal visibility_score. All 7 dimensions must be present. brag_entries must cover every failing dimension. top_fixes must be exactly 5, ordered high→low impact. hard_blockers must be empty array if none apply. If fewer than 3 API sources provided set data_confidence to "low".`;
}
