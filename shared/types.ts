...
export interface RagPipelineSourceCandidate {
  url: string;
  title: string;
  rank: number;
  pre_decision: string;
...
export interface RagPipelineTrustScoreComponent {
  search?: number;
  content?: number;
  structure?: number;
  risk?: number;
...
export interface RagPipelineTrustItem {
  url: string;
  trust_score: number;
  decision: 'accept' | 'partial' | 'reject';
  reason_flags: string[];
  components?: RagPipelineTrustScoreComponent;
}

export interface RagPipelinePayload {
  scan_id?: string;
  execution_class?: 'full' | 'partial' | 'no_results';
  source_candidates?: RagPipelineSourceCandidate[];
  discarded_sources?: Array<Record<string, unknown>>;
  trust_sources?: {
    items: RagPipelineTrustItem[];
    summary: {
      accepted: number;
      partial: number;
      rejected: number;
...
export interface AnalysisResponse {
  ...
  request_id?: string;
  timeline_scan_id?: string;
  rag_pipeline?: RagPipelinePayload;
...
// Predicts how AI answer engines would respond to queries about this entity,
// tracking which entities they use, what's missing, and cross-model overlap.
// This produces the AI Visibility Probability (AVP) metric.

/** Per-model simulation output */
export interface ModelSimulationOutput {
  /** Whether this model execution produced a usable response */
  status: 'ok' | 'error';
  /** The answer text generated by this model for the query */
  answer: string;
  /** Entities identified in the model's answer */
  entities_used: string[];
  /** Target entities that were missing from the model's answer */
  missing_entities: string[];
  /**
   * 0–1 score: how far the model's answer deviates from the truth graph.
   * 0 = no distortion, 1 = complete hallucination / displacement.
   */
  distortion_score: number;
  /** Whether the primary entity was cited by name */
  entity_cited: boolean;
  /** Source URLs cited inline by the model (if any) */
  cited_sources: string[];
  /** Present only when the provider call failed or returned unusable output */
  error_reason?: string;
}

/** Full simulation result across all models for a single query */
export interface QuerySimulationResult {
  /** The query used to probe AI answer engines */
  query: string;
  /** ISO timestamp when simulation was run */
  simulated_at: string;
  /** Whether all, some, or none of the model calls succeeded */
  execution_status: 'complete' | 'partial' | 'failed';
  /** Model calls attempted for this query */
  attempted_models: number;
  /** Model calls that returned usable outputs */
  successful_models: number;
  /** Model calls that failed or returned unusable outputs */
  failed_models: number;
  /** Per-model outputs keyed by model label */
  models: Record<string, ModelSimulationOutput>;
  /**
   * Fraction of models (0–1) that mentioned the same top entities.
   * High overlap = consistent entity presence; low = fragmented.
   */
  cross_model_overlap: number;
  /**
   * AI Visibility Probability (AVP) — 0–1.
   * Fraction of models that cited the primary entity, weighted by distortion.
   * This is the core metric: what is the probability that an AI answer engine
   * will include this entity in response to a relevant query?
   */
  citation_probability: number;
  /**
   * Derived visibility score 0–100.
   * Normalised from citation_probability and cross_model_overlap.
   */
  visibility_score: number;
}

/** Full simulation run across multiple queries */
export interface SimulationRunResult {
  /** The primary entity being simulated */
  primary_entity: string;
  /** The URL being analysed */
  url: string;
  /** scan_id this simulation is attached to (for ledger traceability) */
  scan_id: string | null;
  /** ISO timestamp */
  run_at: string;
  /** Per-query simulation outputs */
  queries: QuerySimulationResult[];
  /** Whether the full run completed, completed partially, or fully failed */
  execution_status: 'complete' | 'partial' | 'failed';
  /** Total model calls attempted across all queries */
  attempted_model_calls: number;
  /** Successful model calls across all queries */
  successful_model_calls: number;
  /** Failed model calls across all queries */
  failed_model_calls: number;
  /**
   * Aggregate AVP across all queries — the headline metric.
   * Average of per-query citation_probability values.
   */
  aggregate_avp: number;
  /**
   * Average cross-model overlap across all queries.
   * Low value = inconsistent entity representation across AI systems.
   */
  average_overlap: number;
  /**
   * Temporal drift delta: AVP change vs last known simulation for this URL.
   * null = no prior simulation exists.
   */
  avp_delta: number | null;
  /** Model labels used in this simulation run */
  models_used: string[];
}